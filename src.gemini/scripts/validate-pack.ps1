...
$shellExecutable = $shellCandidates | Where-Object { Test-Path -LiteralPath $_ } | Select-Object -First 1
...
if ($repoRoot) {
  # -LiteralPath: Set-Location treats a positional -Path argument as a
  # WILDCARD pattern. A repo living under a directory whose name contains a
  # glob-special character (e.g. `repo[v2]`) then fails wildcard resolution
  # -- reproduced directly this session: pwsh 7 throws "Cannot find path
  # ... because it does not exist" (validator never runs); Windows
  # PowerShell 5.1 happens to still succeed here (the opposite engine split
  # from the fallback branch below, so that branch's reasoning does not
  # transfer) -- but relying on an engine-specific accident is not a fix.
  Set-Location -LiteralPath $repoRoot
} else {
  # -LiteralPath is required here: Resolve-Path (and Set-Location) treat a
  # positional -Path argument as a WILDCARD pattern, so a pack living under a
  # directory whose name contains a glob-special character (e.g. `pack[v1]`)
  # would silently resolve to $null on Windows PowerShell 5.1 (a hard
  # PSArgumentNullException at the next line) or relocate the process to
  # $HOME on pwsh 7 (Set-Location $null there resolves to the user's home
  # directory, so the validator would silently validate the WRONG tree
  # instead of failing).
  $runtimeRoot = Resolve-Path -LiteralPath (Join-Path $PSScriptRoot '..\..')
  Set-Location -LiteralPath $runtimeRoot
}
$scriptPath = Join-Path $PSScriptRoot 'validate-pack.sh'
if (-not (Test-Path -LiteralPath $scriptPath)) {
  throw "Unable to locate sibling validate-pack.sh next to $PSCommandPath."
}

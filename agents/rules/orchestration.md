...
- `$consultant`: optional non-blocking independent advisor for the lead; advisory-only and not part of the mandatory pipeline; base usage rules live in `agents/contracts/consultant-workflow.md`, with provider adapters such as `agents/contracts/claude-workflow.md` only when that execution method is selected; if the external provider is unavailable or fails, fall back to an independent subagent consultant using the same advisory-only contract
...
- update `agents/contracts/operating-model.md` when orchestration or gate semantics change
- update `agents/contracts/consultant-workflow.md` and any selected provider adapter when consultant execution policy changes

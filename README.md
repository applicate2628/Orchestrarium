...
- `externalClaudeSecretMode: auto | force` and `externalClaudeApiMode: disabled | auto | force` stay under the `claude` provider. The named Claude API path is the repo-local secret-backed wrapper that runs plain `claude` with `ANTHROPIC_*` from `SECRET.md`; it is a secondary Claude transport, not a fourth provider.

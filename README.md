...
Codex also offers two narrow native Luna roles for deterministic mechanical work: read-only `mechanical-scout` and bounded-write `mechanical-worker`. The caller-neutral `resolve_role_dispatch` policy validates task, role, profile, model, effort, sandbox, and native feature state before returning denied, unavailable, or native-required with no fallback. Host outcomes are nonauthorizing caller observations, never resolver inputs. Slice-A detached validation uses one candidate worktree and publishes only nonauthorizing focused summaries after cleanup.
...
- Codex installs create only absent native role TOMLs under `.codex/agents/`; the source manifest validates the current payload but is never installed as an ownership receipt. Exact existing role bytes are a no-op, while any collision, type mismatch, or reparse point fails without mutation. The installer creates `multi_agent_v2 = true` only when `.codex/config.toml` is absent. An existing ordinary config stays byte-exact when TOML is valid and `features.multi_agent_v2` is absent or Boolean; malformed TOML, a wrong-shaped `features` value, or a non-Boolean flag is preserved byte-exact and fails closed. Codex and Claude compose the same complete canonical `.agents/skills/lead` tree in either install order. Migration, adoption, and reclaim are reserved for 2.0.

...
- `externalCodexProfile: default | gpt-5.5-fast` is the Codex-specific external profile override. The shipped value is `default`, so when `externalProvider: auto` resolves to Codex it inherits `externalModelMode`; `gpt-5.5-fast` is an explicit opt-in that must be verified against the installed Codex runtime.

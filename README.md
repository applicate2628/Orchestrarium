...
- Codex install does not seed `.codex/agents/default.toml`, `worker.toml`, or `explorer.toml`; native subagent model and reasoning selection stays with the runtime or an explicit operator-owned override. Reinstall removes only recognizable retired Orchestrarium-owned copies at those paths and preserves genuinely customized user files.

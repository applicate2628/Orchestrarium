...
Windows Kimi executable admission is global-Codex maintenance. A persistent private `.kimi-binding-v2.lock` file is only a rendezvous name: one share-zero, noninheritable Windows handle provides exclusivity and crash closes it automatically; the file is validated and never unlinked. Under that handle, root/V1/optional-empty-runs are held no-share-delete and checked at C0-C4, live Moonshot evidence authorizes the current executable, and fixed-path receipt transactions rollback before durable decision or forward-settle afterward. Ambiguous recovery, customized/reparse/named-stream state, nonempty runs and unprotected V2 all fail closed. Verify takes the same exclusive probe without creating or changing the rendezvous and remains offline/nonauthorizing. No vendor binary is downloaded or replaced.

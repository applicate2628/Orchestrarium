...
- `externalProvider` accepts `auto | codex | claude | gemini | qwen | kimi | grok`. `auto` remains lane-driven, not host-pack-driven, and shipped production profiles stay on the Codex/Claude pair with `reserve` only as a supplemental advisory/review candidate. Kimi and Grok are explicit-only, read-only, independently verified routes and never enter `auto` profiles.

...
- `$consultant` returns the same first four sections, but replaces the Gate line with `5. Advisory status: NON-BLOCKING`.
...
- `data-engineer` or `data-dev` maps to `$data-engineer`
...
Invocation note:
- `$consultant` maps to the consultant workflow documented in [consultant-workflow.md](consultant-workflow.md)
- provider-specific adapters such as [claude-workflow.md](claude-workflow.md) are optional and should be used only when that provider is installed and selected

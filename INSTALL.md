...
Both Claude Code and Codex CLI expose hook surfaces; both production packs auto-install five structural hooks — three blocking backstops and two warn-only audit hooks:
...
**The installer auto-installs all five hooks by default** in both `--global` and `--target <project>` modes via an idempotent JSON merge that preserves all your other settings and hooks:

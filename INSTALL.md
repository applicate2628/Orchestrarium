...
- The common `$vcpkg-ports-updater` skill covers admitted vcpkg overlay synchronization and read-only named-root inventory sweeps. Codex installs materialize its canonical tree through the existing create-only skill iterator; exact reinstall is a no-op and a differing same-name tree fails closed. Claude carries a provider-neutral body/reference source and uses the existing shared canonical projection at install time; it does not copy Codex-only `agents/openai.yaml` metadata into the Claude source tree.

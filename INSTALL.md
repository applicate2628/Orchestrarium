...
| `python .\scripts\install-codex.py --global --enroll-kimi` | Creates a missing exact Kimi pin; exact replay is a no-op and drift is refused |
| `python .\scripts\install-codex.py --global --replace-kimi-enrollment` | Validates Kimi Code `0.39.1` and atomically rotates only an accepted `0.39.0` rollback pin |
...
- A missing global Windows Kimi binding can be created without reinstalling the pack, and an exact binding can be confirmed without rewriting it: run `python "$HOME\.agents\skills\lead\scripts\invoke-kimi-prompt.py" --enroll-executable`, then check it with the read-only `--verify-enrollment` mode. Ordinary enrollment remains create-only: it accepts the exact current Kimi Code `0.39.1` Windows x64 identity (size `151652352`, SHA-256 `ce3a74ead55994eb1350cc45a1d0d9bf083158f2bba4da49a5ee6168a1830338`) or the retained `0.39.0` rollback identity (size `151532032`, SHA-256 `9ddec448e6de4cacb5c4a07bf57c1909e699a0589c39eda851afdaab47b22dd2`), no-ops on exact replay, and refuses to rewrite a different existing pin. After the fixed executable has been updated to exact `0.39.1`, use `--replace-kimi-enrollment` on either the installed wrapper or the global Codex installer to atomically rotate only an accepted rollback pin; an active replacement lock, validation failure, race, or write failure leaves the old pin untouched or restores it before returning failure. At launch, the exact pin size and SHA-256 travel through the sealed Kimi request/admission path and must match the executable re-observed immediately before process creation; another accepted release is not a substitute for the enrolled member. All maintenance modes stop before provider launch and leave authentication to Kimi Code CLI. Provider launches still set `KIMI_CODE_NO_AUTO_UPDATE=1`; malformed state never falls back to `PATH`, `KIMI_BIN`, or a changed binary.

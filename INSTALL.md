...
- Those shipped override files pin the built-in `default`, `worker`, and `explorer` subagents to `gpt-5.5` with `xhigh` reasoning effort. Reinstall refreshes stale Orchestrarium-owned templates at those paths, even when the stale model string is not the current one, but preserves files whose prompt or structure was actually customized by the user.
...
- `externalCodexProfile` is the Codex-specific external profile override: `default` inherits `externalModelMode` after provider resolution, including under `externalProvider: auto`, while `gpt-5.5-fast` explicitly requests a Codex fast profile when the installed runtime supports it. The shipped presets keep this key on `default`.

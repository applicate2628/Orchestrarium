...
- Those shipped override files pin the built-in `default`, `worker`, and `explorer` subagents to `gpt-5.6-sol` with `xhigh` reasoning effort. Reinstall refreshes stale Orchestrarium-owned templates at those paths, even when the stale model string is not the current one, but preserves files whose prompt or structure was actually customized by the user. A model-only edit in one of those pack-owned template filenames is treated as stale template drift rather than a preserved custom override; use a structurally customized file or a user-added override path when the model-only choice itself must be preserved.
...
- Claude keeps `.claude/CLAUDE.md` short. Leaf role instructions remain under `.claude/agents/*.md`, with five curated exceptions whose canonical contracts live at `.claude/skills/<role>/SKILL.md`: `lead` (loads inline as `/lead`; `.claude/agents/lead.md` is a fail-closed compatibility stub) and the four duals `product-manager`, `analyst`, `architect`, `planner` (each `.claude/agents/<role>.md` is a thin delegate wrapper that loads the same-named skill).
- Project-level installs ensure `/.reports/`, `/.plans/`, `/work-items/`, and `/.scratch/` are present in the target repo `.gitignore` if they are missing, because session logs, plan snapshots, and repo-local task memory are local-only runtime output.
...
- Project-level Gemini installs ensure `/.reports/`, `/.plans/`, `/work-items/`, and `/.scratch/` are present in the target repo `.gitignore` if they are missing, because session logs, plan snapshots, and repo-local task memory are local-only runtime output.
...
project/
  ...
  .claude/
    ...
    AGENTS.md
    CLAUDE.md
    settings.json          ← hook entries (PreToolUse + Stop + SessionStart) merged here
    agents/                ← leaf-role subagents + fail-closed lead stub + delegate-style common-skill wrappers
    commands/
...
- **Stop work-items-archival hook — a per-event sentinel registry, not a single check** (`check-work-items-archival-stop.py` plus thin `.sh`/`.ps1` wrappers; the sibling `workitem_sentinels.py` is the invariant registry, seam for future additions). The wrapper filename is unchanged — Codex hashes the hooks.json entry, never the script's own contents, so a registry change re-trusts nothing on an already-trusted install — and it hosts two invariants: **SEN-0 (archival orphan, `RESOLVE`)**, the original, verdict-equivalent behavior — a delivered or closed work-item left in `work-items/active/` instead of being archived, detected via a `closure.md` or a done-state `status.md` line anchored to the state-key (not free prose), with the per-stop `[acknowledge-open-work-items]` override scoped to this invariant alone (matched against the model's own last message OR the operator's own last genuine typed message); **SEN-1 (dual-state item, `RESOLVE`)** — a work-item slug present in BOTH `work-items/active/` and `work-items/archive/**` (checked across the working tree and `HEAD`), the invariant that directly addresses this guard's own originating incident (a resurrected dual-state item). A third invariant, SEN-2 (delivery drought), was designed and then CUT before release, and a third response tier, `HALT` (`{"continue":false,"stopReason":...}`), was removed with it — both after two live probes (T-14, T-20; see `references-codex/stop-hook-halting-primitives.md`) measured that on the Codex line, neither `stopReason` nor even a bare `systemMessage` reaches the operator, in any payload shape.

  **Read this before relying on this hook's output reaching a human on Codex:** (1) only `RESOLVE` (`decision:"block"`, one model turn) is genuinely cross-line, because it addresses the MODEL, not the operator; (2) every operator-directed output this hook can produce — the `stop_hook_active` escalation notice and the sentinels-unavailable fallback notice, both described below — is Claude-line only; (3) there is no run-terminating tier on either line; (4) a persistent RESOLVE forces a fresh model continuation every user turn, FOREVER -- it does not go silent: `stop_hook_active` is a WITHIN-turn flag that resets at the first Stop fire of each new user turn (measured False on the first of nine envelope fires), so a still-holding finding re-fires to the model at the start of every subsequent turn, costing one continuation each time; the escalation to a notice only happens in the narrower within-turn double-fire case, and even then reaches nobody on Codex -- the net effect there is a per-turn model tax with no operator signal at all, never a one-time notice that then falls silent; (5) that same sentinels-unavailable fallback (FM-1) is itself undeliverable on Codex, so if `workitem_sentinels.py` ever fails to import, this entire guard can vanish with zero operator-visible signal on that line; (6) **a spiral without a resurrection emits nothing at any volume** — SEN-1 addresses this guard's originating incident only because that incident happened to include a resurrection, not because it is a general spiral guard: a spiral WITH a resurrection fires day 0, the identical spiral WITHOUT one produces nothing at 900 files and still nothing at 5,000 files, so an item that only ever accumulates file residue (never a dual-state resurrection) is UNADDRESSED by this release. **The sum of the above, plainly:** on the line the incident happened on, this release adds no enforcement that survives a non-cooperating session; its value there is the extension seam plus a per-turn model prompt.

  It exits immediately when the envelope carries `agent_id` (a subagent context — work-item lifecycle is the MAIN conversation's job) and fails open on any error, including an unimportable registry (a turn-free notice names the gap — Claude-line only, per point 5 above). Registered ONLY on `Stop` (never `SubagentStop`). The same `agent_id` subagent-safety skip is retrofitted onto the passive-polling Stop hook so neither blocking Stop guard interferes with subagents.

...
- Codex installs do not seed built-in subagent overrides into `.codex/agents/default.toml`, `.codex/agents/worker.toml`, or `.codex/agents/explorer.toml`. Native subagent model and reasoning selection stays runtime- or operator-owned.
...
- On reinstall, the Codex installer removes only recognizable retired Orchestrarium-owned `default`, `worker`, and `explorer` files left by older pack versions. Files whose prompt or structure was customized by the user are preserved.
...
- **Stop work-items-archival hook — a per-event sentinel registry, not a single check** (`check-work-items-archival-stop.py` plus thin `.sh`/`.ps1` wrappers; the sibling `workitem_sentinels.py` is the invariant registry). The trust-pinned wrapper filename is unchanged and the registry now hosts three `RESOLVE` invariants: **SEN-0** catches closed/delivered items left active; **SEN-1** catches a slug present in both active and archive state; **SEN-2** gives one model-facing continuation per root user turn for one physically active item whose action is omitted or remains due. Physical membership under `work-items/active/`, not model-authored status prose, owns applicability. When an exact Primary mutation is declared, SEN-2 accepts only a direct semantic exact-target mutation with a same-id explicit-success result; shell/body mentions and failed, ambiguous, missing, or in-flight results do not count. Child and same-turn re-entry contexts are skipped; multiple actionless active items, invalid action input, and invalid transcript input fail open; no hook identity or re-trust changes. The earlier threshold/NOTICE-only SEN-2 design remains historical; `HALT` is still absent and T-14/T-20 still show that operator-directed NOTICE is unreliable on Codex.

  **Read this before relying on this hook's output reaching a human on Codex:** only model-facing `RESOLVE` (`decision:"block"`) is genuinely cross-line; operator-directed NOTICE remains unreliable on Codex, there is no run-terminating tier, and an import failure can therefore remain operator-invisible there. `stop_hook_active` is a within-turn flag: it suppresses a repeated same-turn SEN-2 block, while the next genuine user boundary re-arms work that is still due. SEN-2 uses physical active membership for omission and exact action correlation when declared; it is not a generic residue or file-count detector and has no prose bypass.
...
- **PreToolUse mcp-momentum hook** (`check-mcp-momentum.py` plus thin `.sh`/`.ps1` wrappers; same typed `hooks/` dir, registered on the exact `Grep|Bash|PowerShell|shell_command|exec_command` matcher): a **warn-only AUDIT** consumer of the shared three-event [MCP continuity policy](shared/references/mcp-continuity.md). It recognizes native `Grep`; shell-shaped inputs using default-recursive `rg`/`ag`/`ack` or explicit-recursive `grep`; source scopes, source selectors, symbol patterns, and `rg --files`; and it stays silent only when every explicit scope resolves from the raw envelope `cwd` to `work-items/`, `.reports/`, `.plans/`, or `.scratch/` at the nearest repository root. A matching segment at any other depth is not exempt, and mixed source/exempt scopes still fire. Root and `agent_id` envelopes are evaluated identically. A hit emits `hookSpecificOutput.additionalContext`, but both hit and miss exit 0, no path exits 2, and errors fail open. Advisory output contains only safe matched MCP server names (at most three plus a count), never commands, environment values, or tokens. The hook can re-anchor tool choice; it cannot prove model obedience.

...
The Codex installer materializes the native `mechanical-scout` and `mechanical-worker` Luna role TOMLs through the existing create-only role path. An exact pre-existing payload is retained; a differing user-owned payload is preserved and makes the install fail. These roles are native shared-policy corridors, not new agents-mode or external-provider settings. Dispatch callers consume the policy-only `resolve_role_dispatch` contract immediately before native launch; host outcomes are not replayed and no fallback exists.
...
- Codex installs create only absent native role TOMLs under `.codex/agents/`. The source manifest validates current payload digests and is never installed as an ownership receipt. Identical regular files are no-ops; changed bytes, type collisions, and reparse points fail without mutation. Codex and Claude publish the same complete canonical `.agents/skills/lead` tree in either order, and Claude discovery projections follow the same create-only rule. The installer creates `[features] multi_agent_v2 = true` only when `.codex/config.toml` is absent. An existing ordinary config remains byte-exact when TOML is valid and the feature is absent or Boolean; malformed TOML, a non-table `features` value, or a non-Boolean flag remains byte-exact but fails the install. Adoption, update, deletion, and reclaim are 2.0-only migration work.
- Codex hook inventory authority is exactly `codex-hook-inventory.json` beside the selected `hooks.json`; an alternate name, outside directory, symlink, junction, reparse point, or changed parent identity is rejected. Hook-health failures use a strict 4,096-byte internal envelope and the two health-owned failure identifiers, while successful health output is temporary-file-backed and replayed byte-for-byte without a semantic size ceiling. Failed installs continue every independent rollback action, preserve the original typed cause, remove disposable backups after successful restoration, and report a retained recovery directory when snapshot restoration cannot complete.
...
- On reinstall, Codex native roles remain create-only: absent roles are created, byte-identical files are no-ops, and every differing existing file is preserved while the install fails. There is no installed receipt, historical adoption, update, deletion, or reclaim authority in 1.x.

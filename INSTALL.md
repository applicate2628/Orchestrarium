...
| `python .\scripts\install-codex.py --global --enroll-kimi` | Validates the installed fixed Kimi executable against the live Moonshot manifest and writes its monotonic V2 admission receipt |
| `python .\scripts\install-codex.py --global --enroll-kimi --kimi-offline-policy 24h` | Enables bounded offline replay of only the last exact admitted object; choices are `disabled`, `24h`, or `7d` |
| `python .\scripts\install-codex.py --global --replace-kimi-enrollment` | Compatibility alias for the same live V2 refresh; it no longer rotates a source-pinned release list |
...
- Global Windows Kimi admission validates the already-installed fixed `~/.kimi-code/bin/kimi.exe`; Orchestrarium never downloads or replaces the vendor executable. One persistent `.kimi-binding-v2.lock` rendezvous remains in place across runs; exclusivity exists only while its noninheritable, share-zero Windows handle is open, so crash release needs no stale-file deletion. Writers hold it through recovery, hardening, live evidence, V2 publication and reclaim; strict verify first requires an existing V2 receipt and returns `E_KIMI_V2_RECEIPT_INVALID` without opening the lock or probing when it is absent. With V2 present, verify opens the existing rendezvous read-only/exclusively, returns `E_KIMI_ADMISSION_LOCK_MISSING` if it vanished, treats active sharing as BUSY, and keeps the lock held through receipt, executable, and probe validation. V2 writes use only fixed transaction/update/candidate/rollback/receipt paths, with `.kimi-v2-receipt.txn.update` as the update participant; pre-decision failure rolls back, post-decision recovery forward-settles, and ambiguity makes every reader deny. The sole inherited-ACL corridor is an ordinary fixed root with canonical V1 and optional empty `runs/`; root/V1/runs are held no-share-delete, enumerated/read through handles, hardened on those handles, and revalidated at C0-C4. Nonempty runs, extras, customized state, links/reparse points, named streams or unprotected existing V2 deny.
...
When installation reports `PENDING_MANUAL_TRUST`, start interactive `codex` — not `codex exec` — and choose **Trust all and continue** after reviewing every listed affected identity. A first install can affect all 12 Codex entries; an unchanged registration identity requires no repeat trust step.
Do not press Esc and do not choose **`Continue without trusting`**, because the affected hooks and guards remain installed but inactive.
`codex exec` silently skips untrusted hook entries instead of showing the trust prompt, so interactive `codex` must run first.
The trust modal does not time out; review every affected entry before making the explicit choice.

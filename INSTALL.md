...
Both Claude Code and Codex CLI expose hook surfaces; both production packs auto-install seven structural hooks — four blocking backstops and three warn-only audit hooks — plus two informational `SessionStart` reminders (nine entries total):
...
**The production installers auto-register all nine entries by default** in both `--global` and `--target <project>` modes via an idempotent JSON merge that preserves all your other settings and hooks:

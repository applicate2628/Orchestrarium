...
On Windows, protected child-process working directories must be owned by the current token user or its exact default owner. This admits Windows-created directories under an elevated token without accepting arbitrary groups or changing the POSIX ownership checks. Source files covered by exact-byte manifests use narrowly scoped LF (Line Feed) checkout rules; historical accepted-prior payloads and unrelated files are not normalized.

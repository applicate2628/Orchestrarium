...
For a later global upgrade, a lead runs Claude Code first, checks that the dry run names exactly 26 wrappers from 13 owned stems and no non-hook file, then installs and verifies:
...
Then run Codex, checking that its dry run names exactly 24 wrappers from 12 owned stems:

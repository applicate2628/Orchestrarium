...
- `externalClaudeApiMode` is the named Claude secondary-transport toggle: `disabled` forbids the secret-backed Claude API path, `auto` keeps the installed secret-backed wrapper as the fallback after the allowed Claude CLI path is exhausted, and `force` uses that wrapper-backed path as the primary Claude transport immediately. It remains a Claude transport, not a separate provider.

...
| Repo consistency audit | `$knowledge-archivist` | Weekly | `README.md`, `INSTALL.md`, `docs/`, `shared/references/`, `references-qwen/`, and `src.qwen/` remain consistent | Open a bounded docs or hygiene follow-up; if the fix changes governance semantics, route it through `$architecture-reviewer` before publication |

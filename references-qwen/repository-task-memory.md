...
`shared/references/` is the canonical home for stable repository-wide design methodology. `references-qwen/` keeps Qwen-specific reference material plus compatibility pointers. `docs/agents-mode-reference.md` is the shared operator reference when `.qwen/.agents-mode.yaml` behavior matters.

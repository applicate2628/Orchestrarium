...
`shared/references/` — канонический дом для стабильной общерепозиторной design-методологии. `references-qwen/` хранит Qwen-specific reference material и compatibility pointers. `docs/agents-mode-reference.md` — общий operator reference, когда важно поведение `.qwen/.agents-mode.yaml`.

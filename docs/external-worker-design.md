...
2. **Extended config format**
   ...
   - `externalCodexProfile` — shared Codex-specific profile override: `default` inherits `externalModelMode`; `gpt-6-sol-xhigh` (shipped default) and `gpt-6-sol-max` request the named GPT-6 Sol effort; `gpt-5.6-terra` keeps the balanced Codex model at `high`; explicit `gpt-5.6-sol-xhigh` and `gpt-5.6-sol-max` values retain their named model and effort through 1.x
...
4. **Common rules**
   ...
   - Under `externalCodexProfile: default`, Codex inherits `externalModelMode`; `gpt-6-sol-xhigh` and `gpt-6-sol-max` request the named GPT-6 Sol effort; `gpt-5.6-terra` keeps the balanced Codex model at `high`; explicit `gpt-5.6-sol-xhigh` and `gpt-5.6-sol-max` values retain their named model and effort through 1.x.

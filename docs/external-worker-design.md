...
2. **Extended config format**
   ...
   - `externalCodexProfile` — shared Codex-specific profile override: `default` inherits `externalModelMode`; `gpt-5.6-sol-max` requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; `gpt-5.6-terra` selects the balanced Codex model after provider resolution (a distinct model, `model_reasoning_effort = "high"`, not an effort suffix) and must be verified against the installed runtime; `gpt-5.6-sol-xhigh` (shipped as default) pins model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`, symmetric to Claude's `opus-xhigh`
...
4. **Common rules**
   ...
   - Under `externalModelMode: pinned-top-pro`, hard tasks start on the strongest documented production provider-native path: Codex model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` supplied through a supported config/profile path, or `opus-max` / `--model opus --effort max` on Claude CLI. Example-provider model behavior stays provider-local and is intentionally outside this production design spec.
   - Under `externalCodexProfile: default`, Codex inherits `externalModelMode`; under `externalCodexProfile: gpt-5.6-sol-max`, Codex requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; under `externalCodexProfile: gpt-5.6-terra`, Codex selects the installed runtime's balanced model (a distinct model, `model_reasoning_effort = "high"`, not an effort suffix) and must record unavailable or deviated if that model cannot be verified; under `externalCodexProfile: gpt-5.6-sol-xhigh` (shipped as default), Codex requests model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`.
...
- `externalProvider: auto | codex | claude | gemini | qwen | kimi | grok` — `auto` resolves through the shipped Codex/Claude profile; Kimi is explicit-only Windows no-tools bundle review and Grok is unavailable in 1.x
...
- `externalCodexProfile: default | gpt-5.6-sol-xhigh | gpt-5.6-sol-max | gpt-5.6-terra` — shared Codex-specific profile override.
  `default` inherits `externalModelMode`; `gpt-5.6-sol-xhigh` (shipped as default) pins model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`; `gpt-5.6-sol-max` requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; `gpt-5.6-terra` is the balanced mid-tier reasoning lane (a distinct model, `model_reasoning_effort = "high"`, not an effort suffix; replaces the former `gpt-5.5-fast` and `gpt-5.3-codex-spark`) and is explicit opt-in after provider resolution
...
- `externalClaudeProfile: sonnet-high | opus-xhigh | opus-max | fable-xhigh` — Codex-line only.
  When Codex dispatches to Claude CLI, prefer the matching model/effort profile instead of the provider default or the broader shared model policy; `fable-xhigh` maps to `--model fable --effort xhigh` (the current flagship-family best-effort tier; the `fable` flagship alias as of 2026-07, recorded from the installed model list rather than a verified capability ranking)

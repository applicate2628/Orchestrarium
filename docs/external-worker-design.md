...
2. **Extended config format**
   ...
   - `externalCodexProfile` — shared Codex-specific profile override: `default` inherits `externalModelMode`; `gpt-5.6-sol-max` requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; `gpt-5.6-luna` selects the fast/volume Codex model after provider resolution (a distinct model, `model_reasoning_effort = "medium"`, not an effort suffix) and must be verified against the installed runtime; `gpt-5.6-sol-xhigh` (shipped as default) pins model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`, symmetric to Claude's `opus-xhigh`
...
4. **Common rules**
   ...
   - Under `externalCodexProfile: default`, Codex inherits `externalModelMode`; under `externalCodexProfile: gpt-5.6-sol-max`, Codex requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; under `externalCodexProfile: gpt-5.6-luna`, Codex selects the installed runtime's fast/volume model (a distinct model, `model_reasoning_effort = "medium"`, not an effort suffix) and must record unavailable or deviated if that model cannot be verified; under `externalCodexProfile: gpt-5.6-sol-xhigh` (shipped as default), Codex requests model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`.
...
- `externalCodexProfile: default | gpt-5.6-sol-xhigh | gpt-5.6-sol-max | gpt-5.6-luna` — shared Codex-specific profile override.
  `default` inherits `externalModelMode`; `gpt-5.6-sol-xhigh` (shipped as default) pins model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`; `gpt-5.6-sol-max` requests model `gpt-5.6-sol` with `model_reasoning_effort = "max"` for higher-complexity/hard lanes; `gpt-5.6-luna` is the bounded mechanical-overflow / fast-volume path (a distinct model, `model_reasoning_effort = "medium"`, not an effort suffix; replaces the former `gpt-5.5-fast` and `gpt-5.3-codex-spark`) and is explicit opt-in after provider resolution

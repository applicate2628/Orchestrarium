...
|---|---|
| project-local | `<project>/AGENTS.md`, `<project>/.agents/skills/`, `<project>/.agents/.agents-mode.yaml`, `<project>/.codex/agents/` |
| global | `~/.codex/AGENTS.md`, `~/.codex/skills/`, `~/.codex/.agents-mode.yaml`, `~/.codex/agents/` |

## Notes

- `references-codex/` and `docs/` stay in the source branch; they are not copied into target projects.
- Installs seed the operator file into the active target: `.agents/.agents-mode.yaml` for project installs and `~/.codex/.agents-mode.yaml` for global installs.

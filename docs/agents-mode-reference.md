...
| `kimi` | Kimi explicit-only read-only route | Windows only; policy admits exploration, research, planning, and review through the canonical wrapper with fixed `kimi-code/k3` and no tools or subagents. It is independently verified, nonauthorizing, and never part of `auto`. |

...
| `kimi` | Kimi explicit-only bundle-review route | Windows only; requires a local installer enrollment pin and fixed no-tools/no-subagents bundle transport. It is independently verified, nonauthorizing, and never part of `auto`. |
...
- Example-only Gemini/Qwen, explicit-only Kimi, and unavailable Grok must not appear in shipped or repo-local `auto` production profiles.

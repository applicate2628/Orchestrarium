...
| `kimi` | Explicit-only Kimi Code route | Only eligible read-only `exploration`, `planning`, or `review` work can be admitted by `resolve_external_dispatch`; `auto` and profile orders never select it. Kimi uses `kimi-code/k3`, exposes no native effort control, runs from a neutral scratch snapshot, and requires independent verification. |
| `grok` | Explicit-only official Grok route | Only eligible read-only `exploration`, `planning`, or `review` work can be admitted by `resolve_external_dispatch`; `auto` and profile orders never select it. The signed official Grok binary, model/auth/capability/result-shape gates, and live-root immutability proof are required before launch. |
...
- Example-only Gemini/Qwen and explicit-only Kimi/Grok must not appear in shipped or repo-local `auto` production profiles. Kimi/Grok are dispatch-only names, not provider overlay directories.
- Every explicit Kimi/Grok route must pass `resolve_external_dispatch(provider, task_class, role)` before probing or spawning. It allows only eligible read-only exploration, planning, and review routes, requires independent verification, and never falls back; neither provider result alone can authorize a critical decision, commit, push, release, or publication.
...
- Example-only Gemini/Qwen and explicit-only Kimi/Grok require an explicit scalar provider override and are not valid profile entries.

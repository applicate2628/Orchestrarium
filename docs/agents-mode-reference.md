...
| `kimi` | Kimi policy classifier name | Unavailable and disabled in 1.x. `resolve_external_dispatch` may describe the non-executing policy result but does not authorize a launcher or probe. |
...
- Example-only Gemini/Qwen and unavailable Kimi/Grok must not appear in shipped or repo-local `auto` production profiles. Kimi/Grok are policy names, not provider overlay directories.
- Kimi/Grok policy resolution is non-executing. In 1.x it must not be followed by a provider probe or spawn, and it does not establish that either provider can be used.

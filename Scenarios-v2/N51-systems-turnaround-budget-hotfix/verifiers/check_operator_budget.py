...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N51 visible operator-output budget.")
...
def main():
    ...
    if args.bundle_shape_only:
        print("N51 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N51 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

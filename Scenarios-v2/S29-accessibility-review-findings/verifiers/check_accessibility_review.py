...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the S29 accessibility-review bundle shape or a completed review report."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the S29 bundle root.",
    )
    parser.add_argument(
        "--bundle-shape-only",
        action="store_true",
...
def load_contract(bundle_root: Path):
    contract_path = bundle_root / "oracle" / "accessibility-review-contract.json"
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(metadata == contract["expected_metadata"], "scenario.yaml metadata does not match S29", errors)
...
def check_completed_report(bundle_root: Path, contract, errors):
    ...
    for snippet in contract["prohibited_report_snippets"]:
        require(
            ...
            f"review-report.md contains prohibited snippet: {snippet}",
            errors,
        )
...
def main():
    ...
    print(f"S29 verifier PASS ({mode})")
    return 0

...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N23 owner recovery routing packet.")
...
def evaluate_packet(root: Path, contract: dict):
    failures = []
    packet = root / "candidate" / "owner-recovery-routing-decision.md"
...
def main():
    ...
    if not args.bundle_shape_only:
        ...
        if args.expect_start_state:
            ...
            errors.append(f"Completed N23 packet still fails owner recovery checks: {json.dumps(failures, indent=2)}")
    ...
    print(f"N23 verifier PASS ({mode})")
    return 0

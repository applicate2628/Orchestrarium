...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N41 changed paths stay in scope.")
...
def main():
    ...
    contract = json.loads((root / "oracle" / "staged-incident-budget-contract.json").read_text(encoding="utf-8"))
    ...
    expected_max = len(contract["requiredChangedPaths"])
    if budget.get("maxChangedPaths") != expected_max:
        print(f"ERROR: patchBudget.maxChangedPaths must be {expected_max}", file=sys.stderr)
        return 1
    if actual != required:
    ...
    if len(actual) > budget["maxChangedPaths"]:
        ...
        return 1
    print("N41 scope PASS (changed paths are in bounds)")
    return 0

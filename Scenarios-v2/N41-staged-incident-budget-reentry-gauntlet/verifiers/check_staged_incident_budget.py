...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N41 staged incident-budget reentry gauntlet.")
...
def evaluate_repair_ledger(root: Path):
    ...
    def validation_ledger(ledger):
        ...
        joined = "\n".join(commands)
        if not any(marker in joined for marker in ["check_staged_incident_budget.py", "unittest discover", "test_deploygrid.py"]):
    ...
    def patch_budget_ledger(ledger):
        ...
        assert_equal(budget.get("maxChangedPaths"), len(REQUIRED_BUDGET_PATHS), "maxChangedPaths")
...
def evaluate_reentry_state(root: Path, contract: dict):
    path = root / "candidate" / "reentry-state.json"
    try:
    ...
    if state.get("contractId") != contract["contractId"] or state.get("planFingerprint") != contract["planFingerprint"]:
        return [{"id": "reentry-state-schema", "detail": "contract or fingerprint mismatch"}]
    phase_ids = {
        item.get("id") or item.get("phaseId")
        for item in state.get("phases", [])
        if isinstance(item, dict)
    }
    if set(contract["expectedPhaseIds"]) - phase_ids:
        failures.append({"id": "reentry-phase-ledger", "detail": "missing phase ids"})
    if not contains_all(text, contract["expectedSourceIds"]):
        failures.append({"id": "reentry-source-ledger", "detail": "missing accepted source ids"})
    if not contains_all(text, contract["expectedStaleIds"]):
        failures.append({"id": "reentry-source-ledger", "detail": "missing stale source rejections"})
    if not contains_all(text, contract["runtimeClassificationMarkers"]):
        failures.append({"id": "reentry-runtime-classification", "detail": "runtime classification markers incomplete"})
    return failures


def evaluate_closeout(root: Path, contract: dict):
    path = root / "candidate" / "closeout.json"
    try:
        closeout = load_json(path)
    except Exception as exc:  # noqa: BLE001
        return [{"id": "closeout-schema", "detail": f"invalid JSON: {exc}"}]

    failures = []
    text = json.dumps(closeout, sort_keys=True)
    if closeout.get("contractId") != contract["contractId"] or closeout.get("planFingerprint") != contract["planFingerprint"]:
        return [{"id": "closeout-schema", "detail": "contract or fingerprint mismatch"}]
    if sorted(closeout.get("changedPaths", [])) != sorted(contract["requiredChangedPaths"]):
        failures.append({"id": "closeout-changed-paths", "detail": "changed paths mismatch"})
    if not closeout.get("outcome") or "resumePoint" not in closeout or "residualRisk" not in closeout:
        failures.append({"id": "closeout-schema", "detail": "outcome, resumePoint, or residualRisk missing"})
    if not contains_all(text, contract["closeoutMarkers"]):
        failures.append({"id": "closeout-markers", "detail": "closeout markers incomplete"})
    return failures


def evaluate_invariants(root: Path):
    api = import_api(root)
    contract = load_json(root / "oracle" / "staged-incident-budget-contract.json")
    failures = []
    ...
    failures.extend(evaluate_closeout(root, contract))
    return failures
...
def main():
        ...
        if args.expect_start_state:
            ...
            if failures:
                for failure in failures:
                    errors.append(f"Failed invariant: {failure['id']} :: {failure.get('detail', '')}")
    ...
    if args.bundle_shape_only:
        ...
        mode = "completed run"
    print(f"N41 verifier PASS ({mode})")
    return 0

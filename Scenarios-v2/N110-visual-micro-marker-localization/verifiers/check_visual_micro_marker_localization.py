...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check N110 visual micro-marker localization answers.")
...
def check_shape(root: Path, errors: list[str]) -> None:
    ...
    require(scenario.get("id") == SCENARIO_ID, "scenario.yaml id mismatch", errors)
    require(scenario.get("surface_id") == "RF12-L12", "scenario.yaml surface_id mismatch", errors)
    require("image-localization" in scenario.get("overlay_flags", []), "scenario.yaml missing image-localization flag", errors)

    required_paths = [
        "inputs/task.md",
        "inputs/micro-marker-canvas.png",
        "oracle/answer-schema.json",
        "oracle/localization-oracle.json",
        "oracle/reference-answer.json",
        "candidate/answer.json",
        "verifiers/check_visual_micro_marker_localization.py",
    ]
    ...
    require(oracle.get("scenario_id") == SCENARIO_ID, "oracle scenario_id mismatch", errors)
    require(oracle.get("image_id") == IMAGE_ID, "oracle image_id mismatch", errors)
    ...
    require(thresholds.get("pass_mean_px") == 8.0, "oracle pass_mean_px mismatch", errors)
    require(thresholds.get("pass_max_px") == 14.0, "oracle pass_max_px mismatch", errors)
...
def score_answer(root: Path, answer_path: Path) -> dict[str, Any]:
    ...
    if answer.get("image_id") != IMAGE_ID:
        raise ValueError(f"image_id must be {IMAGE_ID}")
    ...
    pass_mean = float(thresholds["pass_mean_px"])
    pass_max = float(thresholds["pass_max_px"])
    passed = (
        not missing_ids
        and not duplicate_ids
        and not unknown_ids
        and not format_errors
        and mean_error <= pass_mean
        and max_error <= pass_max
    )
    ...
    within_window_count = sum(
        1 for distance in errors_by_id.values()
        if distance is not None and distance <= pass_max
    )
    format_penalty = (5.0 * len(missing_ids)) + (3.0 * len(duplicate_ids)) + (4.0 * len(unknown_ids))
    score_components = {
        "coverage": round(20.0 * ((len(REQUIRED_POINTS) - len(missing_ids)) / len(REQUIRED_POINTS)), 3),
        "format": round(max(0.0, 15.0 - format_penalty), 3),
        "within_window": round(25.0 * (within_window_count / len(REQUIRED_POINTS)), 3),
        "mean_error": round(25.0 * max(0.0, 1.0 - (finite_mean / 120.0)), 3),
        "max_error": round(15.0 * max(0.0, 1.0 - (finite_max / 180.0)), 3),
    }
    ...
    return {
        ...
        "matched_id_count": len(REQUIRED_POINTS) - len(missing_ids),
        "within_window_ids": [
            target_id
            for target_id, distance in errors_by_id.items()
            if distance is not None and distance <= pass_max
        ],
...
def main() -> int:
    ...
    if args.bundle_shape_only:
        print("N110 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        if metrics.get("passed"):
            print("ERROR: expected start-state candidate to fail, but it passed")
            print(json.dumps(metrics, indent=2))
            return 1
        print("N110 verifier PASS (expected start-state failures present)")
        return 0

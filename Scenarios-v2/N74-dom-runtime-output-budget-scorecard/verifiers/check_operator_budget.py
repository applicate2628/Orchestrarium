...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N74 low-noise operator output budget.")
...
def main() -> int:
    ...
    contract = json.loads((root / "oracle" / "dom-runtime-contract.json").read_text(encoding="utf-8"))
    max_bytes = int(contract.get("operator_budget", {}).get("max_worker_output_bytes", 0))
    ...
    if str(max_bytes) not in task_text or "worker-output.txt" not in task_text:
        print("ERROR: task.md must visibly declare the worker-output.txt byte budget", file=sys.stderr)
        return 1

    if args.bundle_shape_only:
        print("N74 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N74 operator-budget PASS ({size} <= {max_bytes} bytes)")
    return 0

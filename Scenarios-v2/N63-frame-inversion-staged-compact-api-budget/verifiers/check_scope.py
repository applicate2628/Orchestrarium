...
def main():
    parser = argparse.ArgumentParser(description="Check N63 compact changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N63 patch budget: {extra}")

    if errors:
        print("N63 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N63 scope PASS")

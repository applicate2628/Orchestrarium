...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N63 visible operator-output budget.")
...
def main():
    ...
    if args.bundle_shape_only:
        print("N63 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N63 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

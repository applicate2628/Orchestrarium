...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N94 staged multipackage protocol reentry bundle.")
...
def evaluate_source_ledger(root: Path, contract: dict):
    ...
    for marker in contract["requiredLedgerRows"]["sourceOwners"]:
        if marker not in text:
            ...
            break
    bindings = as_list(ledger.get("phaseBindings"))
    phase_by_id = {
        first_nonempty(item.get("phaseId"), item.get("phase"), item.get("id")): item
        for item in bindings
        if isinstance(item, dict)
    }
    required_keys = set(contract["stagedRequirements"]["phaseBindingRequiredKeys"])
    for phase_id in contract["expectedPhaseIds"]:
        item = phase_by_id.get(phase_id)
        if not item:
            failures.append({"id": "source-ledger-phase-bindings", "detail": f"missing phase binding {phase_id}"})
            break
        missing = sorted(required_keys - set(item))
        if missing:
            failures.append({"id": "source-ledger-phase-bindings", "detail": f"{phase_id} missing keys: {missing}"})
            break
        if not as_list(item.get("sourceIds")):
            failures.append({"id": "source-ledger-phase-bindings", "detail": f"{phase_id} missing sourceIds"})
            break
        if not all(item.get(key) for key in ["owner", "stateBefore", "stateAfter", "visibleArtifact", "validationCue", "handoffCue"]):
            failures.append({"id": "source-ledger-phase-bindings", "detail": f"{phase_id} has empty staged fields"})
            break
    phase_text = json_text(bindings)
    for expected_owner in contract["stagedRequirements"]["phaseOwners"].values():
        if expected_owner not in phase_text:
            failures.append({"id": "source-ledger-phase-owners", "detail": f"missing staged owner {expected_owner}"})
            break
    return failures
...
def evaluate_migration_state(root: Path, contract: dict):
    ...
    if state.get("phaseOrder") != contract["expectedPhaseIds"]:
        failures.append({"id": "phase-ledger-order", "detail": "phaseOrder mismatch"})
    for phase_id in contract["expectedPhaseIds"]:
        ...
        if not (phase.get("owner") or phase.get("ownerPath")):
            ...
            break
        for key in ["stateBefore", "stateAfter", "visibleArtifact", "validationCue", "handoffCue"]:
            if not phase.get(key):
                failures.append({"id": "phase-ledger-complete", "detail": f"{phase_id} missing {key}"})
                break
...
def evaluate_sdk_compat_ledger(root: Path, contract: dict):
    ...
    for source_id in ["S4", "S5", "S6", "S7", "S8", "S9", "S10"]:
        if source_id not in text:
            ...
            break
    cases = as_list(ledger.get("compatibilityCases"))
    case_text = json_text(cases)
    for case_id in contract["requiredLedgerRows"]["compatibilityCases"]:
        if case_id not in case_text:
            failures.append({"id": "sdk-compat-cases", "detail": f"missing case {case_id}"})
            break
    for item in cases:
        if not isinstance(item, dict):
            failures.append({"id": "sdk-compat-cases", "detail": "compatibilityCases entries must be objects"})
            break
        if not all(item.get(key) for key in ["case", "owner", "publicApi", "validationCue", "replayCue"]):
            failures.append({"id": "sdk-compat-cases", "detail": "compatibility case missing staged fields"})
            break
        if not as_list(item.get("sourceIds")):
            failures.append({"id": "sdk-compat-cases", "detail": "compatibility case missing sourceIds"})
            break
    return failures
...
def evaluate_reentry_state(root: Path, contract: dict):
    ...
    if state.get("finalFreshSession") is not True:
        failures.append({"id": "reentry-fresh-session", "detail": "finalFreshSession must be true"})
    if state.get("phase") != "05-review-closeout":
        failures.append({"id": "reentry-fresh-session", "detail": "phase must be 05-review-closeout"})
    for marker in contract["requiredLedgerRows"]["reentryMarkers"]:
        if marker not in text:
            ...
            break
    replay_text = json_text(state.get("downstreamReplay", []))
    for case_id in contract["stagedRequirements"]["reentryReplayCases"]:
        if case_id not in replay_text:
            failures.append({"id": "reentry-replay-cases", "detail": f"missing replay case {case_id}"})
            break
...
def evaluate_review_response(root: Path, contract: dict):
    ...
    for review_id, decision in contract["reviewDecisions"].items():
        ...
        if not item.get("validationCue"):
            ...
            break
        if not item.get("visibleReturnCue"):
            failures.append({"id": "review-response-complete", "detail": f"{review_id} missing visibleReturnCue"})
            break
        if not as_list(item.get("sourceIds")):
            failures.append({"id": "review-response-complete", "detail": f"{review_id} missing sourceIds"})
            break
    return failures
...
def evaluate_closure(root: Path, contract: dict):
    ...
    if closure.get("status") != "ready-for-verifier":
        failures.append({"id": "closure-readiness", "detail": "status must be ready-for-verifier"})
    if closure.get("phaseOrder") != contract["expectedPhaseIds"]:
        failures.append({"id": "closure-readiness", "detail": "phaseOrder mismatch"})
    if closure.get("nextAction") != "run-hidden-verifier":
        failures.append({"id": "closure-readiness", "detail": "nextAction must be run-hidden-verifier"})
    for flag in contract["stagedRequirements"]["closureRequiredFlags"]:
        if closure.get(flag) is not True:
            failures.append({"id": "closure-readiness", "detail": f"{flag} must be true"})
            break
    validation_text = json_text(closure.get("validation", []))
    for marker in ["python candidate/workspace/tests/test_protocolmesh.py", "check_staged_multipackage_protocol_reentry.py", "clean-room protocol/sdk import", "CLI contract replay"]:
...
def main():
    ...
    contract = load_json(root / "oracle" / "staged-multipackage-protocol-contract.json")
    ...
    if args.bundle_shape_only:
        print("N94 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N94 verifier PASS (expected start-state failures present)")
        return 0
    ...
    if failures:
        ...
        return 1
    print("N94 verifier PASS (100.0 / 100)")
    return 0

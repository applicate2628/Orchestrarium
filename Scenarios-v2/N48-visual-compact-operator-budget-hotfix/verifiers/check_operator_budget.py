...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N48 visible operator-output budget.")
...
def main():
    ...
    contract_path = root / "oracle" / "visual-contract.json"
    ...
    if args.bundle_shape_only:
        print("N48 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N48 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

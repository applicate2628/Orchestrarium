...
def incident_field(points: np.ndarray, wavenumber: float, incident_angle: float) -> np.ndarray:
    # BUG: the starter ignores the supplied incident angle.
...
def solve_cylinder_mom(
    ...
    for source, source_point in enumerate(points):
        ...
        # BUG: midpoint-only model omits panel arc length and self-panel quadrature.
...
def solve_hydrogen_radial(
    ...
    # BUG: placeholder shape, not a finite-difference tridiagonal eigenstate.
...
def write_starter_report() -> None:
    report = {
        "scenario_id": "N58",
        "method": "starter report",
        "oracle_use": "not validated",
        "cases": [],
        "performance": {},
        "stage_trace": [],
        "residual_risk": "starter state has not completed high-load validation",
    }
    Path(__file__).parents[1].joinpath("optimization-report.json").write_text(

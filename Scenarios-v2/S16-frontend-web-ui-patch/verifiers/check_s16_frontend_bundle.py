...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the S16 frontend bundle shape or a completed candidate repair."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the S16 bundle root.",
    )
    parser.add_argument(
        "--bundle-shape-only",
        action="store_true",
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    if scenario_path.exists():
        ...
        require(
            ...
            "scenario.yaml values do not match the accepted S16 contract",
            errors,
        )
...
def check_completed_candidate(bundle_root: Path, contract, errors):
    for relative_path in contract["editable_candidate_files"]:
        require((bundle_root / relative_path).exists(), f"Missing editable candidate file: {relative_path}", errors)
...
def main():
    ...
    contract = load_json(bundle_root / "oracle" / "frontend-contract.json")
    ...
    mode = "bundle shape" if args.bundle_shape_only else "completed frontend repair"
    print(f"S16 verifier PASS ({mode})")
    return 0

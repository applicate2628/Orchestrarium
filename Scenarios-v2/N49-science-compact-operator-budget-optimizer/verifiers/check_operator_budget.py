...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N49 visible operator-output budget.")
...
def main():
    ...
    contract_path = root / "oracle" / "optimizer-contract.json"
    ...
    if args.bundle_shape_only:
        print("N49 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N49 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

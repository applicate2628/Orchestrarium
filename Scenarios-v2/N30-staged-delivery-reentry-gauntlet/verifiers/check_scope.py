...
def main():
    parser = argparse.ArgumentParser(description="Check N30 cumulative changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N30 patch budget: {extra}")

    if errors:
        print("N30 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N30 scope PASS")

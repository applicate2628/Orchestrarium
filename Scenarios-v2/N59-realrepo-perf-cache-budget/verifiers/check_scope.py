...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check N59 changed-path and patch-quality budget.")
    ...
    if args.bundle_shape_only:
        print("N59 scope PASS (bundle shape)")
        return
    ...
    if extra:
        errors.append(f"Changed paths outside N59 patch-quality budget: {extra}")
    if len(observed) > 7:
        errors.append(f"Too many benchmark changed paths for N59 patch-quality budget: {len(observed)} > 7")

    if errors:
        print("N59 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N59 scope PASS")

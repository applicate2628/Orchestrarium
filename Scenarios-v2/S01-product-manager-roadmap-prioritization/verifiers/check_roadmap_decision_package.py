...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the S01 roadmap bundle shape or a completed roadmap decision package."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the S01 bundle root.",
    )
    parser.add_argument(
        "--bundle-shape-only",
        action="store_true",
...
def load_contract(bundle_root: Path):
    contract_path = bundle_root / "oracle" / "roadmap-contract.json"
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    if candidate_root.exists():
        ...
        require(
            ...
            "candidate/ contains files beyond the single roadmap decision package contract",
            errors,
        )
...
def check_completed_packet(bundle_root: Path, contract, errors):
    packet_path = bundle_root / "candidate" / "roadmap-decision-package.md"
    require(packet_path.exists(), "Missing candidate/roadmap-decision-package.md", errors)
    if errors:
        return
    ...
    check_ordered_headings(text, contract["required_priority_headings"], "priority heading", errors)
    check_ordered_headings(text, contract["required_deferred_headings"], "deferred heading", errors)
    ...
    for alternatives in contract["required_keyword_groups"]:
        require(
            ...
            f"Package is missing required keyword coverage from: {alternatives}",
            errors,
        )

    status_pattern = r"(?ms)^## Decision status\s+([A-Z][A-Z -]+)\s*\Z"
    match = re.search(status_pattern, text)
    require(match is not None, "Decision status section is missing or malformed", errors)
    if match is not None:
        require(
            match.group(1) in contract["valid_statuses"],
            "Decision status is not an allowed value",
            errors,
        )

    for marker in contract["disallowed_markers"]:
        require(marker not in text, f"Disallowed marker present in package: {marker}", errors)
...
def main():
    ...
    mode = "bundle shape" if args.bundle_shape_only else "completed roadmap decision package"
    print(f"S01 verifier PASS ({mode})")
    return 0

...
def main():
    parser = argparse.ArgumentParser(description="Check N38 cumulative changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N38 UI/visual patch budget: {extra}")

    if errors:
        print("N38 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N38 scope PASS")

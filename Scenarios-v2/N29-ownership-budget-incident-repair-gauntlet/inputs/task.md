...
effects and can report from durable ledger/audit state after transient notifications are lost. Then
update `candidate/repair-ledger.json` with the source decisions, review responses, validation, and
patch budget that explain the repair.
...
- `patchBudget.maxChangedPaths`: `4`
- `patchBudget.requiredChangedPaths`: exactly these four paths:

...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N29 changed paths stay in scope.")
...
def main():
    ...
    if budget.get("maxChangedPaths") != 4:
        print("ERROR: patchBudget.maxChangedPaths must be 4", file=sys.stderr)
        return 1
    if actual != required:
    ...
    if len(actual) > budget["maxChangedPaths"]:
        ...
        return 1
    print("N29 scope PASS (changed paths are in bounds)")
    return 0

...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N45 ownership-budget immutable report-consumer gauntlet.")
...
def check_shape(root: Path, contract: dict, errors: list[str]):
    ignored_top_level = {".pytest_cache", ".mypy_cache", "__pycache__"}
    actual_entries = sorted(path.name for path in root.iterdir() if path.name not in ignored_top_level)
    require(
...
def check_protected_hashes(root: Path, contract: dict, errors: list[str]):
    for rel_path, expected_hash in contract.get("protected_file_hashes", {}).items():
        ...
        if actual_hash.lower() != expected_hash.lower():
            errors.append(f"Protected file changed: {rel_path}")
...
def evaluate_repair_ledger(root: Path):
    failures = []
    ledger_path = root / "candidate" / "repair-ledger.json"
    try:
        ledger = load_json(ledger_path)
    except Exception as exc:  # noqa: BLE001
        return [{"id": "repair-ledger-schema", "detail": f"invalid JSON: {exc}"}]
    ...
    def repair_ledger_schema(ledger):
        required_keys = {"incidentId", "sourceDecisions", "reviewResponses", "validation", "patchBudget"}
        assert_equal(sorted(ledger.keys()), sorted(required_keys), "ledger top-level keys")
        assert_equal(ledger["incidentId"], "INC-742", "incident id")
        if not isinstance(ledger["sourceDecisions"], list) or not isinstance(ledger["reviewResponses"], list):
            raise AssertionError("sourceDecisions and reviewResponses must be arrays")
        if not isinstance(ledger["validation"].get("commands"), list):
            raise AssertionError("validation.commands must be an array")

    def source_decisions_ledger(ledger):
        rows = {row.get("sourceId"): row for row in ledger.get("sourceDecisions", [])}
        expected = {
            "INC-742": ("accepted", ["activeProfile", "authoritative", "legacyProfile", "fallback"]),
            "review-feedback": ("accepted", ["R1", "R2", "R3", "R4", "R5"]),
            "stale-ui-note": ("rejected", ["UI", "badge"]),
            "legacy-helper": ("rejected", ["legacyProfile", "fallback"]),
            "notification-count": ("rejected", ["notification", "ledger", "audit"]),
        }
        assert_equal(sorted(rows), sorted(expected), "source decision ids")
        for source_id, (decision, terms) in expected.items():
            row = rows[source_id]
            assert_equal(row.get("decision"), decision, f"{source_id} decision")
            haystack = json.dumps(row, sort_keys=True)
            require_terms(haystack, terms, f"{source_id} terms")

    def review_response_ledger(ledger):
        rows = {row.get("id"): row for row in ledger.get("reviewResponses", [])}
        assert_equal(sorted(rows), ["R1", "R2", "R3", "R4", "R5"], "review response ids")
        expected_terms = {
            "R1": ["api.py", "protected"],
            "R2": ["docs", "legacy", "UI"],
            "R3": ["source", "audit", "report"],
            "R4": ["retry", "resume", "executor.py"],
            "R5": ["validation", "test"],
        }
        for row_id, terms in expected_terms.items():
            row = rows[row_id]
            if row.get("decision") not in {"accepted", "rejected", "protected"}:
                raise AssertionError(f"{row_id} has invalid decision {row.get('decision')!r}")
            require_terms(json.dumps(row, sort_keys=True), terms, f"{row_id} terms")

    def validation_ledger(ledger):
        commands = ledger.get("validation", {}).get("commands", [])
        joined = "\n".join(commands)
        if not any(marker in joined for marker in ["check_ownership_budget_repair.py", "unittest discover", "test_deploygrid.py"]):
            raise AssertionError("validation.commands must name verifier or direct tests")

    def patch_budget_ledger(ledger):
        budget = ledger.get("patchBudget", {})
        assert_equal(budget.get("maxChangedPaths"), 3, "maxChangedPaths")
        assert_equal(sorted(budget.get("requiredChangedPaths", [])), sorted(REQUIRED_BUDGET_PATHS), "requiredChangedPaths")

    case("repair-ledger-schema", repair_ledger_schema)
    case("source-decisions-ledger", source_decisions_ledger)
    case("review-response-ledger", review_response_ledger)
    case("validation-ledger", validation_ledger)
    case("patch-budget-ledger", patch_budget_ledger)
    return failures
...
def evaluate_invariants(root: Path):
    ...
    def double_run_replay_consumer(api):
        state = api.DeployState()
        requests = [
            req("old", "canary", version="8.0", source="old-ticket", requested_at=1),
            req("new", "canary", version="8.0", source="new-ticket", requested_at=2),
        ]
        crash_key = "acme:api:8.0:canary:morning"
        try:
            api.run_deploy(
                state,
                config(activeProfile="default", legacyProfile=None),
                copy.deepcopy(requests),
                crash_after_key=crash_key,
            )
        except RuntimeError:
            pass
        else:
        ...
        api.run_deploy(state, config(activeProfile="default", legacyProfile=None), copy.deepcopy(requests))
        summary = api.summarize_state(state)
        assert_equal(released_keys(state), [crash_key], "double-run ledger replay")
        release_audit = [entry for entry in state.audit if entry.get("action") == "released"]
        assert_equal(len(release_audit), 1, "double-run released audit count")
        assert_equal(release_audit[0].get("key"), crash_key, "double-run released audit key")
        assert_equal(release_audit[0].get("source"), "new-ticket", "double-run released audit source")
        assert_equal([item["key"] for item in state.notifications], [], "lost notification must not be replayed")
        resume = summary.get("resume", {})
        assert_equal(resume.get("replayed_committed"), False, "resume replay classification")
        assert_equal(resume.get("released_from_ledger"), 1, "resume ledger count")

    def public_report_consumer(api):
        state = api.DeployState()
        run_config = config(
            activeProfile="default",
            legacyProfile=None,
        ...
        requests = [
            req("old", "canary", version="9.0", source="old-ticket", requested_at=1),
            req("new", "canary", version="9.0", source="new-ticket", requested_at=2),
            req("frozen", "prod", version="10.0", window="night", source="freeze-ticket"),
            req("prod-only", "prod", version="11.0", source="blocked-ticket"),
        ]
        api.run_deploy(state, run_config, copy.deepcopy(requests))
        state.notifications.clear()
        summary = api.summarize_state(state)
        assert_equal(summary["released"], 1, "consumer released count")
        assert_equal(summary["sources"], ["new-ticket"], "consumer release sources")
        assert_equal(summary.get("superseded_sources"), ["old-ticket"], "consumer superseded sources")
        require_terms(json.dumps(summary.get("deferred_causes", []), sort_keys=True), ["freeze-ticket", "frozen"], "consumer deferred causes")
        require_terms(json.dumps(summary.get("blocked_causes", []), sort_keys=True), ["blocked-ticket", "missing"], "consumer blocked causes")
    ...
    case("double-run-replay-consumer", double_run_replay_consumer)
    ...
    case("public-report-consumer", public_report_consumer)
    failures.extend(evaluate_repair_ledger(root))
    return failures
...
def main():
    ...
    contract = load_json(root / "oracle" / "ownership-budget-contract.json")
    check_shape(root, contract, errors)
    check_protected_hashes(root, contract, errors)
    ...
    if args.bundle_shape_only:
        ...
        mode = "completed run"
    print(f"N45 verifier PASS ({mode})")
    return 0

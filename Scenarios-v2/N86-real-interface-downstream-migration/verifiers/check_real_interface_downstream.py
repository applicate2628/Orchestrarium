...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N86 real interface downstream migration bundle.")
...
def import_public_package(root: Path):
    src_root = root / "candidate" / "workspace" / "src"
    sys.path.insert(0, str(src_root))
    for name in list(sys.modules):
        if name == "billingmesh" or name.startswith("billingmesh."):
            del sys.modules[name]
    return importlib.import_module("billingmesh")
...
def evaluate_downstream_public_app(root: Path, contract: dict):
    failures = []
    try:
        pkg = import_public_package(root)
    except Exception as exc:  # noqa: BLE001
        return [{"id": "downstream-public-app", "detail": f"public package import failed: {exc}"}]

    def public_exports():
        public_all = set(getattr(pkg, "__all__", []))
        missing = [name for name in contract["downstreamPublicExports"] if not hasattr(pkg, name)]
        if missing:
            raise AssertionError(f"missing root exports: {missing}")
        not_listed = [name for name in contract["downstreamPublicExports"] if name not in public_all]
        if not_listed:
            raise AssertionError(f"exports missing from __all__: {not_listed}")

    def public_result_models_are_dataclasses():
        for name in ["AccountLookup", "EntitlementDecision", "PublishResult"]:
            model = getattr(pkg, name)
            if not dataclasses.is_dataclass(model):
                raise AssertionError(f"{name} must be a dataclass root export")

    def downstream_public_flow():
        for cls_name, method in [
            ("AccountDirectory", "get_account"),
            ("EntitlementPolicy", "check"),
            ("UsagePublisher", "publish"),
        ]:
            if hasattr(getattr(pkg, cls_name), method):
                raise AssertionError(f"legacy method still visible to downstream app: {cls_name}.{method}")

        directory = pkg.AccountDirectory(sample_accounts(), now=contract["currentTick"])
        policy = pkg.EntitlementPolicy({"disabledTenants": ["blocked"]})
        allowed = policy.evaluate_entitlement(
            directory.lookup_account("acct-pro"),
            {"tenant": "acme", "feature": "metering.write"},
        )
        denied = policy.evaluate_entitlement(
            directory.lookup_account("acct-basic"),
            {"tenant": "acme", "feature": "metering.write"},
        )
        allowed_wire = dataclasses.asdict(allowed)
        assert_equal(allowed_wire["allowed"], True, "downstream.allowed")
        assert_equal(allowed_wire["owner"], "entitlement-policy", "downstream.allowed.owner")
        if "S" not in "".join(allowed_wire.get("source_ids", [])):
            raise AssertionError("downstream source ids not preserved")

        denied_transport = ScriptedTransport([True])
        denied_result = pkg.UsagePublisher(denied_transport).publish_usage(
            {"usage_id": "u-public-denied", "tenant": "acme"},
            denied,
        )
        denied_wire = dataclasses.asdict(denied_result)
        assert_equal(denied_wire["accepted"], False, "downstream.denied.accepted")
        assert_equal(denied_wire["status"], "rejected", "downstream.denied.status")
        assert_equal(denied_wire["reason"], "feature-not-entitled", "downstream.denied.reason")
        assert_equal(len(denied_transport.sent), 0, "downstream denied event must not publish")

        timeout_transport = ScriptedTransport(["timeout"])
        timeout_result = pkg.UsagePublisher(timeout_transport).publish_usage(
            {"usage_id": "u-public-timeout", "tenant": "acme"},
            allowed,
        )
        timeout_wire = dataclasses.asdict(timeout_result)
        assert_equal(timeout_wire["status"], "queued", "downstream.timeout.status")
        assert_equal(timeout_wire["retryable"], True, "downstream.timeout.retryable")

        duplicate_transport = ScriptedTransport([True, True])
        duplicate_publisher = pkg.UsagePublisher(duplicate_transport)
        first = duplicate_publisher.publish_usage({"usage_id": "u-public-dup", "tenant": "acme"}, allowed)
        second = duplicate_publisher.publish_usage({"usage_id": "u-public-dup", "tenant": "acme"}, allowed)
        assert_equal(dataclasses.asdict(first)["status"], "accepted", "downstream.dup.first")
        assert_equal(dataclasses.asdict(second)["status"], "duplicate", "downstream.dup.second")
        assert_equal(len(duplicate_transport.sent), 1, "downstream duplicate must not republish")

        summary = pkg.build_usage_summary([first, timeout_result, denied_result, second])
        assert_equal(summary.get("accepted"), 1, "downstream.summary.accepted")
        assert_equal(summary.get("queued"), 1, "downstream.summary.queued")
        assert_equal(summary.get("duplicate"), 1, "downstream.summary.duplicate")
        reasons = summary.get("reasons", {})
        for reason in ["usage-publish-timeout", "feature-not-entitled", "duplicate-usage"]:
            if reasons.get(reason) != 1:
                raise AssertionError(f"downstream summary reason {reason!r} missing")

    for case_id, fn in [
        ("downstream-public-exports", public_exports),
        ("downstream-public-models", public_result_models_are_dataclasses),
        ("downstream-public-flow", downstream_public_flow),
    ]:
        case_runner(failures, case_id, fn)

    if failures:
        return [{"id": "downstream-public-app", "detail": "; ".join(f"{f['id']}: {f['detail']}" for f in failures)}]
    return []
...
def evaluate_bundle(root: Path, contract: dict):
    ...
    failures.extend(evaluate_downstream_public_app(root, contract))
...
def main():
    ...
    contract = load_json(root / "oracle" / "real-interface-downstream-contract.json")
    ...
    if args.bundle_shape_only:
        print("N86 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N86 verifier PASS (expected start-state failures present)")
        return 0

...
def load_contract(root):
    return json.loads((root / "oracle" / "real-interface-downstream-contract.json").read_text(encoding="utf-8"))


def main():
    parser = argparse.ArgumentParser(description="Check N86 real interface changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N86 patch budget: {extra}")

    if errors:
        print("N86 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N86 scope PASS")

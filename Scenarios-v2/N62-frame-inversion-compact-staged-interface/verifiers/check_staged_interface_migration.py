...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N62 frame-inversion staged interface bundle.")
...
def evaluate_runtime(root: Path, contract: dict):
    ...
    def session_lookup_contract():
        ...
        cases = [
            ("active-admin", None, True, "active", "session-store"),
            ("active-admin", 80, False, "expired", "session-store"),
            ("missing", None, False, "missing", "session-store"),
            ("expired", None, False, "expired", "session-store"),
            ("revoked", None, False, "revoked", "session-store"),
        ]
        for session_id, at_tick, found, reason, owner in cases:
            lookup = store.lookup(session_id, at_tick=at_tick) if at_tick is not None else store.lookup(session_id)
    ...
    def policy_contract():
        ...
        cases = [
            ...
            ("active-user", {"tenant": "acme", "action": "read"}, True, "allowed", "policy-evaluator"),
            ...
            ("revoked", {"tenant": "acme", "action": "read"}, False, "revoked", "session-store"),
        ]
    ...
    def router_contract():
        policy = policy_mod.PolicyEvaluator({"blockedTenants": ["blocked"]})
        store = store_mod.SessionStore(sample_records())
        allowed = policy.evaluate_policy(store.lookup("active-admin"), {"tenant": "acme", "action": "read"})
        denied = policy.evaluate_policy(store.lookup("active-admin"), {"tenant": "blocked", "action": "read"})

        denied_transport = ScriptedTransport([True])
        denied_router = router_mod.EventRouter(denied_transport)
        denied_result = denied_router.dispatch_event({"event_id": "evt-denied", "tenant": "blocked"}, denied)
        ...
        assert_equal(get_field(denied_result, "reason"), "blocked-tenant", "denied.reason")
        assert_equal(len(denied_transport.sent), 0, "denied decision must not dispatch")
    ...
    def integration_contract():
        ...
        denied_transport = ScriptedTransport([True])
        blocked = api.handle_event(records, rules, denied_transport, "active-admin", {
        ...
        assert_equal(len(denied_transport.sent), 0, "blocked event must not dispatch")

        user_read = api.handle_event(records, rules, ScriptedTransport([True]), "active-user", {
            "event_id": "evt-read",
            "tenant": "acme",
            "action": "read",
        })
        assert_equal(user_read["accepted"], True, "user read accepted")

        user_delete = api.handle_event(records, rules, ScriptedTransport([True]), "active-user", {
            "event_id": "evt-delete",
            "tenant": "acme",
            "action": "delete",
        })
        assert_equal(user_delete["reason"], "requires-admin", "user delete reason")
        assert_equal(user_delete["owner"], "policy-evaluator", "user delete owner")
    ...
    def report_contract():
        report = modules["report"].build_audit_summary([
            ...
            {"status": "duplicate", "accepted": False, "retryable": False, "owner": "event-router", "reason": "duplicate-event"},
        ])
        ...
        for reason in ["transport-timeout", "blocked-tenant", "missing", "duplicate-event"]:
...
def evaluate_closure(root: Path, contract: dict):
    ...
    if "python candidate/workspace/tests/test_interfaceflow.py" not in validation_text:
...
def main():
    ...
    contract = load_json(root / "oracle" / "staged-interface-contract.json")
    ...
    if args.bundle_shape_only:
        print("N62 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N62 verifier PASS (expected start-state failures present)")
        return 0

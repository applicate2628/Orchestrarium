...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the N06 authz trust-boundary review bundle shape or a completed review report."
    )
...
def load_contract(bundle_root: Path):
    return json.loads(
        (bundle_root / "oracle" / "authz-trust-review-contract.json").read_text(encoding="utf-8")
    )
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(
        ...
        "scenario.yaml metadata does not match N06",
        errors,
    )
...
def main():
    ...
    print(f"N06 verifier PASS ({mode})")
    return 0

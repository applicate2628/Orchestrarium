...
CONTRACT_ID = "N75-W53-persisted-state-replay-migration"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify N75 persisted-state replay migration.")
...
def main() -> int:
    ...
    if args.bundle_shape_only:
        if failures:
            for _, detail in failures:
                print(detail)
            return 1
        print("N75 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if expected & observed:
            print("N75 verifier PASS (expected start-state failures present)")
            return 0
        print(f"N75 expected start-state failure missing; observed {sorted(observed)}")
        return 1

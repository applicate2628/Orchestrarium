...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N44 interface-refactor/source-id gauntlet.")
...
def evaluate_runtime(root: Path, contract: dict):
    ...
    def integration_contract():
        ...
        assert_source_ids(ok, "ok.sourceIds", ["active-admin", "evt-ok"])
        ...
        assert_source_ids(blocked, "blocked.sourceIds", ["active-admin", "evt-blocked"])
        ...
        assert_source_ids(expired, "expired.sourceIds", ["expired", "evt-expired"])
        ...
        assert_source_ids(timeout, "timeout.sourceIds", ["active-admin", "evt-timeout"])
    ...
    def report_contract():
        report = modules["report"].build_audit_summary([
            ...
            },
            {
                ...
                "sourceIds": ["active-admin", "evt-timeout"],
            },
            {
                "status": "rejected",
                "accepted": False,
                ...
                "sourceIds": ["active-admin", "evt-blocked"],
            },
            {
                "status": "rejected",
                "accepted": False,
        ...
        assert_equal(report.get("sourceIdBearingResults"), 4, "report.sourceIdBearingResults")
        source_ids_by_reason = report.get("sourceIdsByReason", {})
        for reason in ["accepted", "transport-timeout", "blocked-tenant", "missing"]:
            values = source_ids_by_reason.get(reason)
            if not isinstance(values, list) or not values:
                raise AssertionError(f"report sourceIdsByReason missing {reason!r}: {source_ids_by_reason!r}")

    def assert_source_ids(result, label: str, expected_subset: list[str]):
        source_ids = result.get("sourceIds")
        if not isinstance(source_ids, list) or not source_ids:
            raise AssertionError(f"{label} must be a non-empty list")
        for expected in expected_subset:
            if expected not in source_ids:
                raise AssertionError(f"{label} missing {expected!r}: {source_ids!r}")
...
def evaluate_protected_files(root: Path, contract: dict):
    failures = []
    for rel_path, expected_hash in contract.get("protected_file_hashes", {}).items():
        path = root / rel_path
        if not path.exists():
            failures.append({"id": "protected-file-missing", "detail": rel_path})
            continue
        actual_hash = sha256(path)
        if actual_hash != expected_hash:
            failures.append({
                "id": "protected-file-modified",
                "detail": f"{rel_path} hash {actual_hash} != {expected_hash}",
            })
    return failures
...
def evaluate_bundle(root: Path, contract: dict):
    ...
    failures.extend(evaluate_protected_files(root, contract))
    return failures
...
def main():
    ...
    if args.bundle_shape_only:
        print("N44 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N44 verifier PASS (expected start-state failures present)")
        return 0

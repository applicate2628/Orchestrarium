...
# MUTANT class-id=replay: consumed nonces are no longer recorded or rejected, so a
# captured token can be replayed. All other checks are the correct reference logic.
# Detect by authorizing the same token twice with a shared used_nonces set and
# asserting the second call is denied -> 'replay undetected'.

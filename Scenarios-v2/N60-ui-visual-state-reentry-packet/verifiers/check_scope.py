...
def main():
    parser = argparse.ArgumentParser(description="Check N60 exact changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N60 UI/visual patch budget: {extra}")

    if errors:
        print("N60 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N60 scope PASS")

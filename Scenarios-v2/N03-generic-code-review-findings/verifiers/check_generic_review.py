...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the N03 generic-review bundle shape or a completed review report."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the N03 bundle root.",
    )
    parser.add_argument(
        "--bundle-shape-only",
        action="store_true",
...
def load_contract(bundle_root: Path):
    contract_path = bundle_root / "oracle" / "generic-review-contract.json"
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(metadata == contract["expected_metadata"], "scenario.yaml metadata does not match N03", errors)
...
def main():
    ...
    print(f"N03 verifier PASS ({mode})")
    return 0

...
def main():
    parser = argparse.ArgumentParser(description="Check N35 cumulative changed-path budget.")
    ...
    if extra:
        errors.append(f"Changed paths outside exact N35 patch budget: {extra}")

    if errors:
        print("N35 scope FAIL")
        for error in errors:
            print(f"- {error}")
        raise SystemExit(1)

    print("N35 scope PASS")

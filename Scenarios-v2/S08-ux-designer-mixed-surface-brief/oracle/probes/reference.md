...
- Review happens on the web surface; publish stays unavailable until the review-specific blockers
  are resolved, so a green local check cannot expose the publish step during an open clarification.
...
- Make the return reason and repair target explicit, keep the blocker visible, and keep publish
  gated until review-specific blockers are resolved.

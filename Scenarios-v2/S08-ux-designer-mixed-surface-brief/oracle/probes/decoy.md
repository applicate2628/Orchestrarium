...
- Review happens on the web surface; publish is available as soon as local checks are green, so the publish step shows during review.
...
- Make the return reason and repair target explicit, keep the blocker visible, and keep the publish step available whenever local checks pass.

...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N46 low-noise operator budget.")
...
def main():
    ...
    if args.bundle_shape_only:
        print("N46 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N46 operator-budget PASS ({size} <= {max_bytes} bytes)")
    return 0

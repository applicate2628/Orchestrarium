...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N46 changed paths stay in scope.")
...
def main():
    ...
    if required != expected_required:
        print("ERROR: patchBudget.requiredChangedPaths must match the N46 required production/ledger paths", file=sys.stderr)
    ...
    if len(actual) > budget["maxChangedPaths"]:
        ...
        return 1
    print("N46 scope PASS (changed paths are in bounds)")
    return 0

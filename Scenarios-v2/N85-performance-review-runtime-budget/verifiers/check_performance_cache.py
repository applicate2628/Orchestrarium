...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N85 real-repo performance cache bundle.")
...
def main():
    ...
    if errors:
        print("N85 performance-cache FAIL (bundle shape)")
        for error in errors:
            print(f"- {error}")
        return 1
    if args.bundle_shape_only:
        print("N85 performance-cache PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        if failures:
            print("N85 performance-cache PASS (start state)")
            print("Expected start-state failures:", ", ".join(metrics["failure_ids"][:8]))
            return 0
        print("N85 performance-cache FAIL (start state unexpectedly passes)")
        return 1

    if failures:
        print("N85 performance-cache FAIL")
        for failure in failures:
            print(f"- {failure['id']}: {failure['detail']}")
        return 1

    print(f"N85 performance-cache PASS ({metrics.get('runtime_seconds')} <= {metrics.get('max_seconds')} seconds)")
    return 0

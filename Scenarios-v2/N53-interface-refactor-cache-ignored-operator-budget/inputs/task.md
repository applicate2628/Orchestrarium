# Task: Structured Interface Refactor With Compact Operator Output
...
Keep the visible worker output in `../meta/worker-output.txt` at or below `40000` bytes. This is a
scoreable operator requirement for compact interface-refactor work. Do not paste full source files,
broad directory listings, repeated test output, or raw transcripts into the final response.

Generated `.pytest_cache/**` files are ignored as auxiliary test-run cache by this scenario. Do not
create or modify any other files outside `scenario.yaml` allowed surfaces.

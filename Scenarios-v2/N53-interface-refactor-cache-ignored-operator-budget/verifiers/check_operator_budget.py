...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N53 visible operator-output budget.")
...
def main():
    ...
    contract_path = root / "oracle" / "interface-refactor-contract.json"
    ...
    if args.bundle_shape_only:
        print("N53 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N53 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

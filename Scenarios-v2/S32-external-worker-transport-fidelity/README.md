...
selected `gemini` CLI is unavailable and that no approved Gemini wrapper exists for this lane. The
correct result is an explicit disabled-route report, not an internal fallback or a reroute to a
different provider.
...
- provenance completeness and clean disabled-route reporting

...
You are acting as `$frontend-engineer` on a staged UI reentry bundle.

## Goal

Repair the incident review console in `candidate/workspace/` across the four phase files in
`inputs/phases/`. Keep command focus, dirty record state, guarded navigation, validation/focus
return, accessible markup, responsive layout, deterministic raster preview, and closeout evidence
...
`01-source-state-ledger`, `02-state-render-implementation`, `03-layout-raster-validation`, and
`04-reentry-closeout`.

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the N64 security-depth review bundle shape or a completed review report."
    )
...
def load_contract(bundle_root: Path):
    return json.loads(
        (bundle_root / "oracle" / "security-depth-review-contract.json").read_text(encoding="utf-8")
    )
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(
        ...
        "scenario.yaml metadata does not match N64",
        errors,
    )
...
def main():
    ...
    print(f"N64 verifier PASS ({mode})")
    return 0

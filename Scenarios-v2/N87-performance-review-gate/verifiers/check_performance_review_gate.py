...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N87 performance review gate bundle.")
...
def evaluate_response_gate(root: Path, contract: dict):
    ...
    if str(response.get("gateDecision", "")).upper() != contract["requiredGateDecision"]:
        failures.append({"id": "response-gate-decision", "detail": "gateDecision must be REVISE"})
...
def evaluate_closure(root: Path, contract: dict):
    ...
    if contract["requiredGateDecision"] not in text:
        failures.append({"id": "closure-complete", "detail": "required gate decision missing"})
...
def main():
    ...
    contract = load_json(root / "oracle" / "performance-review-gate-contract.json")
    ...
    if args.bundle_shape_only:
        print("N87 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N87 verifier PASS (expected start-state failures present)")
        return 0

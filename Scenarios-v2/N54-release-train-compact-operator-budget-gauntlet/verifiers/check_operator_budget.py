...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N54 visible operator-output budget.")
...
def main():
    ...
    contract_path = root / "oracle" / "deploygrid-contract.json"
    ...
    if args.bundle_shape_only:
        print("N54 operator-budget PASS (bundle shape)")
        return 0
    ...
    print(f"N54 operator-budget PASS ({actual} <= {max_bytes} bytes)")
    return 0

...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N54 release train compact operator-budget gauntlet.")
...
def main():
    ...
    contract = load_json(root / "oracle" / "deploygrid-contract.json")
    ...
    if not args.bundle_shape_only:
        ...
        if args.expect_start_state:
            ...
            if failures:
                ...
                errors.append(f"Completed candidate still fails release train invariants: {rendered}")
    ...
    if args.bundle_shape_only:
        ...
        mode = "completed run"
    print(f"N54 verifier PASS ({mode})")
    return 0

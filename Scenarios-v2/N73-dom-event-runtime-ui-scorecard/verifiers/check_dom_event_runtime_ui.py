...
CONTRACT_ID = "N73-W51-dom-event-runtime-ui"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Verify N73 DOM event runtime UI repair.")
...
def main() -> int:
    ...
    if args.bundle_shape_only:
        if failures:
            for _, detail in failures:
                print(detail)
            return 1
        print("N73 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if expected & observed:
            print("N73 verifier PASS (expected start-state failures present)")
            return 0
        print(f"N73 expected start-state failure missing; observed {sorted(observed)}")
        return 1

...
# MUTANT class-id=tenant-binding: the token's tenant_id is no longer bound to
# expected_tenant. Signature, alg, kid, expiry, user, resource, and replay checks are
# the correct reference logic. Detect by authorizing a validly-signed token for one
# tenant against a DIFFERENT expected_tenant and asserting denial -> 'tenant-binding undetected'.

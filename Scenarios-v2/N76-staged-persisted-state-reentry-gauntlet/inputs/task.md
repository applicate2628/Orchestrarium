...
Complete the StateDock persisted-state replay migration across the staged re-entry flow.
...
- maintain `source-ledger.json` with accepted/stale source decisions from the staged inputs
...
- maintain `reentry-state.json` with the validation status for every phase and the replay/rollback
  repair state after implementation
- maintain `closeout.json` with exact changed paths, commands run, final verifier status, and the
  remaining routing decision

The visible test is intentionally insufficient. Hidden verification exercises migration,
idempotency, rollback, persist/load, immutability, exact scope, source arbitration, re-entry, and
closeout.

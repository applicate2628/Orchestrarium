...
- treat `$consultant` as an independent advisory team member only: ordinary consultant use stays optional, it never becomes a blocking reviewer or approver, any repo-local required consultant-check remains advisory-only rather than a substitute for review or human gates, and active lane policy may ask for one or more external consultant-checks before closure when multiple independent opinions are needed
...
- before checking provider preferences, opinion counts, thread limits, CLI availability, or retry paths for an external request, classify the requested work as one of four buckets: advisory consultant, worker-side adapter, review-or-QA-side adapter, or unsupported-owner-route
...
- external adapters may run in parallel with one another when their scopes are independent, their artifacts do not overlap, the active priority profile asks for multiple independent opinions, and the selected provider runtimes support concurrent non-interactive execution
- same-provider fan-out is allowed for disjoint admitted artifacts and slices; there is no one-instance-per-provider ceiling when the launch items are genuinely independent
- if a bounded set of external helper runs should be launched and aggregated together, prefer the repo-local external-brigade utility over scattering ad hoc helper calls across notes or chat

...
**Windows host limitation.** Codex CLI's hook execution path on Windows is not documented in the official [Codex hooks reference](https://developers.openai.com/codex/hooks): the schema supports neither the `args` exec form nor a `shell` field for selecting bash vs PowerShell, and the default Windows shell semantics are unspecified. As a result the installer **skips the Codex hook auto-install on Windows** (the hook script is still placed under the installed skills directory — `~/.codex/skills/lead/scripts/` for `--global` or `<project>/.agents/skills/lead/scripts/` for `--target`, but the corresponding `hooks.json` — `~/.codex/hooks.json` for `--global` or `<project>/.codex/hooks.json` for `--target` — is not modified). Operators on Windows who know their Codex runtime's shell behavior can manually add a hook entry to the appropriate `hooks.json`. The Claude pack's auto-install is unaffected and uses native PowerShell exec form on Windows.

...
- Epics: when several work-items serve one initiative, group them as an epic — a flat file `work-items/epics/<date>-<slug>.md` (`status: active|closed`, `## Goal` / `## Children` slug list), the same flat shape as `work-items/bugs/`. Each child work-item declares a single bare `Epic: <slug>` line in its `status.md`. Epic progress is derived live from the children (a child is done by the same predicate `check-work-items-archival-stop.py` uses, resolving each slug across `active/` + `archive/`). Close the epic (`status: closed` + `## Closure`) only when all children are closed AND the goal is met — governance-enforced (the archival hook does not scan `work-items/epics/`). `$product-manager` admits the epic; `$lead` links, rolls up, and closes it (full rules in the lead skill).

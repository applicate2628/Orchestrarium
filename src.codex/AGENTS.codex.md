...
**Windows hook command shape.** On Windows, the hook entry uses `bash <script.sh>` shell form (no `args` field — Codex hooks don't support `args`). This assumes the Codex hook interpreter on Windows can locate `bash` on PATH, which is the typical setup when Git Bash is installed alongside Codex. If your Codex runtime uses a different shell, the trust UI will still show the entry and you can edit `~/.codex/hooks.json` directly to match your shell (the marker substring `check-hypothesis-disclosure` is what the installer's idempotent merge keys on, so manually-edited commands containing that substring are still recognized as the pack's entry on next reinstall).

...
**PreToolUse git-push publication-gate hook.** `check-git-push-gate.py` is the structural backstop for the publication-safety rule "human review before `git push` must include a leak-check of staged changes" — previously prose-only while lower-stakes edit/stop moments had blocking hooks. Registered on the `Bash` matcher, it detects `git push` in command position with the same shell-aware parser technique as the stray-artifact audit (quoted-string `git push` is data, not a command; `git push --dry-run` is always allowed), and: skips subagent contexts (envelope `agent_id` → allow; governance still forbids delegating a push to dodge review); allows when the LAST GENUINE USER MESSAGE contains the per-turn override `[approve-publication]` (honored ONLY from the user's own message — never from assistant prose, tool calls, or tool output, because quoted or injected content must not approve a publication); allows when the last genuine user message contains an explicit push instruction (`push`, `запушь`, `залей`, ...) AND the current turn's model tool CALLS show a publication-safety scan invocation (`check-publication-safety` / `check-publication-gate` / `agents-check-safety`). It also supports one transcript-scoped grant for a concrete GitHub pull request (PR): the user's exact whole message `[approve-pr-publication:v1 pr=https://github.com/<owner>/<repo>/pull/<positive-number>]`, revoked by exact `[revoke-pr-publication:v1]`. That route freshly revalidates the open PR, repository/ref/object binding, remote, and destination protection on every attempt; accepts only a 1–255-character ASCII head drawn from `[A-Za-z0-9._/-]` that passes direct-argument `git check-ref-format --branch`; accepts only the owning POSIX/PowerShell canonical literal serialization of the absolute resolved Git executable plus `push <remote> HEAD:refs/heads/<current-head-ref>`; and requires a fresh unused non-empty range receipt bound to that decoded remote, destination, and current `HEAD` tip. Any active-route ambiguity, drift, provider failure, unsafe/out-of-language destination, non-canonical command, stale/reused receipt, or unavailable transcript denies without generic fallback. The grant replaces repeated confirmation only—human review and one fresh leak-check per push remain mandatory. Otherwise the hook emits a structured `permissionDecision: "deny"` payload with exact compliance instructions. It is a BACKSTOP, not a guarantee: generic detection still does not model a push hidden behind a wrapper script, `eval`, or command substitution, while the active PR route explicitly rejects such shapes; the binding governance remains mandatory.

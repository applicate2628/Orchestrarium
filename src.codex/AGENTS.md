...
- keep the system operating as a rolling loop: `PASS` advances immediately, `REVISE` stays in the same role for a bounded correction, after 3 consecutive `REVISE` cycles for the same role and artifact the lead must re-route, escalate, or block instead of looping, and `BLOCKED` is reserved for real external blockers

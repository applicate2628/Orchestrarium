...
- Honor `externalCodexProfile` when the selected provider is Codex: `default` inherits `externalModelMode`; `gpt-5.5-fast` explicitly requests the installed Codex runtime's fast profile and must record unavailable or deviated if that profile cannot be verified; `gpt-5.5-xhigh` (shipped as default) explicitly requests model `gpt-5.5` with `model_reasoning_effort = "xhigh"` regardless of `externalModelMode`, symmetric to Claude's `opus-max`.

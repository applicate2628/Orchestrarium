...
- `externalOpinionCounts` is a same-lane distinct-opinion policy, not a limit on how many same-provider external worker items may run in parallel when the slices are disjoint.
...
- If several independent external helper items need to run together, the orchestrator should route them through `$external-brigade` rather than trying to compress them into one worker artifact.

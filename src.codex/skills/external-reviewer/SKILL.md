...
- Honor `externalCodexProfile` when the selected provider is Codex: `default` inherits `externalModelMode`, while `gpt-5.5-fast` explicitly requests the installed Codex runtime's fast profile and must record unavailable or deviated if that profile cannot be verified.

...
Git metadata is filtered to included paths with rename/copy detection disabled, so an included rename cannot disclose an external old/new path. Operational Git calls disable executable helpers discovered across effective included local/worktree configuration scopes; their standard output and standard error are streamed into independent 8 MiB bounded captures, and timeout, launch, capture, or cap failures are contract errors. ZIP entries use fixed timestamps, permissions, ordering, and compression settings. Before opening a ZIP, the verifier reads only the bounded end-of-central-directory (EOCD) window and any immediately preceding ZIP64 locator and fixed record; it rejects multi-disk archives, malformed records, more than 10,000 entries, a central directory larger than 8 MiB, and central-directory offsets or sizes outside the archive. Producer and verifier both enforce at most 10,000 entries, 128 MiB per entry, 512 MiB declared uncompressed total, and a 100:1 maximum declared compression ratio. For each selected payload, the producer opens one no-follow, nonblocking ordinary leaf, streams and hashes only that held descriptor into ZIP, then verifies size, digest, held identity, parent identities, and final pathname binding before closing it once. It never reopens the pathname for content; FIFOs, sockets, links, directories, devices, alternate data streams, and reparse points fail promptly. The verifier streams ZIP members and source hashes without retaining payload contents. Inventory JSON alone is bounded to 256 MiB and is canonically streamed on write; bounded inventory parsing uses the same cap. Selection, receipts, internal manifests, and every other JSON document remain bounded to 8 MiB. File reads request at most the applicable limit plus one byte before parsing, producer output and internal manifests are refused above their applicable limit, and JSON rejects duplicate keys, named non-finite constants, numeric overflow, and nesting deeper than 64. Trusted verification opens the bundle once and uses that same archive handle for internal-manifest validation, payload hashing, and metadata comparison; malformed archives and unsupported compression are contract errors. Trusted verification also binds the manifest history state and conditional head to the inventory. Source-side trusted verification requires `--inventory`, `--selection`, and `--source`; it derives the exact archive set and rejects duplicates, file/descendant and portable path collisions, path escapes, unsafe member types/names, encryption, missing or extra entries, metadata drift, size drift, and hash drift. Standalone verification checks archive structure and internal hashes only. After receiver extraction, `verify --bundle <zip> --source <repo>` checks payload bytes and declared tracked-deletion absence against the restored tree but does not prove original completeness or Git index equivalence. For an unborn inventory, the metadata members preserve status/staged/unstaged evidence; verification does not recreate index equivalence or fabricate a commit. The tool writes only outside the repository so its own output cannot invalidate the audited snapshot.

Every inventory or selection pathname is bound as one ordinary no-follow file before reading and remains held through bounded parsing and final identity revalidation. One `_BoundOrdinaryFileCore` owns path validation, acquisition, identity, parent and name rebinding, stability, and close behavior for both JSON inputs and ZIP archives; the wrappers supply only their operation-specific options and error identifiers. POSIX JSON inputs enable handle-relative `O_NONBLOCK | O_NOFOLLOW` before `fstat`, so a FIFO with no writer is rejected rather than opened blocking, while ZIP keeps its prior blocking flag set; sockets, devices, links, directories, and unknown special types fail the same binding gate. Windows rejects reparse points, directories, devices, alternate data streams, and namespace aliases before reading. A parent, leaf, size, change-stamp, or pathname-identity drift fails with the wrapper's stable transfer-input or transfer-archive error.

The transfer format has no preamble or comment: its first declared local header begins at byte zero and its EOCD ends at the physical end of the file. Standard or ZIP64 end records exactly adjoin the central directory, and an independent raw-header walk requires local headers, compressed payload extents, optional data descriptors, and central records to cover the complete file without gaps or overlaps before manifest bytes are trusted. Prefixes (including self-extracting markers), shifted offsets, archive comments, trailing bytes, and inter-member or pre-central-directory gaps fail as `TRANSFER-ARCHIVE-BOUNDARY-INVALID`. One `BoundArchiveSession` holds an ordinary no-follow archive object from raw preflight through ZIP, manifest, resource, and payload reads; it binds the initial identity, physical end, and change stamp, rechecks the same handle plus its parent-relative name before success, and closes member streams, ZIP state, the archive stream, and parent handles on every exit. Before any Windows open, one raw-and-canonical prefix-shaped owner rejects alternate data streams, extended/device roots, drive-relative forms, malformed UNC paths, true DOS-device basenames including the official superscript-digit aliases, and trailing-dot/space aliases; only ordinary absolute drive paths, canonicalized ordinary relative paths, and ordinary UNC paths reach `CreateFileW`. Ordinary components merely named `Device`, `pipe`, `GLOBALROOT`, or `PhysicalDrive0` remain valid. Construction has one `_OrdinaryFileAcquisitionOwner` at a time (`HANDLE` to C-runtime descriptor to Python stream to the shared core on Windows, descriptor to stream to the shared core on POSIX); every transfer clears the predecessor immediately, rollback changes its state to `none` before closing exactly once, and no outer layer retries that close. Binding, later identity drift, and settlement failure use the archive wrapper's `TRANSFER-ARCHIVE-BINDING-INVALID`, `TRANSFER-ARCHIVE-IDENTITY-DRIFT`, and `TRANSFER-ARCHIVE-CLOSE-FAILED`, with cleanup failure taking precedence because ownership may remain live. Structural reads validate nonnegative offsets, physical-end bounds, a named cap, and the allowed fixed descriptor lengths before issuing any read.
...
| Regenerable index/cache/log | Exclude only after proving no unique evidence and no live writer. `.scratch/` is disposable workspace, not the sole durable store for important results: preserve unique valuable results in a canonical artifact or verified private external archive before excluding regenerable payload. Names alone do not prove unknown content disposable. |

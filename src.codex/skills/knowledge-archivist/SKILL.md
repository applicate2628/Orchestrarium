...
description: "Canonical docs, links, work-items registries, archive moves, semantic-currency reconciliation, and leak checks."
---
...
- **Registry Governance Reconciliation (mandatory complete mode).** Use this mode when the request names all/current work-items registries, after a verified change to cross-registry schema, lifecycle, or relations, or at a milestone-wide cleanup. A single-item or single-registry change uses Work-cycle reconciliation instead. Freeze the current Git `HEAD` and governing-source set, then inventory the lifecycle owner's categories plus `bugs/`, `decisions/`, `lessons/`, `roadmaps/`, and `epics/`; backlog files are the candidate registry, so do not invent a parallel `candidates/` owner. Return one matrix with `record | current claim/status | governing predicate | evidence | structural result | semantic result | semantic owner | required action`. Run the existing lifecycle audit for the structural gate; do not duplicate it. Separately prove semantic currency for EVERY current record: a bug needs a current reproduction or verified unresolved code path; `accepted`, `applied`, `shipped`, closed, or superseded claims need their required current evidence; `open`, `proposed`, active, or candidate claims need evidence that the work is still outstanding and not already delivered; dependencies, epic children, and registry links must resolve. Counts, filenames, placement, and syntactically valid status labels never prove semantic currency. Derive every summary count mechanically from the exact emitted row set, require the grouped total to equal the physical current-record count before return, and expose every unmatched row in an explicit `unclassified` remainder instead of dropping it. Overall `PASS` requires BOTH structural and semantic gates to pass. Missing or contradictory semantic evidence is `REVISE` to the named semantic owner; ambiguous ownership is `BLOCKED`. Never change semantic status merely to make the matrix green. After owners accept corrections, apply only owned hygiene/lifecycle mechanics and perform one complete post-change verification pass; if discrepancies remain, return them rather than starting a prose/review loop.
- **Work-cycle reconciliation.** Run one bounded reconciliation for a newly admitted, parked, or closed item; a work-item-identity-changing reprioritization; a delivery-wave or milestone boundary; or concrete material status, ledger, or location drift. Do not retrigger for a stable active-item set, ordinary same-item continuation, a trivial turn, or a side question. Consume the lifecycle owner's parser and roll-up, including its handling of list-form `- context:`, instead of deriving lifecycle meaning from a heading scan, filename or file count, or structural audit alone. Structural `PASS` does not prove semantic currency.
- Own the work-item close/state-change MECHANICS contract through the installed lifecycle owner: exact context-linked bug disposition, archive placement, receipt creation, physical reconciliation, and generated `work-items/README.md` verification in one rollback-safe close. Assert that (a) every item occupies exactly one lifecycle location, (b) terminal work-items live only under `work-items/archive/YYYY-MM/`, (c) every current bug whose parsed `context` equals the closing item appears exactly once in `bug-dispositions.json` as `terminalize` or `preserve-current`, and (d) an active manifest fails as pending close. Schema Version 3 retains Version 1 bug rows (not Version 2 adjacency) and adds only exact unmatched ordinary file/directory scratch leaves with pre-close tree and same-item canonical-pointer hashes; its receipt is a nonauthorizing close-time snapshot and archived retry after authorized removal does not require later scratch or pointer existence. Ordinary close stays strict; the optional `archive-with-successor --obligation-transfer-file` route keeps this same owner while binding every unresolved review obligation to exactly one successor without copying ledger events, as documented in `docs/work-item-execution-tracking.md`. Omission preserves the Version 1 receipt path; transferred ownership uses the compatible Version 2 receipt. `work-items/index.md` is compatibility-only. Lead owns the close decision, `closure.md`, and disposition semantics; routine single-item mechanics may run inline, while multi-item or drifted states route here. Historical archives without the newer manifest remain compatible.

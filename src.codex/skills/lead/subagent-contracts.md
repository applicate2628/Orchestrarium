...
- Treat fallback pools asymmetrically: `gpt-5.6-luna` is a bounded mechanical overflow path only, while `reserve` is a symbolic supplemental advisory/review candidate that may appear only after primary `claude`/`codex` in eligible profile orders.

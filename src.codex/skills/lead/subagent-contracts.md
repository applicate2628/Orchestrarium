...
- `externalOpinionCounts` is a same-lane distinct-opinion policy. It does not cap how many same-provider external items may be launched in parallel when the items are disjoint and the provider runtime can handle them.
- When a bounded set of external helper items should be launched together, route the batch through the repo-local external-brigade utility instead of expanding one-off helper notes in place.

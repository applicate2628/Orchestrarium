...
- Kimi is an explicit read-only route for policy-admitted broad research and review. Its transport is fixed to model `kimi-code/k3`, text output, and the runner-bound neutral capture `prompt.md`; it has no native effort selector, remains independently verified and nonauthorizing, and never enters shipped `auto`. Grok remains an unavailable policy name in 1.x: do not launch or probe it.

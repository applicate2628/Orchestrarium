...
externalProvider: auto  # allowed here: auto | codex | claude | kimi | grok; default: auto; kimi is explicit sealed-read-only execution/nonauthorizing and may return an engineering artifact; grok is unavailable in 1.x
...
- Explicit user or Lead override may choose Kimi only for a policy-admitted read-only task or artifact-only engineering task; execution remains sealed read-only, independently verified, and nonauthorizing. Grok remains unavailable and must not be launched or probed in 1.x.
...
- Kimi may be selected only for policy-admitted read-only exploration/research/planning/review or artifact-only engineering through the canonical fixed `kimi-code/k3` no-tools/no-subagents wrapper. Execution stays sealed read-only; record returned engineering output as `returned patch/full-file set; repository unchanged`, nonauthorizing, and independently verified. Kimi never enters `auto`. Grok remains unavailable and must never be selected, executed, or recorded as a realized provider in 1.x.

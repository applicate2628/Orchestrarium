...
externalCodexProfile: default  # allowed: default | gpt-5.5-fast; default: default
...
- `externalCodexProfile: default` means Codex inherits `externalModelMode`. `externalCodexProfile: gpt-5.5-fast` explicitly requests the installed Codex runtime's fast profile and must record unavailable or deviated if that profile cannot be verified.

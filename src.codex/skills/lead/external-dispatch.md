...
- The wrapper projects the selected tools as an upper bound in a private regular `config.toml` without modifying the user source. Empty selection uses native allow-minus-deny composition; `Agent`, `AgentSwarm`, or nonempty `subagents` fails before launch with `E_KIMI_CHILD_SELECTION_UNSUPPORTED`. A user `[tools]` table is copied only when it exactly matches the projection; any conflict fails with `E_KIMI_CAPABILITIES_CONFIG_CONFLICT` rather than overriding user policy.

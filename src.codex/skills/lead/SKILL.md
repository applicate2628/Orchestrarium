...
- **Close.** Set the active epic file `status: closed` and write its `## Closure` (outcome, residual risk, and a `Closed: <YYYY-MM-DD>` line) ONLY when ALL child work-items are closed AND the epic goal is met. Then `$knowledge-archivist` moves that same file to `work-items/epics/archive/<YYYY-MM>/<slug>.md`, synchronizes lifecycle indexes, and verifies unique resolution. A closed epic left in the active root is invalid transitional residue. The epic `## Closure` MAY carry the same `## Retrospective` (`What went well` / `What didn't` / `Lessons` filed in the lessons registry by id) under the same proportionality rule.
...
- **Residual (honest).** The work-items-archival Stop hook scans the active root plus `work-items/epics/archive/<YYYY-MM>/`: it flags ready active epics, closed epics left in the active root, archived active or stale-child epics, and duplicate slugs, reading `status` from the `---` frontmatter and requiring the exact `- <slug> (active|closed)` child shape. Still governance-only: nothing verifies the epic's `## Goal` is actually met (only that the children are closed), and a non-canonical child line is ignored rather than guessed.

...
if [[ $DEV_REPO -eq 1 ]]; then
  ...
  check_normalized_sha256 "$SHARED_REF_DIR/subagent-operating-model.md" \
    "df37ccc769e777acccbf9ef3ce9aa397f3831d3e26489e77c7de577502d69cc7" \
...
# Branch-local backstop for the cross-pack invariant (skill BODY byte-identical
# across all 4 packs). The invariant is owned by tests/test_common_skill_parity.py,
# but tests/ never ships on a standalone provider branch — a branch runs ONLY this
# validator, so without a pin here a body drift ships silently (the exact
# regression class that was live at HEAD until the 2026-07-11 Wave-A
# reconciliation). Hashes pinned 2026-07-11 against the reconciled bodies, same
# normalized-fingerprint pattern as the subagent-operating-model pins above.
# Refresh after a DELIBERATE body change (landed in all 4 packs together):
#   sed 's/\r$//' src.codex/skills/<name>/SKILL.md \
#     | awk 'NR==1 && /^[ \t]*---[ \t]*$/ {fm=1; next} fm==1 {if (/^[ \t]*---[ \t]*$/) fm=2; next} {print}' \
#     | sha256sum
...
for pin in "${COMMON_SKILL_BODY_PINS[@]}"; do
  pin_name="${pin%%=*}"
  pin_hash="${pin#*=}"
  check_common_skill_body_pin "$pin_name" "$pin_hash" "$SKILLS_DIR/$pin_name/SKILL.md"
done
...
for cs in "${indexed_common_skills[@]}"; do
  pin_found=0
  for pin in "${COMMON_SKILL_BODY_PINS[@]}"; do
    if [[ "${pin%%=*}" == "$cs" ]]; then pin_found=1; break; fi
  done
  if [[ $pin_found -eq 1 ]]; then
    pass "common-skill $cs has a body-parity pin"
  else
    fail "common-skill $cs has no body-parity pin (add it to COMMON_SKILL_BODY_PINS in both validators)"
  fi
done
...
# Pin the same four H2 headings the Claude mirror pins, as real `## ` heading
# matches — the previous case-insensitive substring check ("delegation" anywhere
# in the file) stayed green with `## Publication safety` dropped entirely or
# every heading demoted to prose. Anchored so prose mentions cannot satisfy it.
for section in "Role index" "Engineering hygiene" "Publication safety" "Core delegation principles"; do
  if grep -q "^## $section" "$AGENTS_FILE"; then
    pass "Section '## $section' present in AGENTS.md"
  else

...
agents-mode-reminder.sh / agents-mode-reminder.ps1 (the Codex-side variant --
see those files' own headers for why the Claude-side variant under
src.claude/agents/scripts/ is intentionally DIFFERENT content, not a copy: it
walks ./.claude/ + ~/.claude/ and speaks Agent-tool-dispatch idiom, this one
walks ./.agents/ + ~/.codex/ and speaks role/skill-activation idiom).
delegationMode is NOT a Codex CLI built-in; without a hook like this the main
session never sees "delegationMode: force" and never applies it.
...
layers stay out of scope on purpose, mirroring the .ps1/.sh siblings exactly):

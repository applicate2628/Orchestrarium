<#
.SYNOPSIS
    Thin wrapper around check-git-push-gate.py for PowerShell hosts.
.DESCRIPTION
    ...
    stdout: deny JSON if a `git push` is detected without the per-turn user
            approval marker or scan-evidence-plus-instruction; nothing
            otherwise.
...
try {
  ...
  $helper = Join-Path $scriptDir 'check-git-push-gate.py'

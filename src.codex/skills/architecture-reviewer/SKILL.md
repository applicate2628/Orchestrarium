...
Run this check only inside an already-triggered Architecture Reviewer gate; it creates no reviewer, lane, artifact, engine, or review loop. Within that review, it is mandatory for any batch containing 2+ defect fixes — or one fix touching a surface already fixed this cycle. Fixes must correct the single owner's logic, never pile a neighboring check because the first one wasn't trusted (spine: `No logic duplication / no fix layering`; full form: `shared/references/spine/no-fix-layering-one-correct-logic.md`, maintainer reference).
...
A finding's `fix-class` controls design-versus-implementation routing. An `inline-sufficient` finding keeps the Code-level→Implementer route with its advisory HOW attached: no separate fix-design/HOW-review pass is required before implementation. A `design-decision` finding keeps the Design-level→`$architect` route; the review report's HOW remains advisory. A plan-level finding keeps its `$planner` routing; the tag and classification contract still apply.

...
| Bot `+1` on the newest exact trigger, head unchanged, no current finding comment, no unresolved current bot thread, and no other unresolved exact trigger on the same current head | clean | Record bot PASS; continue any separate human/publication gates. |

...
- `$external-worker` и `$external-reviewer` — это двусторонние external adapters, а не новые узкие профессии. В Codex они dispatch'ят в Claude CLI; в Claude Code — в Codex CLI. Сам adapter не делает silent fallback на внутреннего specialist. Если внешний CLI недоступен, роль считается disabled, а orchestrator может reroute'ить работу в обычную internal role.

...
- Codex может расширять shared `agents-mode` schema полем `externalClaudeProfile` для выбора Claude CLI execution profile (`sonnet-high`, `opus-xhigh` — shipped default, или `opus-max` — max-depth escalation), когда `externalProvider` resolves to Claude.

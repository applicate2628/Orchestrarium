...
- `knowledge-archivist` — cross-cutting hygiene role, usually invoked outside the main feature phase.
- `consultant` — advisory-only и никогда не становится reviewer или approver; ordinary consultant use optional, а closeout consultant sweep запускается только по explicit request или repo-local policy while `consultantMode` enabled.

## 8. Claims Chain

Claims chain — это traveling artifact, который гарантирует, что builder claims доходят до reviewers.
...
- `CI`: Continuous Integration; automated checks before merge, push or release.
- `Claim-Verify`: review mode where reviewer checks explicit builder claims against evidence.
- `Codex`: OpenAI Codex runtime used by this pack.
- `QA`: Quality Assurance; verification stage for behavior and regressions.
- `REVISE`: gate result meaning bounded correction is required in the same role.
- `UI`: User Interface.
- `UX`: User Experience.

...
|---|---|---|---|---|
| Freshness audit for active items | `$lead` | Every resume or session start | Physical active membership plus each active item's `status.md` current snapshot | Update `status.md`, or park/archive the item through the lifecycle owner before continuing |
...
| Physical-state reconciliation | `$knowledge-archivist` | Every resume, lifecycle move, or completion | Physical backlog/active/archive roots are exclusive and the generated human read-model reflects them | Repair through the lifecycle owner before delivery continues; compatibility snapshots do not gate progress |
| Registry governance reconciliation | `$knowledge-archivist` | Accepted task-memory governance change, all-registry request, or milestone-wide cleanup | Complete per-record matrix separates structural validity from semantic currency across every current registry; each exception names its semantic owner | Route every semantic exception through `$lead`; overall `PASS` requires both gates, while ambiguous ownership is `BLOCKED` |
...
| Repo consistency audit | `$knowledge-archivist` | Weekly | `AGENTS.md`, `shared/references/`, `references-codex/`, `src.codex/skills/`, and the configured task-memory docs remain consistent | Open a bounded docs or hygiene follow-up; if the fix changes governance semantics, route it through `$architecture-reviewer` before publication |
| Publication-safety spot check | `$lead` or the relevant reviewer | Weekly or before publication | `$lead` runs the publication-safety command documented in the root repository docs or Codex runtime docs when preparing publication; relevant reviewers may run the same check for spot checks, and staged-diff review still shows tracked content is free of secrets, raw logs, full transcripts, and machine-specific paths | Redact or move raw material to `/.scratch/` before publication |
| Refactor debt scan | `$architecture-reviewer` | Milestone close or quarterly | Recent code-touching areas do not accumulate avoidable duplication, coupling drift, or seam erosion; the repo-wide anti-layering pass finds no PILED defect class — each class has ONE owner, historical layering counts (full form: the no-fix-layering spine reference) | Admit a bounded refactor item through normal intake |

...
| Extended config schema | `consultantMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, provider-specific workdir keys, and Claude transport keys when Claude is selectable | `consultantMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, provider-specific workdir keys, `externalClaudeSecretMode`, `externalClaudeApiMode`, optional `externalClaudeProfile` | `consultantMode` controls consultant; `delegationMode` and `mcpMode` are operator-level routing/tooling preferences; `externalProvider` now uses one shared provider universe `auto | codex | claude | gemini`; `auto` resolves through the active named priority profile rather than by line-default asymmetry; `externalPriorityProfile` selects the named provider-order map; `externalPriorityProfiles` stores per-profile lane orderings; `externalOpinionCounts` raises specific lanes above the default single-opinion behavior; provider-specific workdir keys stay separate and default `neutral`; `externalClaudeSecretMode` and `externalClaudeApiMode` are Claude transport controls, and `externalClaudeProfile` matters only on Codex when the resolved provider is Claude CLI. |

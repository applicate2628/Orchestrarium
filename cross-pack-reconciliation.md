...
| Extended config schema | `consultantMode`, `externalClaudeApiMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, provider-specific workdir keys, shared `externalModelMode`, Gemini fallback, and optional Codex-only Claude profile when those providers are selectable | `consultantMode`, `externalClaudeApiMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, provider-specific workdir keys, shared `externalModelMode`, `externalGeminiFallbackMode`, optional `externalClaudeProfile` | `consultantMode` controls consultant; `externalClaudeApiMode` is the single Claude wrapper-transport toggle; `delegationMode` and `mcpMode` are operator-level routing/tooling preferences; `externalProvider` now uses one shared provider universe `auto | codex | claude | gemini`; `auto` resolves through the active named priority profile rather than by line-default asymmetry; `externalPriorityProfile` selects the named provider-order map; `externalPriorityProfiles` stores per-profile lane orderings; `externalOpinionCounts` raises specific lanes above the default single-opinion behavior; provider-specific workdir keys stay separate and default `neutral`; shared `externalModelMode` distinguishes provider runtime-default execution from the pinned top-model policy; `externalGeminiFallbackMode` matters only when Gemini is selected under pinned mode; and `externalClaudeProfile` matters only on Codex when the resolved provider is Claude CLI. |
| Provider dispatch | Shared provider universe with profile-driven `auto`; explicit `claude` is self-provider override only on Claude line | Shared provider universe with profile-driven `auto`; explicit `codex` is self-provider override only on Codex line | `auto` no longer means a line-specific provider default. It resolves through the active priority profile, must not silently self-bounce into the current host provider, and any extra visual-lane preference should now be expressed through an explicit provider override or a repo-local custom profile. |

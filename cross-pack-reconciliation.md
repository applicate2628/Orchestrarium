...
| Extended config schema | `consultantMode`, `externalClaudeApiMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, production-provider workdir keys, shared `externalModelMode`, and any line-specific local fields | `consultantMode`, `externalClaudeApiMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, `preferExternalReviewer`, `externalProvider`, `externalPriorityProfile`, `externalPriorityProfiles`, `externalOpinionCounts`, production-provider workdir keys, shared `externalModelMode`, and optional `externalClaudeProfile` | `consultantMode` controls consultant; `externalClaudeApiMode` controls only the supplemental `claude-secret` advisory/review candidate; `delegationMode` and `mcpMode` are operator-level routing/tooling preferences; `externalProvider` allows explicit example providers but production `auto` uses only `codex | claude`; `externalPriorityProfile` selects the named production provider-order map; `externalPriorityProfiles` stores per-profile lane orderings without example-only providers and may include `claude-secret` only on advisory/review lanes; `externalOpinionCounts` raises specific lanes above the default single-opinion behavior; production-provider workdir keys stay separate and default `neutral`; shared `externalModelMode` distinguishes provider runtime-default execution from pinned production-provider policy; and `externalClaudeProfile` matters only on Codex when the resolved provider is primary Claude CLI. |

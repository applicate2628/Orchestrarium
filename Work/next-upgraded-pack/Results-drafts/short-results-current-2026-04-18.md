...
| `X6 / flash-lite` | `13 / 40` | partial lower-bound row: `13 PASS`, `19 FAIL`, `8 NOT-RUN`; remaining cells are timeout/auth-route requeues |
...
| `63` | `N59` was added on `2026-04-24` as W39/E49 real-repo performance-cache hardening; it is not merged into the old full-v2 denominator; `X1 / gpt-5.5` and `X3 / opus 4.7max` both pass hidden pricing semantics, batch runtime budget, exact patch scope, and evidence gates, so `binary tie remains`. Scored rubric still separates role fit: `X1 PASS 90 / 100` with `336382` output bytes and cost `0`, while `X3 PASS 100 / 100` with `2653` output bytes and cost `10`. `X2` scoreably fails (`50 / 100`) after leaving the starter unchanged and missing runtime/evidence/scope gates; `X6` is runtime `NOT-RUN` after no-summary timeout. X5 quota check after runs produced no explicit quota error, but both wrapper and direct Gemini Pro probes timed out; current X5 status is route/runtime unhealthy, not quota-clear | `N59 real-repo performance cache` |
...
| `../Evidence/x1-mainline-hardening-no-new-failures-2026-04-21.md` | contains the admitted N06, wave-2 review, wave-3 S06, wave-4 S22, and N14..N82 hardening sections |
...
| `../Evidence/n59-perf-cache-rubric-2026-04-24.json` | machine-readable N59 real-repo performance-cache scored-rubric output for admitted `X1`, `X2`, `X3`, and `X6` launch roots; X1 and X3 both pass binary gates, X3 wins cost/compactness (`100` versus `90`), X2 fails scoreably, and X6 is runtime no-summary |

...
The X1 retry after quota also passes at `100 / 100`, so `binary tie remains`; N92 does not change
the `full-v2-hard /40` denominator.

W72 ran `N93-multipackage-protocol-sdk-reentry`: ProtocolMesh multi-package core/SDK/plugin/CLI
migration with v2 wire dataclasses, legacy-envelope migration, plugin idempotency, structured CLI
return, clean-room public imports, staged source/migration/sdk-compat/reentry ledgers, protected
inputs, and exact nineteen-path scope. `X3 / opus 4.7max` passes at `100 / 100`; `X1 / gpt-5.5`
scoreably fails as a no-op with no quota/timeout signature. `X2` also fails as no-op, and `X6` fails
...
| `2` | `X4` current read is valid only on the repo-canonical secret-backed Claude path; earlier same-day non-secret and broken-path attempts are not part of the admitted table. The pre-N85 closing run was `32 / 40`; after current N85 promotion the live `full-v2-hard` row is `31 / 40` with one final-only `NOT-RUN` | `Claude China` |
...
| `81` | `X4 / Claude China opus max` completed the admitted closing run on `2026-04-26` before the later N85 replacement: that batch was `32 / 40`, with `32 PASS`, `8 scoreable FAIL`, and `0 NOT-RUN`. In the current `full-v2-hard` `/40`, N85 is promoted and remains final-only for X4, so the live X4 score is `31 / 40` with one `NOT-RUN`. | `X4 full-v2-hard closing comparison` |
| `82` | `X2 / gpt-spark` filled all `21` former `full-v2-hard` `NOT-RUN` slots with `wrapperExitCode=0`: `7` new PASS and `14` new scoreable FAIL. Canonical `X2` is now closed at `12 / 40`. `X6 / flash-lite` filled `25` additional cells before Gemini quota exhaustion; after the N85 replacement its current score is `13 / 40`, with `9` remaining `NOT-RUN` cells including promoted `N85`. Route/auth requeues remain separate from model failures. | `X2/X6 full-v2-hard fill` |
...
| `../Evidence/n91-staged-security-incident-rubric-2026-04-29.json` | machine-readable W70 real-repo staged security incident evidence; X1 and X3 both pass, so `binary tie remains` |
| `../Evidence/n92-staged-interface-reentry-rubric-2026-04-29.json` | machine-readable W71 staged interface/downstream evidence; X1 and X3 both pass, so `binary tie remains` |
| `../Evidence/n93-multipackage-protocol-rubric-2026-04-29.json` | machine-readable W72 multipackage protocol/SDK/CLI evidence; X3 passes while X1 and calibration rows fail scoreably |

...
| `X5` | `gemini3.1pro` | `14 / 40` | `14 PASS`, `3 FAIL`, `23 NOT-RUN` | deprecated for new runs; historical partial hardened calibration only |
| `X2` | `gpt-spark` | `12 / 40` | `12 PASS`, `28 FAIL`, `0 NOT-RUN` | closed lower-bound row after 2026-04-26 fill run |
| `X6` | `gemini3.1flash-lite-preview` | `13 / 40` | `13 PASS`, `18 FAIL`, `9 NOT-RUN` | deprecated for new runs; historical lower-bound row only |
...
## Performance Review Gate W66 Diagnostic

`N87-performance-review-gate` tests read-only performance architecture review without an output
budget. It is not part of the `full-v2-hard` `/40` denominator.

| Scenario | `X1 / gpt-5.5` | `X3 / opus 4.7max` | Read |
|---|---|---|---|
| `N87` | `PASS`; wrapper `0`; benchmark admissibility, cache-boundary, response gate, and exact scope pass | `PASS`; wrapper `0`; same review gates pass | `binary tie remains`; X3 is compact but not a binary winner |

W66 interpretation: benchmark-admissibility review, cache-key context diagnosis, global cache
...
`N92-staged-interface-downstream-reentry-gauntlet`, `N93-multipackage-protocol-sdk-reentry`, and
`N94-staged-multipackage-protocol-reentry` test interface-breakage axes outside the `full-v2-hard`
`/40` denominator. N92 is closed as a PASS/PASS top-pair tie. N93 is closed as a diagnostic
X3-over-X1 separator after X1 no-ops and X3 completes the multipackage protocol/SDK/CLI migration
contract. N94 reruns that axis with stricter staged phase/reentry/closeout ledgers.
...
| `N94` | `PASS`; wrapper `0`; exact nineteen-path staged multipackage scope; `2453724` worker bytes | `PASS`; wrapper `0`; exact nineteen-path staged multipackage scope; `3164` worker bytes | `binary tie remains`; diagnostic-only |
...
readiness to the N93 ProtocolMesh migration contract. It does not invert N93 into an X1-over-X3
separator: both top rows pass. `X2` fails scoreably as a no-op. `X4` route-fails with
`UNKNOWN_CERTIFICATE_VERIFICATION_ERROR`, so it is not a model fail. `X5` and `X6` are deprecated for
new runs. The canonical `/40` denominator is unchanged.
...
| `short-results-current-2026-04-18.md` | compact operator-facing live status through `N94` |
...
| `../Evidence/n94-staged-multipackage-protocol-rubric-2026-04-29.json` | machine-readable `W73` staged multipackage protocol evidence; X1 and X3 both pass, X2 fails scoreably, X4 route-fails, X5/X6 deprecated, not promoted into `/40` |

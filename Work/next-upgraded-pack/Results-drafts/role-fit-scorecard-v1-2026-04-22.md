...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, and `N14..N35` |
...
| `worker.default-implementation` | `S15`, `S19`, `S20`; plus `N14`, `N15`, `N16`, `N27`, `N28`, `N29`, `N33`, and `N35` diagnostics | passes current ordinary implementation pilots and `N35`; best for staged interface migration | compact winner on `N16`, `N27`, `N28`, `N29`, and `N33`, but scoreable `N35 FAIL` under staged interface re-entry | partial baseline `1 / 3`; passes `N27` and `N35`; fails adjacent implementation repeats | baseline `3 / 3`; mostly route-gated on recent long tasks | baseline `3 / 3`; mostly route/runtime-gated on recent long tasks | split by execution shape: `X3 primary` for compact single-session implementation; `X1 primary` when staged re-entry/ledger accountability is part of the job |
| `worker.staged-delivery-reentry` | `N30` and `N35` staged runners with four fresh invocations over one run root | `PASS` on both; `96 / 100` on N30 and N35; kept persisted ledgers complete despite high output cost | scoreable `FAIL` on both; N30 missed `03-review-response` in phase state; N35 missed hidden runtime semantics and migration-ledger details | `N30 FAIL`; `N35 PASS 91 / 100` but with phase-discipline penalty | recent Gemini Pro staged rows are route/quota failures | recent Flash staged rows are runtime-route/tool-loop failures | `X1 primary` for multi-session delivery, staged interface migration, re-entry, and phase-ledger accountability |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, `N23`, `N24`, `N25`, `N26`, `N27`, `N28`, `N29`, `N30`, `N31`, `N32`, `N33`, `N34`, and `N35` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| `$backend-engineer`, `$data-engineer`, `$platform-engineer` | `worker.default-implementation` | `X3 primary` for compact single-session long-horizon, cross-role, ownership-budget, and ordinary interface-migration repairs | `X1` for verbose trace-heavy patch or staged accountability; `X2` calibration-only despite N27/N35 passes | high for compact single-session work; high for X1 on staged interface migration | N14/N15/N16/N27/N28/N29/N33 keep ordinary top-pair binary gates tied, while N35 proves staged interface migration is a separate X1-favored execution shape |
| `$backend-engineer`, `$platform-engineer`, `$lead` integration-owner simulations | `worker.staged-delivery-reentry` | `X1 primary` for staged re-entry and persisted phase-ledger accountability | `X3` for compact single-session slices only; use an explicit ledger checklist if X3 is selected for staged work | high from two binary separators | N30 and N35 both use four fresh invocations and both produce `X1 PASS` versus `X3 scoreable FAIL`; N35 also ties the result to interface-migration breakage |
...
| staged delivery, multi-session re-entry, phase-ledger accountability | `X1 primary`, `X3 secondary`; N30 and N35 are binary separators on persisted phase-ledger / re-entry accountability |
| staged interface migration / API refactor across fresh sessions | `X1 primary`, `X3 secondary`; N35 is `X1 PASS` versus `X3 scoreable FAIL` |
...
| interface refactor hypothesis | N33 tied as a single-shot task, but N35 found a top-pair binary split once interface migration was staged across fresh sessions; next meaningful repeat should be a real-repo staged interface migration |

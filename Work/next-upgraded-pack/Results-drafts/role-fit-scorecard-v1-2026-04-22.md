...
drift. The explicit `X1 / gpt-5.4 xhigh` hard-5 comparison also passes `5 / 5`, so the staged
X1-over-Claude split is not solely a `gpt-5.5` refresh artifact.
...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, and `N14..N45` |
...
| `worker.default-implementation` | `S15`, `S19`, `S20`; plus `N14`, `N15`, `N33`, and `N44` diagnostics | passes ordinary implementation pilots; more verbose but behavior-safe; `N44 PASS 96` | passes ordinary implementation pilots and leads by compactness on single-session refactor work; `N44 FAIL 72` from patch hygiene, not hidden sourceId semantics | partial baseline `1 / 3`; `N33` scoreable fail | baseline `3 / 3`; recent long tasks mostly route-gated | baseline `3 / 3`; recent long tasks mostly runtime-gated | `X3 primary` for compact ordinary single-session implementation; use `X1` when exact patch hygiene, trace, or test-led delivery dominates |
...
| `worker.long-horizon-incident-repair` | `N16`, `N27`, `N28`, `N29`, `N45`, and `N41` | passes all current cells; `N41 PASS 100 / 100` is the staged incident separator; `N45 PASS 96` ties by binary | passes compact single-session `N16/N27/N28/N29/N45`, but `N41` scoreable `FAIL 78 / 100` | calibration-only; `N27 PASS`, but `N28/N29/N41` all fail scoreably | recent long-horizon family remains smoke-gated or route-fail; `N41` route-fails | route-fail or no-summary across the family; `N41` no final summary | split by execution shape: `X3 primary` for compact single-session long-horizon, cross-role, and ownership-budget repair; `X1 primary` for staged incident-budget re-entry with repair ledger, reentry state, exact patch budget, and closeout |
...
| `worker.systems-performance-implementation` | `S13`, `S14`, `S21`; plus `N19`, `N24`, `N39`, and `N42` systems/toolchain probes | `PASS` on `N19`, `N24`, `N42`, and staged `N39 94 / 100` | `PASS` on `N19`, `N24`, and `N42`; staged `N39` scoreable `FAIL 78 / 100` after stale recovery source, wrong owner, quota classification, and ledger/closure misses | `N19 PASS`; scoreable `N24` and `N39 FAIL 76` | `N24 FAIL` after smoke and `N39` route-fail | scoreable `N19/N24/N39 FAIL` | split by execution shape: `X3 primary` for compact single-session systems/toolchain patches; `N42` shows immutable-CI alone does not make `X1` fail; `X1 primary` for staged systems/toolchain recovery, source arbitration, runtime-status discipline, and closeout |
| `worker.ui-implementation` | `S16`, `S17`, `S18`; plus `N20`, `N25`, `N38`, and `N43` UI probes | `PASS` on `N20`, `N25`, `N43`, and staged `N38 94 / 100`; only scoreable staged UI pass so far | `PASS` on `N20`, `N25`, and `N43`; `N38` stayed runtime no-summary | scoreable `N20`, `N25`, and `N38` FAIL | `N25 PASS 98 / 100`; `N38` route-fail | `ROUTE-FAIL` on `N20`; `N25` route-fail; `N38` route-fail | `X3 primary` for compact single-session UI state/render patches; `N43` shows immutable visible tests alone do not make `X1` fail; staged UI/visual-state remains unresolved because `N38` produced only an `X1` scoreable pass and no scoreable `X3` result |
| `worker.visual-graphics-visualization` | `S22`, `S23`, `S24`; hardened `S22`; plus `N21` and `N38` visual-state evidence | hardened `S22 23 / 23`; `N21 PASS`, rubric `89 / 100`; `N38 PASS 94 / 100` in the combined staged UI/visual packet | hardened `S22 23 / 23`; `N21 PASS`, rubric `100 / 100`; `N38` no-summary | `N21 PASS`, rubric `85 / 100`; `N38` scoreable fail | `N21` and `N38` runtime-route / runtime-fail | `N21` and `N38` runtime-route / no-summary | co-primary for visual correctness; `X3 provisional-primary` for compact single-session raster patches, but staged visual-state remains unresolved because `N38` never produced a scoreable `X3` result |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, `N23`, `N24`, `N25`, `N26`, `N27`, `N28`, `N29`, `N30`, `N31`, `N32`, `N33`, `N34`, `N35`, `N36`, `N37`, `N38`, `N39`, `N40`, `N41`, `N42`, `N43`, `N44`, and `N45` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| `$backend-engineer`, `$data-engineer`, `$platform-engineer` | `worker.default-implementation` | `X3 primary` for compact ordinary single-session implementation; `X1` when exact patch hygiene or test-led source trace matters | `X2` calibration-only | medium-high | N14, N15, and N33 keep ordinary binary gates tied while compactness favors X3; N44 shows exact changed-path hygiene can flip the single-session refactor read toward X1 without proving hidden semantic superiority |
...
| `$backend-engineer`, `$platform-engineer`, `$lead` incident-owner simulations | `worker.long-horizon-incident-repair` | `X3 primary` for compact single-session incident, cross-role, and ownership-budget repair; `X1 primary` once the same family becomes staged re-entry | `X1` for staged repair-ledger/accountability work; `X2` calibration-only | high | N16/N27/N28/N29/N45 all tie by binary and favor X3 on compactness/cost, but N41 is a scoreable staged separator: `X1 PASS 100`, `X3 FAIL 78` |
| `$performance-engineer`, `$reliability-engineer`, `$toolchain-engineer` | `worker.systems-performance-implementation` | `X3 primary` for compact single-session systems/toolchain patch; `X1 primary` for staged recovery / source arbitration / runtime-status closeout | avoid `X2/X5/X6` as primary | high for staged X1, medium-high for single-session X3 | N19 and N24 both preserve binary correctness for X1/X3 and give X3 a `9` point single-session rubric edge; bounded N39 splits staged systems/toolchain as `X1 PASS 94` versus `X3 FAIL 78` |
| `$frontend-engineer`, `$qt-ui-engineer`, `$model-view-engineer` | `worker.ui-implementation` | `X3 primary` for compact single-session UI state/render patch; staged UI remains unresolved rather than overturned | `X1` as safe secondary and the only current scoreable staged UI pass; `X5` as route-healthy contender for single-session form-state UI | medium-high for X3 over X1; medium for X5 | N20 and N25 both favor X3 over X1; N38 gives X1 the only scoreable staged UI pass, but X3 never completed the staged packet |
| `$geometry-engineer`, `$graphics-engineer`, `$visualization-engineer` | `worker.visual-graphics-visualization` | `X3 provisional-primary` for compact single-session raster patch; `X1` co-primary for correctness and staged visual-state delivery | `X2` calibration-only; Gemini rows not promoted | medium for correctness, low for staged top-pair certainty | S22 and N21 tie X1/X3 by binary visual correctness; N38 adds an X1 staged visual-state pass but no scoreable X3 completion |
...
| long-horizon / cross-role / ownership-budget repair, compact patch, low output cost | `X3 primary` for single-session work, `X1 secondary`; `N45` confirms hidden report-consumer hardening still ties by binary while X3 wins cost; `X2` remains calibration-only and fails N28/N29/N41 scoreably |
...
| systems/toolchain ownership patch with path/cache/lock/fingerprint semantics | `X3 primary` for compact single-session patches; `X1 primary` when the same lane is staged across recovery source arbitration, runtime-status discipline, ledger, and closeout; avoid `X2/X5/X6` as primary |
| UI state/render/keyboard/form interaction patch | `X3 primary` for single-session work, `X1 secondary`; consider `X5` when Gemini Pro route is healthy; N38 leaves staged UI unresolved because X3 never completed scoreably |
...
| ordinary single-session interface refactor / migration patch | `X3 primary` for compact migration when patch hygiene is not the main risk; `X1 primary` when exact changed-path budget and test/source trace are first-class; N33 ties by binary and favors X3 compactness, while N44 flips to X1 on patch hygiene |
...
| long-horizon, cross-role, and ownership-budget integration evidence | complete on both branches: `N16`, `N27`, `N28`, `N29`, and `N45` favor `X3` for compact single-session work by rubric/cost while still tying by binary; `N41` is a staged separator in favor of `X1` |
...
| systems/toolchain lane repeat evidence | complete on both branches: `N19` and `N24` favor `X3 95 / 100` over `X1 86 / 100` for compact single-session work; `N42` shows immutable-CI production-only hardening still ties top pair by binary; bounded `N39` favors `X1 PASS 94` over `X3 FAIL 78` for staged systems/toolchain recovery |
| inverse `X1 FAIL / X3 PASS` search | current honest attempts `N42`, `N43`, and `N45` did not find one; both top rows pass once visible tests are explicitly protected and hidden consumers are added. `N44` went the other way: X3 failed patch hygiene through `.pytest_cache` scope drift while preserving hidden sourceId semantics |
| next inverse probe | If continuing synthetic inverse search, make low-noise/cost budget a first-class hard gate in a compact single-session lane; post-hoc transcript size alone is only a cost signal, not a semantic fail |
| UI implementation repeat evidence | single-session evidence is complete for X3 versus X1: `N20` and `N25` both favor `X3`; staged UI remains unresolved because `N38` produced only an `X1` scoreable pass while `X3` timed out without summary; `X5` still needs one more UI-family pass before promotion over X3 |

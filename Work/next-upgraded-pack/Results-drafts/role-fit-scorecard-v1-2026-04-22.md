...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, `N14`, `N15`, `N16`, `N17`, `N18`, `N19`, `N20`, `N21`, `N22`, and `N23` |
...
| `worker.systems-performance-implementation` | `S13`, `S14`, `S21`; plus `N19` systems/toolchain rubric | `PASS` on `N19`; rubric `86 / 100`; test-rich but high output cost | `PASS` on `N19`; rubric `95 / 100`; compact and faster | `PASS` on `N19`; rubric `84 / 100`; calibration only | partial baseline `2 / 3`; not launched on `N19` after recent route timeout | scoreable `N19 FAIL`; missed portable cache key and source trace | `X3 provisional-primary` for systems/toolchain patches; `X1 secondary` when explicit test augmentation matters |
| `worker.ui-implementation` | `S16`, `S17`, `S18`; plus `N20` UI interaction rubric | `PASS` on `N20`; rubric `87 / 100`; safe top-pair secondary | `PASS` on `N20`; rubric `95 / 100`; compact and low output | scoreable `N20 FAIL`; created forbidden `.reports/` bundle entry | baseline `3 / 3`; not launched on `N20` after recent route timeout | `ROUTE-FAIL` on `N20`; partial patch missed multiple UI invariants | `X3 provisional-primary` for UI state/render patches; `X1 secondary`; avoid `X2/X6` as primary |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, and `N23` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| diagnostic labels | current `X3` edges on implementation, systems, UI, owner recovery, and visual raster are useful routing preferences, but not final global proof |
...
| `$performance-engineer`, `$reliability-engineer`, `$toolchain-engineer` | `worker.systems-performance-implementation` | `X3 provisional-primary` for compact systems/toolchain patch | `X1` for test-rich patch; `X2` calibration-only | medium | N19 preserves binary correctness for X1/X3 but gives X3 a `9` point rubric edge; W4 repeat should confirm before hard policy |
| `$frontend-engineer`, `$qt-ui-engineer`, `$model-view-engineer` | `worker.ui-implementation` | `X3 provisional-primary` for compact UI state/render patch | `X1` as safe secondary | medium | N20 gives X3 an `8` point rubric edge; W4 repeat should confirm before hard policy |
...
| new task is long-horizon / integration | yes | only if direct smoke writes worker output | only if runner has not recently no-output timed out | Gemini no-output hangs should stay runtime `NOT-RUN`, not model fail |
...
| systems/toolchain ownership patch with path/cache/lock semantics | `X3 provisional-primary`, `X1 secondary`; avoid `X6` as primary |
| UI state/render/keyboard interaction patch | `X3 provisional-primary`, `X1 secondary`; avoid `X2/X6` as primary |
...
| systems/toolchain lane needs repeat evidence before hard policy change | repeat N19-style path/cache/lock semantics on one platform-infra task if routing policy will depend on it |
| UI implementation needs repeat evidence before hard policy change | repeat N20-style interaction oracle on one browser/Qt-flavored task if routing policy will depend on it |

...
time rather than binary correctness. The explicit
...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, and `N14..N49` |
...
| `worker.systems-performance-implementation` | `S13`, `S14`, `S21`; plus `N19`, `N24`, `N39`, `N42`, and `N50` systems/toolchain probes | `PASS` on `N19`, `N24`, `N42`, `N50`, and staged `N39 94 / 100` | `PASS` on `N19`, `N24`, `N42`, and `N50`; staged `N39` scoreable `FAIL 78 / 100` after stale recovery source, wrong owner, quota classification, and ledger/closure misses | `N19 PASS`; scoreable `N24` and `N39 FAIL 76` | `N24 FAIL` after smoke and `N39` route-fail | scoreable `N19/N24/N39 FAIL` | split by execution shape: `X3 primary` for compact single-session systems/toolchain patches by rubric/elapsed time; `N42` and `N50` show immutable-CI and explicit output budget do not make `X1` fail; `X1 primary` for staged systems/toolchain recovery, source arbitration, runtime-status discipline, and closeout |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, `N23`, `N24`, `N25`, `N26`, `N27`, `N28`, `N29`, `N30`, `N31`, `N32`, `N33`, `N34`, `N35`, `N36`, `N37`, `N38`, `N39`, `N40`, `N41`, `N42`, `N43`, `N44`, `N45`, `N46`, `N47`, `N48`, `N49`, and `N50` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| systems/toolchain ownership patch with path/cache/lock/fingerprint semantics | `X3 primary` for compact single-session patches; `X1 primary` when the same lane is staged across recovery source arbitration, runtime-status discipline, ledger, and closeout; if a hard turnaround SLA exists, verify it as a first-class gate; avoid `X2/X5/X6` as primary |
...
| systems/toolchain lane repeat evidence | complete on both branches: `N19` and `N24` favor `X3 95 / 100` over `X1 86 / 100` for compact single-session work; `N42` and `N50` show immutable-CI plus explicit output budget still tie top pair by binary; bounded `N39` favors `X1 PASS 94` over `X3 FAIL 78` for staged systems/toolchain recovery |
| inverse `X1 FAIL / X3 PASS` search | `N46` found the first honest compact single-session inverse separator by making low-noise/operator-budget behavior a visible hard gate; `N47` repeats it on UI and `N48` repeats it on visual raster. `N49` and `N50` do not repeat it on scientific optimizer or systems/toolchain because both top rows pass the explicit budget. Earlier attempts `N42`, `N43`, and `N45` tied; `N44` went the other way through X3 patch-hygiene drift |

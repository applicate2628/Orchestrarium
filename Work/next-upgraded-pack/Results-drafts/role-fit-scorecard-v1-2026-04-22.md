...
Follow-up inverse probes `N42` and `N43` did not find an inverse `X1 FAIL / X3 PASS` separator:
both top rows pass and keep protected tests untouched. `N44` instead adds an `X1`-over-`X3`
patch-hygiene separator on interface refactor/sourceId work: X3 preserves hidden sourceId semantics
but fails exact changed-path scope through `.pytest_cache` drift. The explicit `X1 / gpt-5.4 xhigh`
hard-5 comparison also passes `5 / 5`, so the staged X1-over-Claude split is not solely a `gpt-5.5`
refresh artifact.
...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, and `N14..N44` |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, `N23`, `N24`, `N25`, `N26`, `N27`, `N28`, `N29`, `N30`, `N31`, `N32`, `N33`, `N34`, `N35`, `N36`, `N37`, `N38`, `N39`, `N40`, `N41`, `N42`, `N43`, and `N44` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| long-horizon, cross-role, and ownership-budget integration evidence | complete on both branches: `N16`, `N27`, `N28`, and `N29` favor `X3` for compact single-session work, while `N41` is a staged separator in favor of `X1` |
...
| inverse `X1 FAIL / X3 PASS` search | current honest attempts `N42` and `N43` did not find one; both top rows pass once visible tests are explicitly protected. `N44` went the other way: X3 failed patch hygiene through `.pytest_cache` scope drift while preserving hidden sourceId semantics |
| next inverse probe | W25/N45 should derive from `N29`: freeze the visible deploygrid test, keep single-session ownership-budget execution, and add hidden double-run replay plus public report-consumer checks before running X1/X3 |

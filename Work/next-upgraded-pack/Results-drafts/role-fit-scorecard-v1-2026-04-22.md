...
| `x1-mainline-hardening-no-new-failures-2026-04-21.md` | admitted hardening record for `N06`, wave 2, `S06`, `S22`, and `N14..N41` |
...
| `n38-ui-visual-state-rubric-2026-04-23.json` | `E28` staged UI/visual/state integration; `X1 PASS 94 / 100`, `X2 scoreable FAIL 78 / 100`; `X3` is repeated runtime no-summary and Gemini rows are route caveats |
| `n39-staged-toolchain-rubric-2026-04-23.json` | `E29` staged systems/toolchain re-entry; `X1`, `X2`, and `X3` all scoreably fail at `78 / 100` on the same exact-scope gate; `X5` is route-fail and `X6` is runtime no-summary |
...
| `worker.default-implementation` | `S15`, `S19`, `S20`; plus `N14`, `N15`, and `N33` diagnostics | passes ordinary implementation pilots; more verbose but behavior-safe | passes ordinary implementation pilots and leads by compactness on single-session refactor work | partial baseline `1 / 3`; `N33` scoreable fail | baseline `3 / 3`; recent long tasks mostly route-gated | baseline `3 / 3`; recent long tasks mostly runtime-gated | `X3 primary` for compact ordinary single-session implementation; `X1 secondary` when extra trace/tests matter |
...
| `worker.systems-performance-implementation` | `S13`, `S14`, `S21`; plus `N19`, `N24`, and `N39` systems/toolchain rubrics | `PASS` on `N19` and `N24`; `N39` scoreable `FAIL 78 / 100` under an over-tight exact-scope gate | `PASS` on `N19` and `N24`; `N39` scoreable `FAIL 78 / 100` under the same over-tight gate | `N19 PASS`; scoreable `N24` and `N39` FAIL | `N24 FAIL` after smoke and `N39` route-fail | scoreable `N19/N24 FAIL`; `N39` runtime no-summary | `X3 primary` for compact single-session systems/toolchain patches; keep `N39` out of policy because the staged variant is currently over-tightened for every scoreable row |
| `worker.ui-implementation` | `S16`, `S17`, `S18`; plus `N20`, `N25`, and `N38` UI rubrics | `PASS` on `N20`, `N25`, and staged `N38 94 / 100`; only scoreable staged UI pass so far | `PASS` on `N20` and `N25`; `N38` stayed runtime no-summary | scoreable `N20`, `N25`, and `N38` FAIL | `N25 PASS 98 / 100`; `N38` route-fail | `ROUTE-FAIL` on `N20`; `N25` route-fail; `N38` route-fail | `X3 primary` for compact single-session UI state/render patches; staged UI/visual-state remains unresolved because `N38` produced only an `X1` scoreable pass and no scoreable `X3` result |
...
| `E28 staged UI/visual/state integration` | `N38` staged runner roots plus X3 rerun | useful positive X1 staged UI evidence (`PASS 94 / 100`), but not a top-pair separator because X3 never produced a scoreable final summary; `X2` separates lower and Gemini rows are route caveats |
| `E29 staged systems/toolchain re-entry` | `N39` staged runner roots | not routing-grade; X1/X2/X3 all fail the same exact-scope gate at `78 / 100`, so the row is currently over-tightened rather than separating |
...
| cross-lane ranking | do not compare `N16`, `N19`, `N20`, `N21`, `N22`, `N23`, `N24`, `N25`, `N26`, `N27`, `N28`, `N29`, `N30`, `N31`, `N32`, `N33`, `N34`, `N35`, `N36`, `N37`, `N38`, `N39`, `N40`, and `N41` as one global `0..100` leaderboard without splitting semantic and efficiency points |
...
| diagnostic labels | current `X3` edges on compact single-session implementation, systems/toolchain, UI, owner recovery, and visual raster remain useful routing preferences; `N40` and `N41` add semantic staged-owner and staged-incident separators in favor of `X1`, while `N38` is unresolved and `N39` is over-tightened |
...
| `$backend-engineer`, `$data-engineer`, `$platform-engineer` | `worker.default-implementation` | `X3 primary` for compact ordinary single-session implementation and interface refactor work | `X1` for verbose trace-heavy patch or explicit test-led delivery; `X2` calibration-only | medium-high | N14, N15, and N33 keep ordinary binary gates tied while compactness favors X3; staged families now route separately |
...
| systems/toolchain ownership patch with path/cache/lock/fingerprint semantics | `X3 primary`, `X1 secondary`; avoid `X2/X5/X6` as primary; do not over-read `N39` until the staged scope contract is relaxed |
...
| systems/toolchain lane repeat evidence | single-session evidence is complete: `N19` and `N24` both favor `X3 95 / 100` over `X1 86 / 100`; if staged systems policy matters, redesign or relax `N39` because the current contract over-tightens every scoreable row |

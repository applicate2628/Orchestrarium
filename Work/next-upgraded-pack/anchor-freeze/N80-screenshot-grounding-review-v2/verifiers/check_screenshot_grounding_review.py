...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N80 screenshot grounding review bundle.")
...
def check_shape(root: Path, contract: dict, errors: list[str]):
    ...
    require(metadata.get("id") == "N80", "scenario id mismatch", errors)
...
def main():
    ...
    if args.bundle_shape_only:
        print("N80 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if metrics["matched_count"] != 0 or metrics["verdict"] == "PASS":
            ...
            return 1
        print("N80 verifier PASS (expected start-state failures present)")
        return 0
    ...
    if metrics["verdict"] != "PASS":
        ...
        print(f"N80 score: {metrics['score_0_100']} matched={metrics['matched_count']}/{metrics['expected_count']}", file=sys.stderr)
        return 1

    print("N80 verifier PASS")
    return 0

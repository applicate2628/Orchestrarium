...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the N07 scalability/maintainability cross-review bundle shape or a completed review report."
    )
...
def load_contract(bundle_root: Path):
    return json.loads(
        (bundle_root / "oracle" / "cross-review-contract.json").read_text(encoding="utf-8")
    )

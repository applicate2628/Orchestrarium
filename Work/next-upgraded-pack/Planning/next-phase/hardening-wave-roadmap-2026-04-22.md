...
`W4 / N24`, `W4 / N25`, `W6 / N26`, `W7 / N27`, `W8 / N28`, and `W9 / N29` are complete.
...
repairs with an exact changed-path gate do not binary-split the top pair. All four keep the top-pair
binary gate tied, so this is a role-fit and cost/compactness policy signal rather than a semantic
correctness separator. `X2` passed N27 as calibration but failed N28/N29 scoreably, so it remains
calibration-only. `X5` has no N27/N28/N29 semantic read because smoke failed or timed out.

Next admitted branch: stop trying to force a top-pair binary split from synthetic benchmark variants.
For role-fit policy, current evidence is enough to prefer `X3` for compact cross-role integration and
ownership-budget repair, while keeping `X1` as a fully capable secondary. If a binary separator is
still required, the next useful step is an external real-repo lane trial or a much longer
multi-session delivery simulation with accumulated context decay, not another single-scenario
DeployGrid variant.
...
14. Next wave should either move out of synthetic single-scenario variants or run a real-repo lane trial with time/cost/patch-quality scoring.
...
If interrupted now, resume after the `W9 / N29` closeout commit. Systems/toolchain, UI
...
binary gates still tie. `X5` is a live UI and owner-recovery contender after route-healthy `N25` and
`N26` wins/ties, but N27/N28/N29 produced only smoke-gated `REQUEUE`. The next useful work item is a
real-repo lane trial or multi-session delivery simulation, not another synthetic DeployGrid variant.

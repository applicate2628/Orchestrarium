...
PROTECTED_MARKERS = ("/docs/", "/legacy/", "/inputs/", "/oracle/", "/verifiers/")


def parse_args():
    parser = argparse.ArgumentParser(description="Score N19 toolchain run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N19-systems-toolchain-cache-gauntlet"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    source_changed = [path for path in changed if path.startswith("candidate/workspace/src/toolgate/")]

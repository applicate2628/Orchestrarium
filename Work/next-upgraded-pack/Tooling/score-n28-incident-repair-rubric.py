...
TOTAL_RECONCILIATION_INVARIANTS = 4
STATEFUL_INVARIANTS = {
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N28 incident-driven integration repair run roots.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N28-incident-driven-integration-repair-gauntlet"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    if binary == "PASS":
        ...
        runtime_failed = {item for item in failed if not item.endswith("-note") and item != "stale-source-rejected"}
        correctness = round(max(0, TOTAL_RUNTIME_INVARIANTS - len(runtime_failed)) / TOTAL_RUNTIME_INVARIANTS * 45)
    else:
        correctness = 0
    ...
    reconciliation_failed = {
        item
        for item in failed
        if item in {"source-arbitration-note", "stale-source-rejected", "review-response-note", "validation-evidence-note"}
    }
    reconciliation = (
        round((TOTAL_RECONCILIATION_INVARIANTS - len(reconciliation_failed)) / TOTAL_RECONCILIATION_INVARIANTS * 20)
    ...
    note_changed = any(path == "candidate/reconciliation-note.md" for path in changed)
    ...
    total = correctness + stateful_recovery + reconciliation + patch_quality + tests_points + time_points + cost_points
    ...
    notes.append("note changed" if note_changed else "note unchanged")
    if protected_like:
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Correct | Stateful | Recon | Patch | Tests | Time | Cost | Elapsed | Output | Notes |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {total} | {correctness} | {stateful_recovery} | "
            "{reconciliation} | {patch_quality} | {tests_points} | {time_points} | {cost_points} | {elapsed} | {bytes} | {notes} |".format(
                row=result.get("row"),
                ...
                reconciliation=result.get("reconciliation", 0),

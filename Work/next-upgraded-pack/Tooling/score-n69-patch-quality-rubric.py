...
SCENARIO_ROOT = Path("Scenarios-v2/N69-realrepo-patch-quality-scorecard")
VERIFIER = SCENARIO_ROOT / "verifiers" / "check_patch_quality_scorecard.py"
ROW_MODELS = {
    "X1": "gpt-5.5",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N69 patch-quality run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n69-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    correctness_fail = any(item.startswith(("correctness", "import")) for item in failure_ids)
    runtime_fail = any(item.startswith("performance") for item in failure_ids)
    patch_fail = any(item.startswith(("scope", "patch", "ledger")) for item in failure_ids)
    auxiliary_churn = bool(auxiliary_paths)

    correctness = 45 if not correctness_fail else 0
    runtime = 20 if not runtime_fail and runtime_seconds is not None else 0
    if patch_fail or not verifier_passed(summary, "check_patch_quality_scorecard.py"):
        patch_quality = 0
    elif auxiliary_churn:
        patch_quality = 10
    ...
    if output_bytes is None:
        cost = 0
    elif output_bytes <= 40000 and (wall_seconds or 999999) <= 420:
        cost = 15
    elif output_bytes <= 120000:
        cost = 8
    else:
        cost = 0
    ...
    return {
        ...
        "rubric": correctness + runtime + patch_quality + cost,
...
def print_table(results: list[dict]):
    headers = ["Row", "Binary", "Rubric", "Correct", "Runtime", "Patch", "Cost", "Verify s", "Wall s", "Bytes", "Failures"]
    print("| " + " | ".join(headers) + " |")

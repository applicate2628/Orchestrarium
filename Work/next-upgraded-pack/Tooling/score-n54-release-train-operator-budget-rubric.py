...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N54 release train compact operator-budget run roots.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N54-release-train-compact-operator-budget-gauntlet"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    if binary == "PASS":
        ...
        correctness = round(max(0, TOTAL_INVARIANTS - len(failed)) / TOTAL_INVARIANTS * 50)
    else:
        correctness = 0

    stateful_passed = len(STATEFUL_INVARIANTS - failed)
    stateful_recovery = round(stateful_passed / len(STATEFUL_INVARIANTS) * 15) if scoreability == "scoreable" else 0
    ...
    if diff_totals["added"] + diff_totals["deleted"] <= 420:
        patch_quality += 2
    patch_quality = max(0, min(15, patch_quality)) if scoreability == "scoreable" else 0

    tests_points = 10 if tests_changed else 4 if binary == "PASS" else 0
    ...
    total = correctness + stateful_recovery + patch_quality + tests_points + time_points + cost_points
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Correct | Stateful | Patch | Tests | Time | Cost | Elapsed | Output | Notes |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {total} | {correctness} | {stateful_recovery} | "
            "{patch_quality} | {tests_points} | {time_points} | {cost_points} | {elapsed} | {bytes} | {notes} |".format(
                row=result.get("row"),

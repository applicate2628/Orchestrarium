...
CONTRACT_PATH = (
    REPO_ROOT
    / "Scenarios-v2"
    / "N57-realrepo-compact-api-migration-operator-budget"
    / "oracle"
    / "compact-api-contract.json"
)


def parse_args():
    parser = argparse.ArgumentParser(description="Score N57 compact API migration operator-budget run roots.")
...
def score_one(run_root: Path, contract: dict, display_base: Path):
    ...
    ledger_failures = {
        ...
        "migration-callSites",
        "migration-compatibilityCases",
        "migration-validation",
        "migration-patch-budget",
    ...
    interface = max(0, 25 - 8 * len(failed_set & interface_failures))
    hidden = max(0, 30 - 5 * len(failed_set & hidden_failures))
    ledger = max(0, 25 - 4 * len(failed_set & ledger_failures))
    tests = 5 if "visible-test-markers" not in failed_set else 0
    ...
    patch = 10 if sorted(scored_changed) == sorted(contract["requiredChangedPaths"]) else 0
    ...
    return {
        ...
        "scored_changed_paths": scored_changed,
...
def main():
    ...
    if args.json_out:
        ...
        with args.json_out.open("w", encoding="utf-8", newline="\n") as handle:
            json.dump({"scenario": "N57", "surface": "E47", "results": results}, handle, indent=2)
            handle.write("\n")
    return 0

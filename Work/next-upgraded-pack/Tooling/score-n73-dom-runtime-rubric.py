...
SCENARIO_ROOT = Path("Scenarios-v2/N73-dom-event-runtime-ui-scorecard")
VERIFIER = SCENARIO_ROOT / "verifiers" / "check_dom_event_runtime_ui.py"
ROW_MODELS = {
    "X1": "gpt-5.5",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N73 DOM event runtime UI run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n73-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    runtime = 45 if not runtime_fail else 0
    keyboard_state = 20 if not keyboard_fail else 0
    if patch_fail or not verifier_passed(summary, "check_dom_event_runtime_ui.py"):
        patch_quality = 0
    ...
    return {
        ...
        "rubric": runtime + keyboard_state + patch_quality + cost,
        "runtime": runtime,
...
def print_table(results: list[dict]):
    headers = ["Row", "Binary", "Rubric", "Runtime", "Keys", "Patch", "Cost", "Wall s", "Bytes", "Failures"]
    print("| " + " | ".join(headers) + " |")

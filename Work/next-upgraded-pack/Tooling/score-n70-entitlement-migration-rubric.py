...
SCENARIO_ROOT = Path("Scenarios-v2/N70-entitlement-event-migration-scorecard")
VERIFIER = SCENARIO_ROOT / "verifiers" / "check_entitlement_event_migration.py"
ROW_MODELS = {
    "X1": "gpt-5.5",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N70 entitlement migration run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n70-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    patch_fail = any(item.startswith(("scope", "patch", "ledger", "static")) for item in failure_ids)

    correctness = 50 if not correctness_fail else 0
    runtime = 15 if not runtime_fail and runtime_seconds is not None else 0
    if patch_fail or not verifier_passed(summary, "check_entitlement_event_migration.py"):
        patch_quality = 0

...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N24 toolchain repeat run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N24-systems-toolchain-staging-repeat"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    if 3 <= len(changed) <= 8:
        patch_quality += 3
    if diff_totals["added"] + diff_totals["deleted"] <= 240:
        patch_quality += 2

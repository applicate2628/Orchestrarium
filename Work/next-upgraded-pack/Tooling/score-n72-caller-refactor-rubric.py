...
SCENARIO_ROOT = Path("Scenarios-v2/N72-caller-spanning-api-refactor-scorecard")
VERIFIER = SCENARIO_ROOT / "verifiers" / "check_caller_spanning_api_refactor.py"
ROW_MODELS = {
    "X1": "gpt-5.5",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N72 caller-spanning API refactor run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n72-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    api_fail = any(item in failure_ids for item in ["import", "tests-visible", "behavior-api-v2", "behavior-api-legacy", "behavior-payload-immutability"])
    caller_fail = any(item in failure_ids for item in ["behavior-service-v2", "behavior-cli-v2", "behavior-report-v2"])
    patch_fail = any(item.startswith(("scope", "ledger", "static")) for item in failure_ids)

    correctness = 45 if not api_fail else 0
    caller_coverage = 20 if not caller_fail else 0
    if patch_fail or not verifier_passed(summary, "check_caller_spanning_api_refactor.py"):
        patch_quality = 0
    ...
    return {
        ...
        "rubric": correctness + caller_coverage + patch_quality + cost,
...
def print_table(results: list[dict]):
    headers = ["Row", "Binary", "Rubric", "Correct", "Callers", "Patch", "Cost", "Wall s", "Bytes", "Failures"]
    print("| " + " | ".join(headers) + " |")

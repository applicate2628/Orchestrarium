...
CONTRACT_PATH = (
    ...
    / "oracle"
    / "owner-recovery-contract.json"
)


def parse_args():
    parser = argparse.ArgumentParser(description="Score N40 staged owner-recovery run roots.")
...
def failed_invariants(summary: dict):
    ...
    for result in summary.get("verificationResults", []):
        ...
        for line in log.read_text(encoding="utf-8", errors="replace").splitlines():
            ...
            elif line.startswith("N40 scope FAIL"):
            ...
            elif line.startswith("- Changed paths outside exact N40 owner recovery budget"):
...
def artifact_score(case_root: Path, contract: dict):
    notes = []
    ledger = load_json(case_root / "run" / "candidate" / "source-ledger.json") or {}
    route_text = (case_root / "run" / "candidate" / "route-decision.md").read_text(encoding="utf-8", errors="replace") if (case_root / "run" / "candidate" / "route-decision.md").exists() else ""
    runtime = load_json(case_root / "run" / "candidate" / "runtime-policy.json") or {}
    ...
    source = 20 if contains_all(ledger_text, contract["expectedSourceIds"]) else 0
    if not source:
        notes.append("source ids incomplete")
    stale = 15 if contains_all(ledger_text, contract["expectedStaleIds"]) and "no global winner" in ledger_text.lower() else 0
    if not stale:
        notes.append("stale rejection incomplete")
    route = 20 if contains_all(route_text, contract["routeDecisionRequiredSections"]) and contains_all(route_text, contract["routeDecisionMarkers"]) else 0
    if not route:
        notes.append("route decision incomplete")
    runtime_score = 15 if contains_all(runtime_text, contract["runtimePolicyMarkers"]) else 0
    if not runtime_score:
        notes.append("runtime policy incomplete")
    closure_score = 10 if (
        sorted(closure.get("changedPaths", [])) == sorted(contract["requiredChangedPaths"])
        and contains_all(closure_text, contract["closureMarkers"])
    ) else 0
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Source | Stale | Route | Runtime | Closure | Phase | Patch | Time | Output | Failed invariants |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
...
def main():
    ...
    if args.json_out:
        ...
        with args.json_out.open("w", encoding="utf-8", newline="\n") as handle:
            json.dump({"scenario": "N40", "surface": "E30", "results": results}, handle, indent=2)
            handle.write("\n")
    return 0

...
SCENARIO_ROOT = Path("Scenarios-v2/N75-persisted-state-replay-migration-scorecard")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N75 persisted-state replay migration run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n75-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    correctness_fail = any(
        item.startswith(("import", "tests", "normalization", "migration", "behavior-red", "behavior-blue", "behavior-status", "persist"))
        or item in {"behavior-schema-version", "behavior-source-immutability"}
        for item in failure_ids
    )
    replay_fail = any(item in {"behavior-dedupe", "behavior-idempotent-replay", "behavior-rollback", "behavior-rollback-nondestructive"} for item in failure_ids)
    patch_fail = any(item.startswith(("scope", "ledger", "static")) for item in failure_ids)

    correctness = 45 if not correctness_fail else 0
    replay_rollback = 20 if not replay_fail else 0
    if patch_fail or not verifier_passed(summary, "check_persisted_state_replay_migration.py"):
        patch_quality = 0
    ...
    if output_bytes is None:
        cost = 0
    elif output_bytes <= 50000 and (wall_seconds or 999999) <= 420:
        cost = 15
    elif output_bytes <= 150000:
        cost = 8
    else:
        cost = 0
    ...
    return {
        ...
        "rubric": correctness + replay_rollback + patch_quality + cost,
...
def print_table(results: list[dict]):
    headers = ["Row", "Binary", "Rubric", "Correct", "Replay", "Patch", "Cost", "Wall s", "Bytes", "Failures"]
    print("| " + " | ".join(headers) + " |")

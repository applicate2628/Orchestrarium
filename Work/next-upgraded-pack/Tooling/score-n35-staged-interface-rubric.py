...
CONTRACT_PATH = (
    REPO_ROOT
    / "Scenarios-v2"
    / "N35-staged-interface-migration-reentry-gauntlet"
    / "oracle"
    / "staged-interface-contract.json"
)


def parse_args():
    parser = argparse.ArgumentParser(description="Score N35 staged interface-migration run roots.")
...
def score_artifacts(case_root: Path, contract: dict):
    ...
    if "python candidate/workspace/tests/test_interfaceflow.py" not in closure_text:
...
def score_tests(case_root: Path, contract: dict, changed: list[str]):
    test_path = case_root / "run" / "candidate" / "workspace" / "tests" / "test_interfaceflow.py"
    if not test_path.exists():
        return 0, ["test file missing"]
    text = test_path.read_text(encoding="utf-8", errors="replace")
    if "candidate/workspace/tests/test_interfaceflow.py" not in changed:
...
def main():
    ...
    if args.json_out:
        ...
        with args.json_out.open("w", encoding="utf-8", newline="\n") as handle:
            json.dump({"scenario": "N35", "surface": "E25", "results": results}, handle, indent=2)
            handle.write("\n")
    return 0

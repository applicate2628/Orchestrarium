...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N33 interface-refactor run roots.")
...
def score_one(run_root: Path, contract: dict, display_base: Path):
    ...
    rubric = interface + hidden + ledger + tests + patch + output
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Interface | Hidden | Ledger | Tests | Patch | Output | Bytes | Failed invariants |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {rubric} | {interface} | {hidden} | "
            "{ledger} | {tests} | {patch} | {output} | {bytes_count} | {failed} |".format(
                row=result.get("row"),

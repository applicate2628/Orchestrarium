...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N45 ownership-budget immutable report-consumer run roots.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N45-ownership-budget-immutable-report-consumer"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    protected_test_clean = "candidate/workspace/tests/test_deploygrid.py" not in changed
    tests_points = 5 if binary == "PASS" and protected_test_clean else 0

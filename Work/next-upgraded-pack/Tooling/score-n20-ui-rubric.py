...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N20 UI run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N20-ui-command-palette-interaction-gauntlet"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    css_changed = any(path.endswith(".css") for path in changed)

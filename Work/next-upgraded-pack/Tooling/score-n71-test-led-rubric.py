...
SCENARIO_ROOT = Path("Scenarios-v2/N71-test-led-rate-limit-regression-scorecard")
VERIFIER = SCENARIO_ROOT / "verifiers" / "check_test_led_rate_limit.py"
ROW_MODELS = {
    "X1": "gpt-5.5",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N71 test-led regression run roots.")
...
def run_metrics(bundle_root: Path, meta_root: Path):
    metrics_path = meta_root / "n71-score-metrics.json"
...
def score_one(root_arg: str):
    ...
    correctness_fail = any(item.startswith(("correctness", "tests-visible")) for item in failure_ids)
    ...
    correctness = 40 if not correctness_fail else 0
    regression_test = 25 if not test_fail else 0
    if patch_fail or not verifier_passed(summary, "check_test_led_rate_limit.py"):
        patch_quality = 0
    ...
    return {
        ...
        "rubric": correctness + regression_test + patch_quality + cost,
...
def print_table(results: list[dict]):
    headers = ["Row", "Binary", "Rubric", "Correct", "Test", "Patch", "Cost", "Wall s", "Bytes", "Failures"]
    print("| " + " | ".join(headers) + " |")

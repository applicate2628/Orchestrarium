...
PROTECTED_MARKERS = ("/docs/", "/inputs/", "/oracle/", "/verifiers/")


def parse_args():
    parser = argparse.ArgumentParser(description="Score N47 UI compact operator-budget run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N47-ui-compact-operator-budget-hotfix"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    semantic_pass = any(
        result.get("passed") and "check_ui_dirty_state.py" in result.get("command", "")
    ...
    correctness = 40 if semantic_pass else 10 if scoreability == "scoreable" else 0

    changed = list(summary.get("benchmarkChangedPaths", []))
    tests_changed = any(path == "candidate/workspace/tests/editor-contract.test.mjs" for path in changed)
    source_changed = [path for path in changed if path.startswith("candidate/workspace/src/")]
    css_changed = any(path == "candidate/workspace/src/editor.css" for path in changed)
    ...
    if 2 <= len(changed) <= 4:
        patch_quality += 3
    if diff_totals["added"] + diff_totals["deleted"] <= 220:
        patch_quality += 2
    ...
    operator_budget = 10 if scoreability == "scoreable" and operator_budget_pass else 0
    total = correctness + patch_quality + operator_budget + time_points + cost_points if scoreability == "scoreable" else 0
    ...
    notes.append("css changed" if css_changed else "css unchanged")
    if protected_like:
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Correctness | Patch quality | Budget | Time | Cost | Elapsed | Output | Notes |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {total} | {correctness} | {patch_quality} | "
            "{operator_budget} | {time_points} | {cost_points} | {elapsed} | {bytes} | {notes} |".format(
                row=result.get("row"),

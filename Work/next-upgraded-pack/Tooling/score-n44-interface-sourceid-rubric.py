...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N44 interface-refactor/source-id run roots.")
...
def score_one(run_root: Path, contract: dict, display_base: Path):
    ...
    hidden = max(0, 35 - 7 * len(failed_set & hidden_failures))
    ledger = max(0, 15 - 3 * len(failed_set & ledger_failures))
    protected = 5 if not (failed_set & protected_failures) else 0
    ...
    rubric = interface + hidden + ledger + protected + patch + output
    ...
    return {
        ...
        "notes": ["failed invariants: " + ", ".join(failed)] if failed else [],
    }


def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Interface | Hidden/sourceIds | Ledger | Protected | Patch | Output | Bytes | Failed invariants |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {rubric} | {interface} | {hidden} | "
            "{ledger} | {protected} | {patch} | {output} | {bytes_count} | {failed} |".format(
                row=result.get("row"),

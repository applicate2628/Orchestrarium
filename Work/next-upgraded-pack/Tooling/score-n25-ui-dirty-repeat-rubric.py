...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N25 UI dirty-state repeat run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N25-ui-dirty-state-navigation-guard-gauntlet"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    correctness = 40 if binary_pass else 10 if scoreability == "scoreable" else 0
    ...
    total = correctness + patch_quality + time_points + cost_points if scoreability == "scoreable" else 0
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Correctness | Patch quality | Time | Cost | Elapsed | Output | Notes |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {total} | {correctness} | {patch_quality} | "
            "{time_points} | {cost_points} | {elapsed} | {bytes} | {notes} |".format(
                row=result.get("row"),

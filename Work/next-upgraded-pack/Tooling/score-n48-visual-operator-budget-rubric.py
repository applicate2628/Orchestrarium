...
def parse_args():
    parser = argparse.ArgumentParser(description="Score N48 visual compact operator-budget run roots with a diagnostic rubric.")
    parser.add_argument("run_roots", nargs="+", type=Path)
    parser.add_argument(
        "--scenario-root",
        type=Path,
        default=Path("Scenarios-v2/N48-visual-compact-operator-budget-hotfix"),
    )
...
def score_one(scenario_root: Path, run_root: Path):
    ...
    semantic_pass = any(
        result.get("passed") and "check_visual_panel.py" in result.get("command", "")
    ...
    source_changed = [path for path in changed if path == "candidate/visual-owned/src/visual_panel/renderer.py"]
    ...
    total = correctness + patch_quality + test_points + operator_budget + time_points + cost_points if scoreability == "scoreable" else 0
...
def print_markdown(results):
    print("| Row | Binary | Scoreability | Rubric | Correctness | Patch quality | Tests | Budget | Time | Cost | Elapsed | Output | Notes |")
    print("|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
    for result in results:
        print(
            "| {row} | {binary} | {scoreability} | {total} | {correctness} | {patch_quality} | "
            "{tests} | {operator_budget} | {time_points} | {cost_points} | {elapsed} | {bytes} | {notes} |".format(
                row=result.get("row"),

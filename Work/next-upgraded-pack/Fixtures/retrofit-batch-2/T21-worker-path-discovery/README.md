...
This is the normalized runnable fixture for `T21 / legacy G11`.

It upgrades the old worker path-discovery lane into the shared contract with:

- explicit `broken/` and `control-pass/` copies
- one real owner seam
- anti-drift checks for decoys and runtime surface
- generic owner selection across more than one basename

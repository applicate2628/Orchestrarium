...
SCRATCH_TEMP_ROOT = REPO_ROOT / ".scratch" / "l04x-authoring" / "test-tmp"
...
EXPECTED_SCORES = {
    "V4L04XB-store-buffer-litmus": {
        "competent": Decimal("87.15"),
        "decoy": Decimal("66"),
        "overclaim_raw": Decimal("93.33"),
        "max_atom_points": Decimal("8"),
    },
    "V4L04XF-store-buffer-litmus": {
        "competent": Decimal("87.86"),
        "decoy": Decimal("59.4"),
        "overclaim_raw": Decimal("94"),
        "max_atom_points": Decimal("8"),
    },
}
...
def _with_generated_candidate(
    ...
    candidate_path = SCRATCH_TEMP_ROOT / f"l04x-{root.name}-{uuid.uuid4().hex}.json"
...
class L04XPilotProbeMatrixTests(unittest.TestCase):
    maxDiff = None
    ...
    def _run_form_matrix(self, scenario_id: str) -> None:
        ...
        self.assertTrue(root.is_dir(), f"missing L04X root: {root}")
        ...
        self.assertEqual(_decimal(overclaim["raw_score"]), expected_scores["overclaim_raw"])
        self.assertEqual(
            overclaim["commitment_violations"],
            [{"atom_ref": "equivalence-gate.machines-verdict", "wrong_present_count": 1}],
        )
        ...
        for component_id, atom in case_atoms:
            observation_id = atom["id"]
            atom_ref = f"{component_id}.{observation_id}"
            declared_points = _decimal(reference_atoms[atom_ref]["max_points"])
            deleted = deepcopy(reference_candidate)
            del deleted[component_id][observation_id]
        ...
        emptied_set["divergent_program_ids"] = []
        ...
        self._assert_only_atom_changed(
            reference,
            mutant,
            "divergent-programs.divergent-set",
            _decimal(reference_atoms["divergent-programs.divergent-set"]["max_points"]),
        )

        gate_deleted = deepcopy(reference_candidate)
        del gate_deleted["equivalence_gate"]["machines_verdict"]
        ...
        def _check_gate_deletion(report: dict[str, Any]) -> None:
            self._assert_only_atom_changed(
                reference,
                report,
                "equivalence-gate.machines-verdict",
                _decimal(reference_atoms["equivalence-gate.machines-verdict"]["max_points"]),
            )
    ...
    def test_base_probe_matrix(self) -> None:
        self._run_form_matrix("V4L04XB-store-buffer-litmus")

    def test_frontier_probe_matrix(self) -> None:
        self._run_form_matrix("V4L04XF-store-buffer-litmus")

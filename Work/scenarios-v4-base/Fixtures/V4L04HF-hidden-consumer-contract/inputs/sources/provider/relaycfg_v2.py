...
`inputs/sources/changelog.md` enumerates every change as C01..C12.
"""
...
def _fold_key(key: str) -> str:
    """C11: config keys are canonicalized case-insensitively."""
...
class ConfigClient:
    ...
    C11: key spellings are canonicalized case-insensitively at load and at
    lookup; when two source entries fold to the same key, the later entry
    wins within its layer.
    """

    def __init__(self, file_values, env_values=None, stale_keys=()):
        self._file = {_fold_key(key): value for key, value in dict(file_values).items()}
        self._env = {_fold_key(key): value for key, value in dict(env_values or {}).items()}
        self._stale = frozenset(_fold_key(key) for key in stale_keys)
    ...
    def fetch(self, key: str, *, deadline: Optional[float] = None, timeout: Optional[float] = None) -> Any:
        ...
        C12: a key absent everywhere now raises MissingKeyError (a
        ConfigError) instead of the mapping-level KeyError.

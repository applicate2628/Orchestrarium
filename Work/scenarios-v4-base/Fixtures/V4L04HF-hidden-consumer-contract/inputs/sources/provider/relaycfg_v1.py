"""relaycfg v1.11 -- in-house configuration client SDK (pre-migration snapshot).
...
class MissingKeyError(ConfigError):
    """Raised by strict map accessors when a key is absent everywhere."""
...
class ConfigClient:
    ...
    Keys are case-sensitive: the reader stores and looks up key spellings
    exactly as given.
    """
    ...
    def fetch(self, key, *, timeout=None):
        ...
        A key absent everywhere surfaces as the mapping-level KeyError from
        the underlying store. The timeout argument bounds the replica wait;
        None means the client default.
        """
        self._backend_reads += 1

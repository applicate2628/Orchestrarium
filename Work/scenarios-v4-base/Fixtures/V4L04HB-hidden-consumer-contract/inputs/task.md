# Migration readiness review: relaycfg v1.9 -> v2.0

The platform team wants to ship relaycfg v2.0. Their own acceptance tests
(`inputs/sources/tests/`) all pass. Sixteen registered downstream services
consume the SDK; their code is not available here -- only the census
extraction records in `inputs/sources/consumers.md`. Your job is the
...
- A changelog entry is **breaking** when it breaks at least one registered
  consumer; `breaking_change_ids` is exactly that set. Claiming an entry
  that breaks no registered consumer counts against you.
...
Fill every consumer ID `CONS-01`..`CONS-16` in each of the three maps.
...
- `causes`: per consumer, `value` is one changelog ID `C01`..`C10` or

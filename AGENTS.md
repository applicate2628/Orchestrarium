...
- Default/tracked and explicit-path scans are manual pre-commit evidence only. When a pending push is otherwise admissible, the push gate captures the verified three-node local closure (`hook_common.py`, machine-path classifier, scanner), runs one fresh canonical sibling range scan, and accepts only the exact pending invocation's bounded, reaped, non-empty version-2 result bound to remote, destination, resolved source, current `HEAD`, and receipt tip. Transcript/manual output, legacy or zero-commit results, acquisition ambiguity, correlation/provenance failure, and drift deny. Human review and the explicit user publication marker remain required by their existing routes.

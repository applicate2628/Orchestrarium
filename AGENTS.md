...
When working inside this monorepo itself, do not assume that a missing local `.agents/` tree means the Codex install is broken. Maintainers may intentionally rely on the global Codex install under `~/.codex/` while editing the installer source. Before treating missing `.agents/.agents-mode.yaml` as a blocker or creating `.agents/` inside this monorepo, first ask the user whether this session should use the global install or whether they explicitly want a repo-local install created through `scripts/install-codex.*` (or the root router). If a local install is desired, explain that it should be created by the installer rather than ad hoc file creation.
...
- **Claude secondary transport:** on provider lines that may route work to Claude, the repo-local secondary transport is the installed secret-backed wrapper under `.claude/agents/scripts/invoke-claude-api.sh` or `.ps1`, which runs plain `claude` with `ANTHROPIC_*` loaded from `SECRET.md`. Claude CLI remains the first transport unless the active mode or operator override explicitly requests the secret-backed path first; after the allowed Claude CLI path is exhausted, the runtime must try that wrapper before declaring Claude unavailable.
...
- **Monorepo install choice:** when a skill or repo-local workflow wants Codex runtime state such as `.agents/.agents-mode.yaml` while operating inside this monorepo, treat the install location as an explicit operator choice. Ask whether to use the global install at `~/.codex/` or to create a repo-local install via `scripts/install-codex.sh|ps1` or `install.sh|ps1`; do not synthesize `.agents/` by hand inside the monorepo just to satisfy a gate.

...
- `$external-worker`: cross-provider implementation adapter for eligible implementer roles; preference comes from `.agents/.agents-mode` or explicit user routing
- `$external-reviewer`: cross-provider review and QA adapter for eligible reviewer roles; preference comes from `.agents/.agents-mode` or explicit user routing

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Validate that S15 changed paths stay inside the declared allowed change surface."
    )
...
def main():
    ...
    if len(allowed) != 2:
        print("ERROR: S15 expected exactly two allowed change paths", file=sys.stderr)
        return 1
    if not all(path.startswith("candidate/backend-owned/") for path in allowed):
        print("ERROR: Allowed change surface escapes the backend-owned root", file=sys.stderr)
        return 1
    ...
    if args.changed_path:
        print("S15 scope PASS (changed paths are in bounds)")
    else:
        print("S15 scope PASS (manifest is backend-owned only)")
    return 0

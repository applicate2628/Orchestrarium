...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the N83 interface-refactor gauntlet.")
...
def check_shape(root: Path, contract: dict, errors: list[str]):
    ignored_entries = set(contract.get("ignoredRuntimeArtifacts", []))
    actual_entries = sorted(path.name for path in root.iterdir() if path.name not in ignored_entries)
    require(
...
def evaluate_runtime(root: Path, contract: dict):
    ...
    def result_models():
        for name in contract["requiredResultModels"]:
            ...
            if getattr(modules["pkg"], name, None) is not model:
                raise AssertionError(f"package must export {name}")
        if not hasattr(modules["api"], "handle_event_batch"):
            raise AssertionError("api.handle_event_batch missing")
        if getattr(modules["pkg"], "handle_event_batch", None) is not getattr(modules["api"], "handle_event_batch"):
            raise AssertionError("package must export handle_event_batch")
        ...
        requests = [
            {
                "request_id": "req-0",
                "session_id": "active-admin",
                "event": {"event_id": "evt-batch-1", "tenant": "acme", "action": "read"},
            },
            {
                "request_id": "req-1",
                "session_id": "active-admin",
            ...
            },
            {
                "request_id": "req-2",
                "session_id": "active-user",
                "event": {"event_id": "evt-user-delete", "tenant": "acme", "action": "delete"},
            },
            {
                "request_id": "req-3",
                "session_id": "expired",
                "event": {"event_id": "evt-expired", "tenant": "acme", "action": "read"},
            },
            {
                "request_id": "req-4",
                "session_id": "active-admin",
                "event": {"event_id": "evt-blocked", "tenant": "blocked", "action": "read"},
            },
        ]
        results = modules["api"].handle_event_batch(records, rules, transport, requests, at_tick=contract["sessionNow"])
        if not isinstance(results, list):
            raise AssertionError("handle_event_batch must return a list")
        assert_equal(len(results), len(requests), "batch result count")
        for index, result in enumerate(results):
            assert_equal(result.get("batch_index"), index, f"batch[{index}].batch_index")
            assert_equal(result.get("request_id"), requests[index]["request_id"], f"batch[{index}].request_id")

        assert_equal(results[0]["status"], "accepted", "batch[0].status")
        assert_equal(results[0]["accepted"], True, "batch[0].accepted")
        assert_equal(results[0]["reason"], "accepted", "batch[0].reason")

        assert_equal(results[1]["status"], "accepted", "batch[1].status")
        assert_equal(results[1]["accepted"], True, "batch[1].accepted")
        assert_equal(results[1]["reason"], "duplicate-event", "batch[1].reason")

        assert_equal(results[2]["status"], "rejected", "batch[2].status")
        assert_equal(results[2]["reason"], "requires-admin", "batch[2].reason")
        assert_equal(results[2]["owner"], "policy-evaluator", "batch[2].owner")

        assert_equal(results[3]["status"], "rejected", "batch[3].status")
        assert_equal(results[3]["reason"], "expired", "batch[3].reason")
        assert_equal(results[3]["owner"], "session-store", "batch[3].owner")

        assert_equal(results[4]["status"], "rejected", "batch[4].status")
        assert_equal(results[4]["reason"], "blocked-tenant", "batch[4].reason")
        assert_equal(results[4]["owner"], "policy-evaluator", "batch[4].owner")

        assert_equal(len(transport.sent), 1, "batch must dispatch only the first accepted non-duplicate event")
    ...
    def structured_report_contract():
        dispatch = models.DispatchResult
        structured_results = [
            dispatch(
                accepted=True,
                status="accepted",
        ...
        report = modules["report"].build_audit_summary(structured_results)
        assert_equal(report.get("accepted"), 1, "structured-report.accepted")
        assert_equal(report.get("retryable"), 1, "structured-report.retryable")
        assert_equal(report.get("rejected"), 2, "structured-report.rejected")
        assert_equal(report.get("queued"), 1, "structured-report.queued")
        owners = report.get("owners")
        if owners != ["event-router", "policy-evaluator", "session-store"]:
            raise AssertionError(f"structured report owners drifted: {owners!r}")
        reasons = report.get("reasons", {})
        for reason in ["transport-timeout", "blocked-tenant", "missing"]:
            if reasons.get(reason) != 1:
                raise AssertionError(f"structured report reason {reason!r} missing")
    ...
    for case_id, fn in [
        ...
        ("batch-api-hidden-consumer", batch_api_hidden_consumer),
        ("report-contract", report_contract),
        ("structured-report-input", structured_report_contract),
    ]:
...
def main():
    ...
    if args.bundle_shape_only:
        print("N83 verifier PASS (bundle shape)")
        return 0
    ...
    if args.expect_start_state:
        ...
        if missing:
            ...
            return 1
        print("N83 verifier PASS (expected start-state failures present)")
        return 0

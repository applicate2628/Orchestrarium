...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N83 changed paths stay in scope.")
...
def main():
    ...
    ignored_prefixes = tuple(contract.get("ignoredChangedPathPrefixes", []))
    changed_paths = [path for path in args.changed_paths if not path.startswith(ignored_prefixes)]
    ...
    if budget.get("maxChangedPaths") != len(contract["requiredChangedPaths"]):
        print("ERROR: patchBudget.maxChangedPaths must match N83 required path count", file=sys.stderr)
        return 1
    if actual != required:
    ...
    print("N83 scope PASS (changed paths are in bounds)")
    return 0

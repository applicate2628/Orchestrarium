...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N53 changed paths stay in scope.")
...
def main():
    ...
    changed_paths = [
        path for path in args.changed_paths
    ...
    unexpected = sorted(path for path in args.changed_paths if path not in allowed and not any(path.startswith(prefix) for prefix in ignored_prefixes))
    if unexpected:
    ...
    if budget.get("maxChangedPaths") != len(contract["requiredChangedPaths"]):
        print("ERROR: patchBudget.maxChangedPaths must match N53 required path count", file=sys.stderr)
        return 1
    if actual != required:
    ...
    print("N53 scope PASS (changed paths are in bounds)")
    return 0

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the N65 visual-correctness review bundle shape or a completed review report."
    )
...
def load_contract(bundle_root: Path):
    return json.loads(
        (bundle_root / "oracle" / "visual-correctness-review-contract.json").read_text(encoding="utf-8")
    )
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(
        ...
        "scenario.yaml metadata does not match N65",
        errors,
    )
...
def main():
    ...
    print(f"N65 verifier PASS ({mode})")
    return 0

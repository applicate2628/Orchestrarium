...
def parse_args():
    parser = argparse.ArgumentParser(description="Check N33 changed paths stay in scope.")
...
def main():
    ...
    if budget.get("maxChangedPaths") != len(contract["requiredChangedPaths"]):
        print("ERROR: patchBudget.maxChangedPaths must match N33 required path count", file=sys.stderr)
        return 1
    if actual != required:
    ...
    print("N33 scope PASS (changed paths are in bounds)")
    return 0

...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the S13 performance-constraint bundle shape or a completed package."
    )
    ...
    parser.add_argument(
        ...
        help="Path to the S13 bundle root.",
    )
    parser.add_argument(
        "--bundle-shape-only",
        action="store_true",
...
def load_contract(bundle_root: Path):
    contract_path = bundle_root / "oracle" / "performance-constraint-contract.json"
...
def check_completed_package(bundle_root: Path, contract, errors):
    ...
    for marker in contract["disallowed_markers"]:
        require(marker not in package, f"Disallowed marker present in package: {marker}", errors)

    for section in contract["required_sections"]:
        require(section in package, f"Missing package section: {section}", errors)

    for budget_id in contract["required_budget_ids"]:
        require(budget_id in package, f"Missing budget reference: {budget_id}", errors)

    for bottleneck_id in contract["required_bottleneck_ids"]:
        require(bottleneck_id in package, f"Missing bottleneck reference: {bottleneck_id}", errors)

    for measurement_id in contract["required_measurement_ids"]:
        require(measurement_id in package, f"Missing measurement reference: {measurement_id}", errors)

    for tradeoff_id in contract["required_tradeoff_ids"]:
        require(tradeoff_id in package, f"Missing tradeoff reference: {tradeoff_id}", errors)

    for constraint_id in contract["required_constraint_ids"]:
        require(constraint_id in package, f"Missing constraint reference: {constraint_id}", errors)

    for gap_id in contract["required_gap_ids"]:
        require(gap_id in package, f"Missing evidence-gap reference: {gap_id}", errors)
    ...
    for phrase in contract["required_phrases"]:
        require(phrase.lower() in package_lower, f"Missing required phrase: {phrase}", errors)
...
def main():
    ...
    print(f"S13 verifier PASS ({mode})")
    return 0

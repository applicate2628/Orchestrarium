...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Check the S19 data pipeline bundle shape, start state, or completed run."
    )
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    if scenario_path.exists():
        ...
        require(
            ...
            "scenario.yaml values do not match the accepted S19 contract",
            errors,
        )
...
def main():
    ...
    contract = load_json(bundle_root / "oracle" / "pipeline-contract.json")
    ...
    print(f"S19 verifier PASS ({mode})")
    return 0

...
`run_visualization_checks.py` supports three modes:
...
- `--expect-start-state` checks that the bundled candidate root still exhibits the intended
  visualization-interpretation failures and nothing else
- default mode checks whether a completed candidate run satisfies the encoding oracle and the
  direct local test file
...
1. run `python verifiers/run_visualization_checks.py --bundle-shape-only`
2. run `python verifiers/run_visualization_checks.py --expect-start-state`
...
1. apply the candidate patch inside `candidate/visualization-owned/`
2. run `python tests/test_anomaly_section.py` from `candidate/visualization-owned/`
3. run `python verifiers/run_visualization_checks.py` from the bundle root

...
| `architecture-reviewer` | Maintainability reviewer | Independent architecture and maintainability acceptance | Architecture review report | Blocking design deviations не остаётся |
...
```text
`knowledge-archivist`: поддерживайте принятые документы, отчёты, references и структуру архива без выдумывания новых требований или переписывания принятой истории.
...
`architecture-reviewer`: независимо проверяйте alignment дизайна, dependency direction, coupling, complexity, extensibility и blast radius.
```
...
### Задача по repository-hygiene или documentation-maintenance

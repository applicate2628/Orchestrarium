...
- `BLOCKED` зарезервирован для реальных внешних blocker'ов, недостающих решений или недоступных prerequisites.
  - `BLOCKED` имеет два типизированных класса:
    - `BLOCKED:dependency` — невозможно продолжить, отсутствует tool, environment, доступ или информация, которую ни один текущий агент не может предоставить. Orchestrator представляет пользователю для решения.
    - `BLOCKED:prerequisite` — обнаружена смежная работа, которая должна завершиться первой (например, сломанный соседний модуль, отсутствующая миграция). Orchestrator фиксирует в `work-items/bugs/`, пользователь решает приоритет, возобновление после решения.
  - Если класс не указан, считается `BLOCKED:dependency` (консервативный дефолт).
...
- Re-intake cap: item может возвращаться к `product-manager` для re-intake не более 2 раз. На 3-й re-intake lead должен эскалировать пользователю со всеми предыдущими причинами re-intake и запросить финальное решение (reduce scope, defer или cancel).
...
1. Зафиксируйте issue в `work-items/bugs/`, используя bug registry format (из `qa-engineer.md`), с `context: adjacent-finding` и `status: open`.
2. Отметьте находку в текущем артефакте в секции "Adjacent findings".
3. НЕ расширяйте scope — orchestrator решает приоритет и scheduling.
...
Когда reviewer обнаруживает существенную проблему за пределами своего домена:

1. Пометьте finding: `[CROSS-DOMAIN: <target-domain>]`.
2. Опишите наблюдение фактически — не оценивайте severity за пределами своей экспертизы.
3. Orchestrator маршрутизирует помеченный finding к соответствующему специалисту.
4. Этот finding не блокирует текущий review gate, если только reviewer не может завершить assessment по собственному домену без него.
...
5. Gate: PASS | REVISE | BLOCKED | BLOCKED:<class> | RETURN(role)
```
...
Если work item включает admitted bug или prerequisite issue, исправление всегда Phase A. Cleanup, смежные fixes и feature work идут только после того, как admitted issue верифицированно исправлен.
```
...
Bugfix с известным файлом или функцией по умолчанию маршрутизируется через `quick-fix` template, даже если в ходе анализа обнаружены смежные проблемы. Смежные проблемы идут в `work-items/bugs/`, а не в текущий план.
...
При допуске нового candidate approach в discovery roadmap decision package должен включать:

- **Coherence statement**: какой shared state или contract удерживает этого кандидата как единое целое
- **Improvement hypothesis**: какой baseline он превосходит, на каких случаях, по какой метрике, через какой механизм
- **Non-redundancy argument**: почему он существенно отличается от предыдущих отклонённых кандидатов с похожими failure modes
- **Expected win/fail cases**: где кандидат успешен и где испытывает затруднения
- **Evaluation metric mapping**: как optimization objective отображается на benchmark objective
- **Shortest falsification experiment**: 2-3 случая, чёткий порог PASS/FAIL, минимальный tuning
- **Implementation seam**: изолированная lane, защищённые surfaces, минимальный seam

`$product-manager` обеспечивает 3 pre-admission gates (coherence, improvement hypothesis, non-redundancy). `$analyst` обеспечивает 4 research-phase gates (regression risk, metric alignment, known limits, bounded falsification). `$architect` подтверждает implementation isolation gate.
...
- `closure.md` обязателен перед перемещением item в `work-items/archive/`. Он содержит финальную запись о закрытии: outcome, residual risk и archive location.
- `status.md` имеет определённый формат с YAML frontmatter (template, orchestrator, started, updated) и секциями: Current state, Active agents, Completed agents, REVISE loop (optional), Next action. Полный формат определён в `subagent-contracts.md`.
...
- **Ambiguity resolution discipline:** не угадывайте; проверяйте. Разрешайте фактическую неоднозначность путём инспекции кода, конфигурации, данных или авторитетной документации, прежде чем выбирать интерпретацию, и указывайте, что было подтверждено. Если неоднозначность касается intent пользователя, policy, scope или архитектуры и инспекция не может её разрешить, не выдумывайте ответ: либо спросите, либо действуйте только с наименьшим безопасным reversible подмножеством, которое не фиксирует неразрешённый выбор, и укажите, что было отложено и почему. Блокируйте только если не существует безопасного forward action. Implementation-relevant решения должны прослеживаться к verified evidence или явной инструкции пользователя, а не к assumption.
- **Explicit bounds for background and fan-out work:** не создавайте долгоживущие фоновые процессы, автоматизацию, выполняющуюся вне прямого пути запроса (cron jobs, scheduled tasks, system hooks, startup scripts), или network listeners без явного одобрения пользователя. Если такой механизм необходим, обоснуйте его и спросите перед реализацией. Любая одобренная фоновая или fan-out работа должна иметь явные trigger conditions, concurrency limits, cancellation и shutdown behavior.
- **Autonomous external side effects:** не создавайте тикеты, не отправляйте сообщения, не публикуйте во внешние сервисы, не мутируйте состояние SaaS или cloud, не запускайте действия, видимые третьим сторонам, без явного одобрения пользователя. Если внешний side effect необходим, обоснуйте его и спросите перед выполнением.

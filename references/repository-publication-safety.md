...
- In this development repository, run `bash src.codex/skills/lead/scripts/check-publication-safety.sh` on Git Bash, macOS, or Linux as the manual pre-publication scan for staged tracked content before publication.
- In this development repository, run `powershell -ExecutionPolicy Bypass -File src.codex/skills/lead/scripts/check-publication-safety.ps1` on Windows PowerShell; the wrapper resolves Git-for-Windows `bash.exe` and calls the shared scan script so it does not rely on the WSL `bash.exe` stub from `PATH`.

...
| Repo consistency audit | `$knowledge-archivist` | Weekly | `AGENTS.md`, `references/`, `skills/`, and `work-items/` links remain consistent | Open a bounded docs or hygiene follow-up; if the fix changes governance semantics, route it through `$architecture-reviewer` before publication |

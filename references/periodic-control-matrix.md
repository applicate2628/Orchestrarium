...
|---|---|---|---|---|
| Work-items completeness audit | `$knowledge-archivist` | Every session start (before lead routing) | Active item folders contain `roadmap.md`, `brief.md`, `status.md`; brief has scope/owners/stage; status has current snapshot | Lead creates missing artifacts before proceeding; incomplete items are parked until restored |
...
| Repo consistency audit | `$knowledge-archivist` | Weekly | `CLAUDE.md`, `references/`, and `agents/` links remain consistent | Open a bounded docs or hygiene follow-up; if the fix changes governance semantics, route it through `$architecture-reviewer` before publication |
| Publication-safety spot check | `$lead` or the relevant reviewer | Weekly or before publication | `$lead` runs `bash agents/scripts/check-publication-safety.sh` on Git Bash/macOS/Linux or `powershell -ExecutionPolicy Bypass -File agents/scripts/check-publication-safety.ps1` on Windows when preparing publication; relevant reviewers may run the same check for spot checks, and staged-diff review still shows tracked content is free of secrets, raw logs, full transcripts, and machine-specific paths | Redact or move raw material to `/.scratch/` before publication |
...
- `$knowledge-archivist` completeness audit at session start before any specialist delegation

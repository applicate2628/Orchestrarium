...
| Publication-safety spot check | `$lead` or the relevant reviewer | Weekly or before publication | `bash scripts/check-publication-safety.sh` on Git Bash/macOS/Linux or `powershell -ExecutionPolicy Bypass -File scripts/check-publication-safety.ps1` on Windows, plus staged-diff review, show tracked content is free of secrets, raw logs, full transcripts, and machine-specific paths | Redact or move raw material to `/.scratch/` before publication |

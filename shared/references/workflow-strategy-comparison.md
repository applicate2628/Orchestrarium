...
- the shared `subagent-operating-model.md` core and the corresponding pack-local addendum
- the corresponding pack-local `operating-model-diagram.md`
- the corresponding agent pack's operating-model contract
...
| Stop-and-wait churn | `Rolling loop` | `PASS` advances, `REVISE` follows the shared spine's consecutive same-role/same-artifact cycle cap, `BLOCKED` stays rare |

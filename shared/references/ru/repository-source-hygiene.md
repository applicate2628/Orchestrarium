...
- Byte-identical canon status enforced'ится `tests/test_universal_hook_surfaces.py`, но ничто его не PROPAGATE'ило — каждый lane был обязан hand-copy'ить в остальные два дерева по памяти (`work-items/bugs/2026-07-26-mirror-parity-is-detected-but-never-propagated.md`). `scripts/sync-universal-hooks.py --sync` теперь propagate'ит canon-директорию в оба provider-mirror'а, refusing (никогда не guessing) файл, чья mirror-копия несёт uncommitted local changes, которые нельзя безопасно overwrite'ить; `--check` — быстрый standalone pre-report drift-check для собственного targeted pass лейна. Редактировать canon-копию, никогда mirror напрямую.

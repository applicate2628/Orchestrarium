...
- Version 3 range selection resolves the unique configured push destination without exposing it in child arguments. When the destination ref exists, its exact authoritative object identifier remains the sole exclusion. When the destination ref is absent, the scanner queries that same push destination with bounded `git ls-remote --refs`, validates a nonempty inventory of unique refs and exact repository-format object identifiers, then uses one bounded local `git cat-file --batch-check` owner to derive ancestry exclusions. Exact remote tips absent from the local object database are skipped because they cannot be ancestors of the locally resolved source; local commit tips are retained, annotated tags are accepted only through an exact local peel to a commit, and any other locally present object type refuses. Empty, malformed, duplicate, conflicting, over-limit, or failed authoritative inventories still refuse; local tracking refs are never selection authority, and a source already fully reachable from the derived commit tips remains a non-authorizing zero-commit selection.

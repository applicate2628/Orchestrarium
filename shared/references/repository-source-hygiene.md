...
- `src.claude/agents/scripts/` — Claude pack: co-locates blocking-enforcement hooks (`check-bugfix-discipline.*`, `check-git-push-gate.*`, `check-passive-polling-stop.*`, plus the shared `hook_common.py`), SessionStart context hooks (`mcp-usage-reminder.*`, `agents-mode-reminder.*`), provider invocation wrappers (`invoke-claude-api.*`, `invoke-claude-prompt.*`, `invoke-codex-prompt.*`), publication gates (`check-publication-safety.*`), and pack validators (`validate-skill-pack.*`).
...
- Byte-identical canon status is enforced by `tests/test_universal_hook_surfaces.py` but was never PROPAGATED by anything — every lane had to hand-copy to the other two trees from memory (`work-items/bugs/2026-07-26-mirror-parity-is-detected-but-never-propagated.md`). `scripts/sync-universal-hooks.py --sync` now propagates the canon dir to both provider mirrors, refusing (never guessing) a file whose mirror copy carries uncommitted local changes it cannot safely overwrite; `--check` is the fast, standalone pre-report drift check for a lane's own targeted pass. Edit the canon copy, never a mirror directly.

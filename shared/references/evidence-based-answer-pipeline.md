...
The discipline fires at two enforcement points for code-bearing work:
...
2. **Pre-commit (text rule only)** — before authoring a commit that fixes/alters behavior, all 5 steps of the Bootstrap (the four diagnostic steps plus Recovery readiness) must complete and the commit message must disclose the verified hypothesis chain. There is no machine check here; the agent is expected to follow the text rule.

The structural hook deliberately fires at pre-fix moment, not at git push: hypothesis-discipline is about *thinking before editing*, which is upstream of any publication mechanism. Tying the structural check to `git push` (an earlier design we removed) would have been theatre — by push time the unverified-hypothesis edit has already happened and the harm is done. Catching at edit time is what actually prevents wasted work.

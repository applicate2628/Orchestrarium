...
3. **Re-dispatch** the receipt-admitted angles on the revised artifact, carrying the ORIGINAL objective verbatim. Initial and full-artifact rounds admit all three angles; an eligible exact-delta correction round admits only its affected angles.
...
### Exact-delta correction rounds

The initial round and every full-artifact round run all three angles. After a correction, the orchestrator may admit a non-empty strict subset only when an accepted change classification identifies exactly the affected angles and every unaffected effective result from the immediately previous round remains valid for the corrected artifact: each retained verdict resolves to `PASS`, and a retained scout result is fully reconciled. Use a full-artifact round when the human requests one, a new defect class appears, the upstream artifact changes materially, or the affected-angle classification is unclear.

A retained angle is provenance to a previously completed effective result, not a fresh verdict, finding, or run. It has no new attempt ID and does not copy `PASS`, rationale, findings, or reconciliation into the new round. The orchestrator launches only the fresh attempt IDs returned by the committed receipt; it must not synthesize an attempt or manually copy a prior result. Exact-delta selection is provider- and model-agnostic and changes neither the existing dispatched role and external provenance requirements nor review authority, retry ownership and failure reconciliation, the three-round cap, or the absence of automatic continuous-integration enforcement.
...
The supported formal state path in a governed pack uses one runtime transaction. The transaction freezes the artifact, atomically persists and reads back the admitted round, then emits a committed receipt carrying the fresh attempt IDs and the round artifact revision. A correction-round receipt may also carry exact-delta coverage that binds the affected angles to the immediately previous round and revision; retained-angle provenance remains in the committed state. This binding is procedural, not an executable host-enforced dispatch gate. Provider bindings may add bounded dispatch-time observation, but the helper itself does not observe bypass; direct/ad-hoc launches remain outside its guarantees. The historical observer gap is fixed without expanding the helper's guarantees. Personal/operator-owned reviews remain outside this state path.

The state owner records pinned objective/scope/runtime-root anchors, ordered idempotent operations, and per-round phase, diff, frozen artifact identity, fresh attempts, retained references when applicable, failures, results, and evidence. Each admitted lane's failures form one append-only immediate-successor chain (`A → B → C`) ending at its current attempt: it has one root and tip, no branch, merge, cycle, cross-lane reuse, dangling/disconnected node, or historical unresolved failure, and at most three retry edges. Every result or failed attempt asserts the owning round's identity. Retry keeps that identity; a corrective edit creates a new round and a newly frozen identity. Missing outputs, unresolved failures, mismatched identity, changed snapshots, conflicting operation replay, or failed commit/read-back remain non-clean. A stable lock file carries an operating-system exclusive kernel lock; file presence is not ownership. Graceful cancellation cleans owned uncommitted resources. Hard termination cannot clean synchronously; the next lock-owning invocation reconciles only unreferenced engine-owned resources. Invalid or uncertain state returns `RLSTATE_RECOVERY_REQUIRED` without deleting candidate evidence. A repository-only structural validator exposes the same schema for development and continuous integration, but does not own a second copy of the rules and still cannot judge semantic soundness.

Existing V1 ledgers remain read-only; migration never guesses historical identity. Provider completion, work-item closure, scoped gate invalidation, and the operator-owned exception remain owned by their existing contracts.

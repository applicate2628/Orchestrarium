...
- **Documentation terminology amendment:** always expand domain terms, role names, provider or model names, workflow labels, acronyms, and unclear English terms on first use in the main conversation and chat replies, not only in documents (calibrate to comprehension; do not define ordinary words). When creating or materially updating a human-facing document, also end it with `## Terms and Abbreviations` or a localized equivalent whenever it uses such terms, expanding and briefly explaining them there, especially English abbreviations and mixed-language terms in non-English documents.
...
- Expand terms in the main conversation and chat too, not only docs; end terminology-heavy human-facing documents with a terms and abbreviations section.

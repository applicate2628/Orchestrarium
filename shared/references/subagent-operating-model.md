...
- `$consultant` is an independent advisory role; ordinary consultant use remains optional, and it never becomes a reviewer or approver. Completed lead-managed batches still end with one final external consultant-check before closure, but that check remains advisory-only and does not replace review or human gates. Consultant is configured via the pack-local `agents-mode` file. No config file = disabled for ordinary optional use. The shared schema is `consultantMode`, `delegationMode`, `mcpMode`, `preferExternalWorker`, and `preferExternalReviewer`; pack-local addenda may extend it with provider-specific fields. `consultantMode` governs consultant behavior, `delegationMode: auto` keeps ordinary team delegation enabled without per-turn approval while `manual` keeps explicit-request behavior, `mcpMode: auto` uses MCP by judgment while `force` treats relevant MCP use as an explicit standing instruction, and the preference flags route eligible implement roles through `$external-worker` and eligible review or QA roles through `$external-reviewer`.

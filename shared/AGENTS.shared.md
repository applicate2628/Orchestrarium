...
- treat `$consultant` as an independent advisory team member only: ordinary consultant use stays optional, it never becomes a blocking reviewer or approver, and any repo-local required consultant-check or consultant-check set remains advisory-only rather than a substitute for review or human gates
- when repo-local lane policy requires multiple external advisory opinions for the same lane, collect the configured number of distinct eligible external opinions before closure or fail closed with an explicit shortfall
...
- external adapters may run in parallel with one another when their scopes are independent, their artifacts do not overlap, and the selected provider runtimes support concurrent non-interactive execution
- do not impose a one-instance-per-provider cap on eligible external work: when admitted scopes are independent, the lead may launch multiple simultaneous instances of the same external helper and the same external provider
- keep `externalOpinionCounts` separate from general external fan-out: opinion counts govern how many distinct external opinions one lane must collect before it may close, while independent brigade-style external lanes may reuse the same provider repeatedly for different disjoint artifacts or slices
- when a bounded set of independent external lanes should launch together, the lead may use the pack-local external-brigade surface to define one brigade plan, track the parallel helper set, and aggregate their results without collapsing them into one vague delegation
- provider-specific addenda may define supplemental profile candidates or wrapper details; honor the pack-local lane restrictions exactly and do not infer a generic provider fallback, primary-transport retry, or worker-side route from a wrapper's existence
...
- **Visual artifact verification discipline:** generated images, diagrams, drawings, renders, charts, plots, screenshots, CAD or exported drawings, and other visual artifacts require direct visual inspection before acceptance, commit, user delivery, or evidentiary use. Successful generation commands, file existence, metadata, hashes, or model claims are not enough. Use an available viewer, renderer, screenshot path, or repo-standard visual check for the artifact type; if visual inspection is impossible, say so explicitly and do not claim the artifact is visually correct.
...
- If the session used a provider-backed path or an external adapter, add a short execution record that keeps these facts on separate lines instead of collapsing them into one mixed label:
  ...
  - `Requested provider` using `internal`, a production provider such as `claude` or `codex`, or an explicit example-provider label documented by the pack such as `gemini` or `qwen`

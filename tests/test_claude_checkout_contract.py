...
LF_ONLY_PATHS = (
    "tests/test_codex_skill_catalog_budget.py",
...
def test_claude_lf_contracts_survive_checkout_without_rewriting_prior_bytes(tmp_path, autocrlf, eol):
    ...
    for relative in (*LF_ONLY_PATHS, *BYTE_EXACT_PATHS, *MIGRATION_PIN_PATHS):
        path = origin / relative
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(b"exact LF contract\n" if relative in (*LF_ONLY_PATHS, *MIGRATION_PIN_PATHS) else b"historical\r\npayload\r\n")
    _git(origin, "init", "-q")
    ...
    for relative in MIGRATION_PIN_PATHS:
        assert (checkout / relative).read_bytes() == b"exact LF contract\n", relative

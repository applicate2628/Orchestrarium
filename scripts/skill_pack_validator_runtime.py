        ...
        if lines and lines[0].strip() == "---":
            for index in range(1, len(lines)):
                if lines[index].strip() == "---":
                    body_start = index + 1
                    break
        actual = hashlib.sha256("".join(lines[body_start:]).encode("utf-8")).hexdigest()
        if actual == expected:
            self.ok(f"common-skill {name} body matches the pinned canonical cross-pack body")
        else:
            self.fail(
                f"common-skill {name} body drifted from the pinned canonical cross-pack body "
                f"(expected {expected}, actual {actual} — a deliberate body change must land "
                "in ALL FOUR packs plus both validators' COMMON_SKILL_BODY_PINS)"
            )
...
def _direct(
    ...
    *,
    ...
    if kind == "common_pin_completeness":
        for name in extract_roles(layout.agents_text, "## Common skills"):
            _verdict(
                validator,
                name in COMMON_SKILL_BODY_PINS,
                f"common-skill {name} has a body-parity pin",
                f"common-skill {name} has no body-parity pin "
                "(add it to COMMON_SKILL_BODY_PINS in the canonical validator engine)",
            )
        return

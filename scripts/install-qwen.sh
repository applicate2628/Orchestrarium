...
# Qwen-native agents/ role layer is removed. This cleanup is source-independent
...
#   - user override tier ($AGENTS_TARGET, e.g. ~/.qwen/agents): remove ONLY the
...
if [[ ! -d "$SOURCE/skills" || ! -d "$SOURCE/commands" || ! -f "$SOURCE/QWEN.md" || ! -f "$SHARED_AGENTS_SOURCE" ]]; then

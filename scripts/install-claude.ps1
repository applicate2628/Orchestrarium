...
if (-not $NoHypothesisHook -and -not $DryRun -and [string]::IsNullOrEmpty($env:ORCHESTRARIUM_NO_HYPOTHESIS_HOOK)) {
    ...
    if (-not (Test-Path $HookInstaller)) {
        ...
        & $PythonCmd $HookInstaller --hook-runtime $HookRuntime --target $SettingsTarget --platform claude --host-os windows --script-marker check-mcp-momentum --tool-matcher "Grep|Bash" --script-path $McpMomentumScriptTarget
        if ($LASTEXITCODE -ne 0) {
            Write-Error "hypothesis-hook installer exited with code $LASTEXITCODE"
            exit $LASTEXITCODE
        }

...
cat <<'EOF'
{"hookSpecificOutput": {"hookEventName": "UserPromptSubmit", "additionalContext": "[turn anchor - re-shown every turn because a once-per-session reminder is overwritten by whatever you did last]\nRoot main conversation / Lead: continue until blocked; a passed slice is not completion. Record it, take the next unchecked action, and dispatch the next admitted role. A final-style summary while a known next action remains IS the defect. If you genuinely need the operator, name the blocker or decision as the reason for stopping.\nDispatched subagent: continue only the bounded work for your assigned profession, artifact, and gate; then return evidence and an optional non-binding recommended next role to the root, and stop. Never adopt $lead, dispatch a peer or downstream stage, advance the pipeline, or write agent-runs.jsonl.\nRoot-only delegation: at the first decision point of non-trivial work, the root main conversation holding $lead classifies and routes to the matching specialist role/skill via the host delegation surface; take external-launch flags from the external-dispatch contract, never from memory. The root may directly launch a configured external wrapper; no provider or leaf may recursively launch another wrapper.\nMCP checkpoint: for repository navigation or understanding, discover and use the relevant configured MCP before an ad-hoc shell search; if shell is genuinely the right instrument, state why. For stateful/indexed MCPs, after repository/project/ branch/worktree/indexed-input changes, use the MCP's own freshness probe; stale or pending means sync/update/reindex, fresh recheck, then repeat the query. Report a refresh failure; stateless/live MCPs are exempt."}}
EOF
exit 0

...
def resolve_external_executable(
    ...
    for root in worktrees:
        if _inside(resolved, root):
            raise VerificationError(
                f"{label} executable resolves inside tested worktree: {resolved}"
            )
    return resolved
...
def build_repository_env(
    *, tools: ExternalTools, lane_root: Path, repo_root: Path
    ...
    """Create a clean lane environment with one explicit reviewed import root."""
...
def _run_git(
    ...
    *arguments: str,
    ...
    if result.returncode != 0:
        ...
        raise VerificationError(
            f"git --no-replace-objects -C {repo} {' '.join(arguments)} "
            f"failed ({result.returncode}): {stderr.strip()}"
        )
...
def assert_clean_worktree(
    ...
    *,
    ...
    if actual_ref != expected_ref:
        raise VerificationError(
            f"worktree HEAD mismatch for {repo}: expected={expected_ref}, actual={actual_ref}"
        )
    if expected_tree is not None:
        actual_tree = (
            str(_run_git(tools, env, repo, "rev-parse", "HEAD^{tree}"))
            .strip()
            .lower()
        )
        if actual_tree != expected_tree:
            raise VerificationError(
                f"worktree tree mismatch for {repo}: expected={expected_tree}, actual={actual_tree}"
            )
    status_text = str(
        _run_git(
            tools,
    ...
    if ignored_paths:
        raise VerificationError(
            f"ignored executable, test, configuration, or bytecode input in {repo}:\n"
            + "\n".join(ignored_paths)
        )
    flag_data = bytes(
        _run_git(tools, env, repo, "ls-files", "-v", "-z", text=False)
    )
    ...
    if hidden_flags:
        raise VerificationError(
            f"hidden or non-normal index flags in {repo}:\n" + "\n".join(hidden_flags)
        )

...
| Repo consistency audit | `$knowledge-archivist` | Weekly | `README.md`, `INSTALL.md`, `references-gemini/`, `docs/agents-mode-reference.md`, and `src.gemini/` remain consistent | Open a bounded docs or hygiene follow-up; if the fix changes governance semantics, route it through `$architecture-reviewer` before publication |

---
name: agents-review
description: Run a full read-only repository impact review starting from current changes or a specified review target. Changed files are only the entry point; the review must cover the wider affected surface, including unchanged dependents, contracts, tests, config, and nearby logic.
...
2. **Run the review chain.** Follow the `review` template from CLAUDE.md:
   ...
   - **Anti-layering audit**: when the reviewed change is a multi-fix batch (2+ defect fixes, or one fix touching a surface already fixed this cycle), the architecture-reviewer lane MUST include the anti-layering audit on an engine distinct from the batch's author; a `PILED` verdict maps to `REVISE` and blocks push

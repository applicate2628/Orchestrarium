...
- Bind every manual request and result to the current `headRefOid` and exact `@codex review` comment ID. An automatic `New commits` run instead needs unique hosted automatic-run evidence on the current head plus a verified run start or observed head transition.
...
- Apply the same ownership rule to uncorrelated clean evidence. A submitted-review or REST issue-comment no-findings result may bind to a manual run only when complete hosted state leaves exactly one unresolved exact trigger candidate on the same `(repository, pull request, headRefOid)`. Two or more unresolved same-head exact trigger candidates mean that uncorrelated manual clean evidence remains `indeterminate`, even when its timestamp/order is later and its reviewed commit matches the head.
- Accept a manual-trigger reaction only when its author satisfies the connector author identity rule below and it is attached to that newest exact trigger. An automatic-run reaction or review has no manual trigger binding and must satisfy the automatic-run condition below. A reaction is intrinsically correlated only to its exact manual trigger. An intrinsically correlated reaction does not erase another unresolved same-head run. Accept a review, review comment, or review thread only when its author satisfies the same rule, it is bound to the current head, and its own authoritative timestamp is strictly later than the trigger timestamp. Because the REST issue-comment tie-breaker does not transfer to another surface, same-time evidence from different surfaces is incomparable, including between REST issue comments and either REST reviews or GraphQL threads; the run remains `indeterminate`. An earlier same-head review or reaction can never produce `clean`.
- Trigger or resolve threads only with explicit user authorization or a standing authorization for that PR.
- Before asking again for publication confirmation or treating the cycle as unauthorized, read the publication-gate owner's latest genuine grant, stored binding, and revocation state. New consent uses the standalone genuine-user marker `[approve-pr-publication]`, raw or inside one balanced pair of single backticks or double asterisks. Only a current-turn marker initializes a binding. Lead behaviorally reuses an active same-PR grant only for ordinary corrective pushes, exact fixed-thread resolution, and the next review trigger within the admitted task; unrelated same-branch work requires fresh consent. The gate mechanically binds repository, remote, head branch, and pull request, not semantic task scope or branch-wide consent. A historical short marker with no binding can initialize only from the latest unretracted exact scoped grant, after fresh root, remote, head, PR, and leak/range checks; otherwise it denies. Fresh authorization is required for first publication or no active binding, another PR, remote, or head branch, widened scope, exact revocation, or a later genuine user no-push instruction. Existing exact Version 1 URL/Markdown/numeric grants and exact unversioned URL-targeted forms remain supported compatibility input. A structured UI reply contributes only its scalar answer; question text cannot authorize. A later genuine user no-push instruction stops push even without the exact revoke token. This continuity never bypasses current-head and PR/Git binding checks, branch protection, human review, or a fresh leak/range receipt.
...
- A summary-only issue comment remains nonauthorizing for `clean`; so do other summary comments and `gh pr view` review/comment fields. A `Completed` summary alone is never `clean`; a summary may identify a running or completed run but is never `clean` alone. Findings from the exact post-trigger review on the current head take precedence over summaries, reactions, and clean-review signals. Any current-head automatic-run finding also takes precedence. Classify `clean` only when complete collections show no current finding comments or unresolved current connector threads, no competing run, no other unresolved exact trigger on the same current head, and either a manual exact trigger has a qualifying `+1`/review-result or an automatic `New commits` run satisfies the condition below.
...
- If an embedded REST author type differs, accept that reaction only when a fresh authoritative connector-account lookup returns `Bot` and its nonempty numeric REST user ID and node ID equal the reaction's corresponding identifiers. Missing, null, mismatched, or incomplete lookup remains `indeterminate`; never trust login, suffix, or avatar alone.
...
- An automatic `New commits` reaction may support `clean` only when unique hosted automatic-run evidence is current-head-bound, its authoritative timestamp is strictly later than the verified run start or observed head transition, and that comparison uses not poll time or commit-author timestamp.
...
| Connector-authored `+1` on the newest exact manual trigger, or an automatic `New commits` reaction satisfying the automatic-run condition, with head unchanged, no current finding comment, no unresolved current connector thread, and no competing run | clean | Record bot PASS; continue any separate human/publication gates. |

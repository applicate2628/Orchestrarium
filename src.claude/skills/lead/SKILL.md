...
- **Dispatch economics**: before every provider or subagent dispatch, resolve the model/profile and effort tier and record a one-line complexity rationale in the `status.md` Active agents `Model/effort` column. Native role definitions supply fixed installed profiles and default effort floors; a requested override changes effective execution only when the host explicitly supports it and confirms it in returned runtime metadata. Otherwise record the fixed resolved profile/floor or the unavailable/deviated result; a requested value alone is not execution truth. Once a run is launched, it keeps its resolved effort; preference changes apply only to the next dispatch — never swap an in-flight run.

...
- **Dispatch economics**: record a one-line route-complexity rationale in the `status.md` Active agents `Model/effort` column without inventing execution values. Claude internal Agent model and effort are host-selected; role definitions declare no fixed model, effort, or floor. Unless returned actual runtime metadata proves the effective values, record `unspecified by runtime`. Once a run is launched, preference changes apply only to the next dispatch — never swap an in-flight run.

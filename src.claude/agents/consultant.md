...
> 5. (external mode) Shell out synchronously to the selected provider via the prompt-orchestration wrapper (`.claude/agents/scripts/invoke-codex-prompt.sh` for Codex, `.claude/agents/scripts/invoke-claude-prompt.sh` for routine Claude, `.claude/agents/scripts/invoke-claude-api.sh` only when `reserveResolver` resolved to `claude-wrapper`). The prompt wrappers enforce file-based delivery, own the provider timeout, and return exactly one `ORCHESTRARIUM_PROVIDER_RESULT_V1=<json>` line after the provider reaches a terminal state. Read the complete provider response from the JSON object's `resultText`; validate its bounded primary outcome, combined `exitCode`/`token`/`status`/`gate`/`note`, `cleanupStatus`, `captureRecoveryRetained`, `cancelled`, `timedOut`, and `stderrMarkerCount`, plus the wrapper process exit. The wrapper materializes the result, disposes of its one private run directory, emits and flushes the envelope, then records the optional path-free terminal ledger. A tracked caller reads that ledger back separately; transient paths are not a consumer contract.
...
Required pattern — use the prompt-orchestration wrapper `invoke-codex-prompt.sh` (Bash / Git Bash) or `invoke-codex-prompt.ps1` (PowerShell). The wrapper enforces file-based prompt delivery so you do not have to construct the file/redirect chain by hand: it probes Codex availability, creates one randomized private run directory, runs `codex exec --json` non-interactively with the prompt on stdin, enforces its configured timeout and combined stream quota, validates the bounded provider result, tombstones and reclaims the exact run directory, emits and flushes one `ORCHESTRARIUM_PROVIDER_RESULT_V1=<json>` line, and only then records the optional path-free terminal event. Consume the complete response from `resultText`; for a tracked run, verify the process exit and read the ledger back separately. Do not expect or retain prompt, stdout, stderr, or process paths. Never embed the substantive prompt in argv.

...
externalProvider: {value}  # allowed here: auto | codex | claude | gemini | qwen | kimi | grok; default: auto; kimi/grok are explicit-only; gemini/qwen are explicit example-only and not recommended
...
- Every consultant memo must include a provenance header:
  ...
  - **Resolved provider:** a concrete Codex, Claude, Gemini, Qwen, Kimi, or Grok CLI route, or `none`

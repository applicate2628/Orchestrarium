...
- **Close.** Set the epic file `status: closed` and write its `## Closure` (outcome, residual risk) ONLY when ALL child work-items are closed (each per the per-item close step) AND the epic goal is met. This mirrors the per-item `closure.md`-before-archive discipline.

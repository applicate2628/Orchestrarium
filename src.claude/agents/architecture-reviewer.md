...
- **Procedure:** group the batch's changes by defect class; for each class verify exactly ONE owner holds the corrected logic. Red flags: the same invariant re-checked at several heights; duplicated producer+consumer validation without a process/artifact boundary; a fix papering over an earlier fix; an if-else pile begging for a table; an interim stub without a tracked root-cause item.
- **Per-class verdicts:** `CLEAN-SINGLE-OWNER` (one owner, no residual guards) / `JUSTIFIED-DEPTH` (duplication crosses a process/artifact boundary with agreed thresholds — record the justification) / `PILED` (layered patches — name the consolidation refactor).

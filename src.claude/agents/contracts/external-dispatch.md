...
- When the resolved provider is Codex, `externalCodexProfile: default` inherits `externalModelMode`; `gpt-6-sol-xhigh` (shipped default) and `gpt-6-sol-max` request the named GPT-6 Sol effort; `gpt-5.6-terra` keeps the balanced GPT-5.6 Terra lane; and explicit `gpt-5.6-sol-xhigh` and `gpt-5.6-sol-max` values retain their named model and effort through 1.x. Consultant invocations instead use the complete ordinary selector result.
...
- Use `gpt-5.6-terra` as a balanced Codex reasoning option when full `gpt-6-sol` depth is not required. Compare cost per verified result by task, model, and effort rather than assuming a static price or speed advantage; its output stays review-gated like any external lane.
...
- Kimi may be selected only for policy-admitted read-only exploration/research/planning/review or mapped `external-worker` engineering. Omission of the capability file keeps the prior no-tools/no-servers/no-subagents `reject` behavior; a validated file may select admitted read tools/servers or scoped worker tools. Every result remains nonauthorizing and independently verified. Grok remains unavailable and must never be selected, executed, or recorded as a realized provider in 1.x.

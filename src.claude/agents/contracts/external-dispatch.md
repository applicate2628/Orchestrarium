...
- `externalCodexProfile` is the Codex-specific external profile override after provider resolution. `default` inherits `externalModelMode`, including when `externalProvider: auto` resolves to Codex. `gpt-5.5-fast` selects the fast Codex model tier (the underlying model variant); reasoning_effort still stays at `xhigh`, so this is a model-tier choice rather than an effort downgrade — it must still be verified against the installed runtime before it is reported as used. `gpt-5.5-xhigh` (shipped as the default; symmetric to `externalClaudeProfile: opus-max`) explicitly requests model `gpt-5.5` with `model_reasoning_effort = "xhigh"` via `-c model_reasoning_effort=xhigh` regardless of `externalModelMode`. Consultant lane invocations always use `gpt-5.5-xhigh` (best effort) regardless of the operator-set value, mirroring the consultant-side Claude rule that always uses `--model opus --effort max`.

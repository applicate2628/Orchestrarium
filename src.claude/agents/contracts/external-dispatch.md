...
- The canonical Claude-pack transports for primary runs are `.claude/agents/scripts/invoke-codex-prompt.sh` / `.ps1` and `.claude/agents/scripts/invoke-claude-prompt.sh` / `.ps1`; invoke them synchronously and consume their single `ORCHESTRARIUM_PROVIDER_RESULT_V1` envelope, using complete `resultText`, combined outcome, cleanup status, and the wrapper process exit. For tracked runs, read the path-free terminal ledger back separately after return. Wrapper-private prompt, stdout, stderr, and process paths are not consumer surfaces. A transport-neutral inline chain must reproduce the same private-run lifecycle and terminal ordering; otherwise fail or reroute instead of using an argv prompt.
...
- A prompt-wrapper run is complete only after the provider reaches a terminal state and the wrapper performs this order: enforce one live combined raw-byte quota across supervisor-owned stdout/stderr pipes; extract one complete bounded result and primary outcome; dispose of the exact private run directory through validated same-parent tombstoning; emit and flush exactly one `ORCHESTRARIUM_PROVIDER_RESULT_V1=<json>` line; append exactly one optional path-free terminal ledger event; return. Codex runs as `exec --json` and `resultText` is the last strict UTF-8, newline-complete `item.completed` record whose `item.type=agent_message`; Claude uses its bounded stdout. Raw stdout and raw stderr are never duplicated into public output.
- The envelope carries bounded `primaryOutcome` metadata plus the combined `exitCode`, `token`, `status`, `gate`, `note`, `cleanupStatus`, `cleanupIssueCount`, `captureRecoveryRetained`, `cancelled`, `timedOut`, and `stderrMarkerCount`, plus provider/model/effort identity. It deliberately carries no ledger-status field because the ledger append occurs only after the envelope flush. `COMPLETE:PASS` and `COMPLETE:REVISE` mean the corresponding anchored final gate line was observed; each `UNVERIFIED:*` token names a failed oracle condition; `FAILED:nonzero-exit`, `FAILED:result-materialization`, `FAILED:cleanup-incomplete`, and `FAILED:result-emission` are distinct outcomes retaining bounded primary classification where available. A terminal-ledger failure after a successful flush is reported by stderr and a nonzero wrapper exit, never by a speculative envelope field. Any failed combined check remains non-PASS: re-dispatch it or return `BLOCKED:dependency`.
...
- `result envelope`: the single strict-prefix `ORCHESTRARIUM_PROVIDER_RESULT_V1=<json>` line emitted and flushed after complete bounded `resultText` materialization and private-run cleanup, but before the optional terminal ledger append; it carries no claim about that later append.

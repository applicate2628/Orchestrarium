...
3. **Re-dispatch** only the receipt-admitted angles on the revised artifact, carrying the ORIGINAL objective verbatim. Initial and full-artifact rounds admit all three angles; an eligible exact-delta correction round admits only its affected angles.
...
The initial round and every full-artifact round run all three angles. After a correction, the coverage choices are mutually exclusive: omit both flags, or pass `--full-review`, for backward-compatible full review; otherwise repeat `--affected-lane {surgical,deep,scout}` only when an accepted change classification identifies a non-empty strict subset and every unaffected effective result from the immediately previous round remains valid for the corrected artifact. Each retained verdict must resolve to `PASS`, and a retained scout result must be fully reconciled. Full review is required when the human requests it, a new defect class appears, the upstream artifact changes materially, or classification is unclear.

Launch only the fresh attempt IDs returned in the committed receipt's `attempts`; a delta receipt also carries `coverage`. A retained lane is provenance in the committed state, not a fresh verdict, finding, or run: it has no new attempt ID, must not be launched, and must not gain a manually copied `PASS`, rationale, findings, or reconciliation. Delta selection is provider- and model-agnostic and changes neither the existing dispatched role and external provenance requirements nor review authority, retry ownership and failure reconciliation, the three-round cap, or the absence of automatic continuous-integration enforcement.
...
A SHIPPED loop using the supported formal state path MUST use the installed `review_loop_state.py` owner. Resolve it from project-local `.claude/agents/scripts/` first and global `~/.claude/agents/scripts/` second. `begin` freezes the artifact, writes schema V2 atomically below `.scratch/reviews/<loop-id>/`, reads the committed bytes back, and only then emits one `ORCHESTRARIUM_REVIEW_LOOP_STATE_V2` receipt carrying all three initial attempt IDs and the round artifact revision. A full `next-round` keeps that three-attempt shape. A delta `next-round` emits `attempts` only for affected lanes plus `coverage`; retained-lane provenance remains in committed state rather than a separate receipt field. This is a supported `/agents-review-loop` procedure, not an executable host-enforced dispatch gate. Claude's `dispatch_sentinels.py` adds advisory same-role turn-depth observation for internal Agent dispatches; the helper itself does not observe bypass, and direct/ad-hoc launches outside that monitored surface remain outside its guarantees. The historical observer gap is fixed without expanding the helper's guarantees.

Schema V2 records immutable objective/scope/runtime-root anchors, ordered idempotent operations, and per-round phase, diff, frozen artifact, fresh attempts, optional exact-delta coverage and retained references, failures, results, and evidence. Full rounds have three current attempts; delta rounds give attempts only to affected lanes, while each unaffected lane is a retained reference to the immediately previous round and revision with no copied result. Each admitted lane's failures are one append-only immediate-successor chain (`A → B → C`) ending at its current attempt: one root and tip, no branch, merge, cycle, cross-lane reuse, dangling/disconnected node, or historical unresolved failure, and at most three retry edges. A retained lane cannot receive results, failures, or retries. Every attempt and failure echoes the owning round's `artifact_revision`; mismatch or mutation fails without counting a result. `admit-retry` creates a new attempt on the same revision, while `next-round` alone freezes a corrected artifact. Exact-delta admission does not change the global three-round cap. The engine rejects path/link escapes, flushes and atomically replaces state, validates read-back, and replays only identical operation IDs. A stable lock file carries an operating-system exclusive kernel lock; file presence is not ownership. Graceful cancellation cleans owned uncommitted resources. Hard termination cannot clean synchronously; the next lock-owning invocation reconciles unreferenced engine-owned resources, while invalid or uncertain state returns `RLSTATE_RECOVERY_REQUIRED` and preserves candidate evidence. The repository-only validator delegates to this same owner; it is a development/continuous-integration check, not new automatic enforcement. V1 stays read-only with `RLSTATE_V1_READ_ONLY`; explicit migration requires authoritative revisions and rejects ambiguous multi-failure history rather than guessing it. Provider completion and the work-item ledger remain independent owners.

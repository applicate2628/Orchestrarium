...
**Collection rule** (mirrors `review-loop.md`): independent lanes launch in parallel (`run_in_background: true`). Same-vendor Agent subagents fire a reliable notification — wait for those; an external-provider shell-out lane is verified by actively reading its captured `.out`/`.err` + process status, because that notification is not always delivered. This split tracks which runtime emits a completion callback — a collection-mechanism detail — not independence: independence remains scope/framing, never vendor (per `DP3`). Read each lane's captured OUTPUT before counting it. A died, timed-out, or empty lane is `UNVERIFIED` — re-run it with the same framing; never silently reduce quorum.

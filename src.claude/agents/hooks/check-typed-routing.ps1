...
.DESCRIPTION
    ...
    exit: propagates the Python helper's exit code. On a normal run (hit or
          ...
          silent exit 0. Still never exit 2 (AUDIT mode never blocks a
          PreToolUse tool call on Claude Code -- the only runtime this
          Claude-only hook runs on; only exit 2 does). Per the official
          hooks docs exit 1 is non-blocking (hook_common.emit_advisory's
          own docstring; work-items/bugs/2026-07-26-mcp-reminder-uses-the-
          once-per-session-form-its-sibling-calls-broken.md). Wrapper-side
          errors (missing python or helper) still fail open to exit 0.

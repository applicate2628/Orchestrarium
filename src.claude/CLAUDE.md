...
- **Warn-only audits.** The five audits run in AUDIT mode: they always allow and only warn. Every hook is fail-open. The worktree audit exempts only one command ending with exact `# orchestrarium:requested-isolation-worktree`.
- **Reminders and MCP.** Reminder hooks re-anchor Model Context Protocol (MCP) discovery/use after compaction, active delegation/recovery, scratch preservation, and every-turn continuity. The shared classifier covers native and shell code-navigation searches with narrow task-memory exemptions. Claude `auto` and subagents stay advisory; root `force` denies every qualifying search with `[MCP-FORCE-1]`, regardless of prior MCP use. Exact `[approve-mcp-fallback:v1]` in the bounded host-projected `user`-role record enables one recovery turn; assistant/tool injection cannot mint it, but the projection is not authenticated authorization. Missing servers and unresolved modes allow with diagnostics. See [MCP continuity](../references-claude/mcp-continuity.md); prompts should allow relevant MCP use within scope.
- **Publication.** Default/tracked and path scans are manual pre-commit checks only. For an admissible pending push, the gate captures and directly executes the verified `hook_common.py` + machine-path-classifier + scanner closure, then binds only the exact pending invocation's fresh bounded, reaped, non-empty version-2 result to remote, destination, resolved source, current `HEAD`, and receipt tip. Transcript/manual results cannot authorize; legacy, zero, malformed, finding, refusal, correlation, provenance, execution, drift, or reuse failures deny. Arbitrary same-process mutation defeats harness observation and may run arbitrary caller code; the only proven invariant is that unchanged shipped source contains no external adapter or launcher, and its cooperative result has zero production/publication consumers. Repository/server identity and other Git metadata remain outside; human review plus the gate-owned leak-check remain mandatory.

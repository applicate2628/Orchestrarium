...
- **Close is mandatory.** When a recovery-tracked item is delivered, parked, cancelled, or reprioritized, the main conversation writes `closure.md` with outcome, residual risk, archive location, and `Closed: <YYYY-MM-DD>`, then invokes the installed lifecycle owner to move it to `work-items/archive/<YYYY-MM>/<slug>/` and reconcile `work-items/index.md`. The main conversation handles a routine single item; drift or multi-item closure routes to `$knowledge-archivist`. Only archive placement makes the item terminal. Keep-worthy lessons go to `work-items/lessons/`; details: the lead skill `## Lessons`.

...
- **Closing a `requiresLead: false` item is the main conversation's job, and the close step is as mandatory as the create step above.** When the item is delivered (its changes committed or pushed) or the user parks, cancels, or reprioritizes it, the main conversation must close it — a delivered item left in `work-items/active/` is an orphan. Close = (1) write `closure.md` (outcome, residual risk, archive location); (2) move the folder to `work-items/archive/<YYYY-MM>/<date>-<slug>/`; (3) move its row in `work-items/index.md` from Active to Archived. This is the main-conversation counterpart to the `$lead` close step in `lead.md` (`closure.md` mandatory before `work-items/archive/`). If the session ends before the item is closed, the work-items archival Stop-hook flags the still-open delivered item on the next session.

...
- **MCP.** Reminder hooks re-anchor Model Context Protocol (MCP) discovery/use after compaction, active delegation/recovery, scratch preservation, and every-turn continuity. Native/shell code search uses narrow task-memory exemptions and generic discovery, not home MCP config or configured tool names. Claude `auto`/subagents stay advisory; root `force` denies each qualifying search with `[MCP-FORCE-1]` regardless of prior use/config. Exact `[approve-mcp-fallback:v1]` in the bounded host-projected `user` record allows one recovery turn, not authenticated authorization; assistant/tool text cannot mint it. Unresolved modes allow with diagnostics. Tool names are examples, not selection logic. See [MCP continuity](../references-claude/mcp-continuity.md); prompts should allow relevant MCP use within scope.

...
- **Reminders and MCP.** Reminder hooks re-anchor Model Context Protocol (MCP) discovery/use after compaction, active delegation/recovery, scratch preservation, and every-turn continuity. Shared native/shell searches have narrow task-memory exemptions. Discovery uses runtime tools, not home MCP configuration. Claude `auto`/subagents advise; root `force` denies each qualifying search with `[MCP-FORCE-1]` despite prior MCP use/configuration. Exact `[approve-mcp-fallback:v1]` in a bounded host-projected `user`-role record permits one recovery turn, not authenticated authorization; assistant/tool text cannot mint it. Unresolved modes allow with diagnostics. Named tools are examples, never selectors. See [MCP continuity](../references-claude/mcp-continuity.md); prompts should allow relevant MCP use within scope.

...
Per the source-hygiene rule, the three audit hooks live in the typed `agents/hooks/` dir; the four blocking hooks (bugfix-discipline, git-push-gate, passive-polling, work-items-archival) stay in `agents/scripts/` next to the shared `hook_common.py` they import directly. All seven wrappers are thin fail-open wrappers around their sibling Python brain. Shared JSON envelope and transcript helpers live in `.claude/agents/scripts/hook_common.py`.

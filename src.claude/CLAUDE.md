...
3. Subagents may invoke common-skills (`$bug-hunting`, `$analyzing-video-bugs`, `$windows-gui-manual-testing`, `$mathtype-book-page`, `$repo-cleanup`, `$manual-repo-transfer`, `$github-pr-review-bot`, `$explain-simply`, `$vak-dissertation-review`, `$vcpkg-ports-updater`, `$vcpkg-builder`) via in-context `Skill`. They typically cannot spawn subagents; common-skills canonically share methodology across the delegation tree.

...
- **Reminders and MCP.** Reminder hooks re-anchor Model Context Protocol (MCP) discovery/use after compaction, active delegation/recovery, scratch preservation, and every-turn continuity. Native/shell searches share narrow task-memory exemptions. Discover MCP at runtime, not from home config/names. Claude `auto`/subagents stay advisory; root `force` denies each qualifying search (`[MCP-FORCE-1]`) regardless of prior MCP use/config. One recovery turn requires exact `[approve-mcp-fallback:v1]` in the bounded host-projected `user` record; assistant/tool text cannot mint it; projection is not authenticated authorization. Unresolved modes allow with diagnostics. Tool-name examples never drive selection. See [MCP continuity](../references-claude/mcp-continuity.md); prompts should allow relevant MCP use within scope.

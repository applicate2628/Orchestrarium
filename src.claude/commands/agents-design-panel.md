...
5. **Collect.** Verify each external lane through the approved thin wrapper owner in `agents/contracts/external-dispatch.md` — its strict V2 parser, full external-nonauthorizing tuple, and untrusted/potentially-sensitive resultText contract — not through a notification or private capture. A died, timed-out, or empty lane is `UNVERIFIED` — re-run it with the same framing; never silently reduce quorum below `N=2`.

...
- how `externalCodexProfile: default | gpt-5.5-fast | gpt-5.5-xhigh` controls Codex-specific profile choice after provider resolution (where `gpt-5.5-fast` selects the fast Codex model tier with reasoning_effort still at `xhigh`, and `gpt-5.5-xhigh` pins model `gpt-5.5` with `model_reasoning_effort = "xhigh"`)

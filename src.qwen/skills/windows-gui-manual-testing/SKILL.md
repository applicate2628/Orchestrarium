...
description: Use when Qwen needs evidence-backed manual visual verification of Windows desktop GUI before/after a fix across Qt, Avalonia, WinUI/WPF, WebView/native-child, or other Windows surfaces via screenshots, video frames, or live inspection.
---
...
2. **The agent captures the video itself** when no recording exists. Use ffmpeg's `gdigrab` on Windows:
   ...
   ffmpeg -f gdigrab -framerate 60 -i desktop -t 9 -c:v libx264 -preset ultrafast -pix_fmt yuv420p .scratch/capture.mp4

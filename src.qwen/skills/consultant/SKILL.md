...
- the consultant lane runs at high effort by default regardless of the operator-set `externalModelMode` or `externalCodexProfile`: when Codex resolves, model `gpt-5.6-sol` with `model_reasoning_effort = "xhigh"`; when Claude resolves, `--model opus --effort xhigh`. `xhigh` is the default for both providers; for especially heavy / complex tasks the orchestrator may escalate the consultant to the provider's deepest tier (Claude `--effort max`). Do not downgrade consultant memos below `xhigh` — not to `gpt-5.6-terra`, runtime-default, or a lower `--effort` — between attempts on the same consultant lane

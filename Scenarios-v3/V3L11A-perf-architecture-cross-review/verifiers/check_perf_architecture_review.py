#!/usr/bin/env python3
"""Verifier for V3L11A - performance-architecture cross-review discriminator.

Findings-table oracle (N07/S28 family): file:line-bound required findings with exact
severity/category/evidence binding, an exact finding count, forbidden false-positive
traps, required false-positive-boundary terms, and a gate decision. Read-only; executes
no candidate code.

Separation design (near-peer, not merely hard): the four real findings require tracing
cost across files (LaneStore rebuild in store.py) and recognizing a quadratic hidden in a
comprehension, while three planted decoys are surface-level "perf smells" that a near-peer
weaker reviewer plausibly flags. The exact_finding_count + forbidden_findings + distinct-row
matching is the separation mechanism, not raw difficulty.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Check the V3L11A perf-architecture review bundle or report.")
...
def load_contract(bundle_root: Path):
    return json.loads(
        (bundle_root / "oracle" / "perf-architecture-review-contract.json").read_text(encoding="utf-8")
    )
...
def parse_simple_yaml(path: Path):
    ...
    for raw_line in path.read_text(encoding="utf-8").splitlines():
        ...
        if not line or line.lstrip().startswith("#"):
            continue
        if line.startswith("  - "):
            data.setdefault(current_list, []).append(strip_quotes(line[4:].strip()))
            continue
...
def check_bundle_shape(bundle_root: Path, contract, errors):
    ...
    require(
        ...
        "scenario.yaml metadata does not match V3L11A",
        errors,
    )
...
def main():
    ...
    print(f"V3L11A verifier PASS ({mode})")
    return 0

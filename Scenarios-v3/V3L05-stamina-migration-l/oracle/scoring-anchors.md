# Scoring anchors -- V3L05 stamina (l)

## Graded measure
- `breadth_fraction = migrated_correct / 36` (0..1), reported per run.
...
- Rationale for 0.9: the migration is ONE atomic contract shared by all
  ...
  migration. The visible tests cover only 2 of 36 consumers
  (5.6%), far below the bar, so a "stop when tests pass" model fails by

# Scoring anchors -- V3L05 stamina (s)

## Graded measure
- `breadth_fraction = migrated_correct / 8` (0..1), reported per run.
...
- Rationale for 0.9: the migration is ONE atomic contract shared by all
  ...
  migration. The visible tests cover only 2 of 8 consumers
  (25.0%), far below the bar, so a "stop when tests pass" model fails by

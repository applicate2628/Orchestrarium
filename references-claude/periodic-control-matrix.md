...
| Freshness audit for active items | `$lead` | Every resume or session start | Physical `work-items/active/` membership plus each active item `status.md` current snapshot | Update `status.md`, or park/archive the item through the lifecycle owner before continuing |
...
| Physical-state reconciliation | `$knowledge-archivist` | Every resume, lifecycle move, or completion | Physical backlog/active/archive roots are exclusive and generated `work-items/README.md` reflects them | Repair through the lifecycle owner before delivery continues; `index.md` is compatibility-only |
...
| Closure and archive hygiene | `$knowledge-archivist` | Monthly or at milestone close | Completed or cancelled items are in `work-items/archive/` with exclusive physical placement and a refreshed `work-items/README.md` read-model | Archive the item, reconcile physical roots, and regenerate the read-model |

...
> **Note**: this file is a standalone reference/addendum, not the authoritative operator blueprint. Canonical operator truth for `.claude/.agents-mode.yaml`, the accepted init-time preset family, the shared lane matrix, and Claude transport semantics lives in [../docs/agents-mode-reference.md](../docs/agents-mode-reference.md). Use `.claude/CLAUDE.md` for runtime entry behavior.
...
- `$consultant` is an optional independent advisory role; it may be fulfilled by an external provider or an internal independent subagent, but it never becomes a delivery gate. Its current mode is controlled through `.claude/.agents-mode.yaml` via `consultantMode`; provider selection and Claude transport behavior follow the same file's `externalProvider`, `externalClaudeSecretMode`, and `externalClaudeApiMode` keys. Init-time presets are shortcuts only and are not persisted as a preset name.

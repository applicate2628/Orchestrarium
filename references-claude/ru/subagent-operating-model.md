...
- `$consultant` — опциональная независимая advisory-роль; она может исполняться внешним провайдером или внутренним независимым subagent, но никогда не становится delivery gate. Consultant включается через consultant-mode file. Нет файла = disabled. Имя файла не меняется ради backward compatibility, но схема теперь расширена: `mode`, `preferExternalWorker`, `preferExternalReviewer`. `mode` по-прежнему управляет только consultant, а флаги preference направляют eligible implement-роли в `$external-worker`, а eligible review/QA-роли — в `$external-reviewer`.
- `$external-worker` и `$external-reviewer` — это двусторонние external adapters, а не новые узкие профессии. В Claude Code они dispatch'ят в Codex CLI; в Codex — в Claude CLI. Сам adapter не делает silent fallback на внутреннего specialist. Если внешний CLI недоступен, роль считается disabled, а orchestrator может reroute'ить работу в обычную internal role.
...
| `external-worker` | External implementation adapter | Выполняет любую eligible implement-side role через другой CLI, сохраняя assigned role как provenance | External implementation package | Одобренная implementation-фаза проходит через external adapter без изменения semantics ownership |
| `external-reviewer` | External review and QA adapter | Выполняет любую eligible review-side или QA-side role через другой CLI, сохраняя assigned role как provenance | External review или QA report | Eligible review/QA slot проверяется через external adapter без замены mandatory internal safety gates |
